...
#pragma omp parallel for schedule(dynamic, 256)
...
#endif
        if(All.TopNodeAllocFactor > topnode_limit) {printf("something seems to be going seriously wrong here. Stopping.\n"); fflush(stdout); gizmo_hard_abort_reviewed(90000010, "TEMP_HARD_CANDIDATE_MIDCOLLECTIVE: original endrun(781) -- TopNodeAllocFactor runaway (mid domain-decomp; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);}
      }
    }
    while(retsum);
    ...
    for(i = 0; i < NumPart; i++) {if(P[i].Type > 5 || P[i].Type < 0) {printf("task=%d:  P[i=%d].Type=%d\n", ThisTask, i, P[i].Type); gizmo_hard_abort_reviewed(90000011, "TEMP_HARD_CANDIDATE_MIDCOLLECTIVE: original endrun(111111) -- invalid P[i].Type post-decomp (per-rank; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);}}
...
#pragma omp parallel for schedule(dynamic, 256)
    ...
    do
    {
        ...
        if(exchange_limit <= 0) {gizmo_hard_abort_reviewed(90000012, "TEMP_HARD_CANDIDATE_MIDCOLLECTIVE: original endrun(1223) -- exchange_limit<=0 in domain_Decomposition_light (mid-exchange; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);}
...
void domain_free_trick(void)
{
  if(domain_allocated_flag)
    ...
    {gizmo_hard_abort_reviewed(90000013, "TEMP_HARD_CANDIDATE_INTERNAL: original endrun(131231) -- domain_free_trick called when not allocated (invariant; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);}
}
...
int domain_decompose(void)
{
    ...
    do
    {
        ...
        if(exchange_limit <= 0)
        {
            ...
            gizmo_hard_abort_reviewed(90000014, "TEMP_HARD_CANDIDATE_MIDCOLLECTIVE: original endrun(1223) -- exchange_limit<=0 in domain_Decomposition (mid-exchange; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);
        }
  ...
  if(NumPart > All.MaxPart)
    {
      ...
      gizmo_hard_abort_reviewed(90000015, "TEMP_HARD_CANDIDATE_MIDCOLLECTIVE: original endrun(787878) -- NumPart>MaxPart after domain_exchange (per-rank; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);
    }

  if(N_gas > All.MaxPartGas)
    gizmo_hard_abort_reviewed(90000016, "TEMP_HARD_CANDIDATE_MIDCOLLECTIVE: original endrun(787879) -- N_gas>MaxPartGas after domain_exchange (per-rank; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);
...
int domain_countToGo(size_t nlimit)
{
    ...
    if(package >= nlimit) {gizmo_hard_abort_reviewed(90000017, "TEMP_HARD_CANDIDATE_MIDCOLLECTIVE: original endrun(212) -- package>=nlimit in domain_countToGo (mid-exchange-sizing; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);}
    ...
    if(retsum)
    {
        ...
        do
        {
            ...
            do
            {
                ...
                if(flagsum > 100) {if(ThisTask==0) {printf("Failed to converge in domain.c, flagsum=%d",flagsum); fflush(stdout);} gizmo_hard_abort_reviewed(90000018, "TEMP_HARD_CANDIDATE_MIDCOLLECTIVE: original endrun(1013) -- domain exchange failed to converge (mid-collective Bcast loop; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);}
                MPI_Alltoall(toGo, 1, MPI_INT, toGet, 1, MPI_INT, MPI_COMM_WORLD);
                MPI_Alltoall(toGoGas, 1, MPI_INT, toGetGas, 1, MPI_INT, MPI_COMM_WORLD);
            }
            while(flag);
  ...
  if(ncpu >= 2)
    {
      ...
      if(domainkey_top_left == domainkey_top_right) {gizmo_hard_abort_reviewed(90000019, "TEMP_HARD_CANDIDATE_INTERNAL: original endrun(123) -- degenerate top-tree key split (mid tree-combine; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);}
...
int domain_determineTopTree(void)
{
  ...
  if(GrNr >= 0 && count != NumPartGroup)
    gizmo_hard_abort_reviewed(90000020, "TEMP_HARD_CANDIDATE_INTERNAL: original endrun(1222) -- count!=NumPartGroup in determineTopTree (SUBFIND invariant; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);
#endif
  ...
  if(NTopleaves < multipledomains * NTask) {gizmo_hard_abort_reviewed(90000021, "TEMP_HARD_CANDIDATE_INTERNAL: original endrun(112) -- NTopleaves < multipledomains*NTask (mid determineTopTree; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);}
  ...
  if(treeB[noB].Size < treeA[noA].Size)
      ...
      if(treeA[noA].Daughter < 0)
	{
	  if((NTopnodes + 8) <= MaxTopNodes)
	    ...
	    gizmo_hard_abort_reviewed(90000022, "TEMP_HARD_CANDIDATE_INTERNAL: original endrun(88) -- out of TopNodes in domain_insertnode (mid tree-combine; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);
	}
    ...
    gizmo_hard_abort_reviewed(90000023, "TEMP_HARD_CANDIDATE_INTERNAL: original endrun(89) -- unexpected node size in domain_insertnode (mid tree-combine; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);
}
