...
def _force_softening():
    ...
    return 2.8 * float(parse_params(f"{TEST_DIR}/{VARIANT}.params")["Softening_Type5"])
...
def _plot_summary():
    ...
    ax_rho.set_title(f"{VARIANT}: final density profile (initial in black)")
    ...
    plt.tight_layout()
    plt.savefig(f"{TEST_DIR}/{VARIANT}_summary.png", dpi=120)
    plt.close()
...
def _plot_variant_density_evolution(variant_id, snaps):
    ...
    ax.set_title(f"{VARIANT} [{variant_id}]: density profile per snapshot")
    ax.legend(fontsize=7, ncol=2)
    plt.tight_layout()
    plt.savefig(f"{TEST_DIR}/{VARIANT}_{variant_id}_density.png", dpi=120)
    plt.close()
...
@pytest.mark.parametrize("extra_config_flags", [
    ...
    # rejected together at compile time (see precompiler_logic.h).
