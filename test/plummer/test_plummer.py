...
# --- secular energy-drift rate, per variant ----------------------------------------------
# Companion to the |dE|/KE_0 < 0.04 endpoint check below, which is a blow-up guard: the worst
# The reference variant sits at 1.7e-2, so a regression has to nearly triple the worst
# legitimate drift before that assertion notices.
#
# This one gates the SHAPE. A symplectic KDK leapfrog's energy error oscillates about zero
# within a bound; a systematic loss trends. Statistic is the least-squares slope of the SIGNED
# (E(t)-E0)/|E0| against t -- signed, because rectifying with |dE| first turns a healthy
# oscillation into an apparent trend. Being a rate, it is independent of TimeMax, so it does
# not need recalibrating if the run length changes. See test/hernquist/test_hernquist.py,
# where the same gate was derived and the reasoning is written out in full.
#
# Rates below are the reference run's, fitted over its full TimeMax=48 trajectory, times 3x.
# Per variant because secular drift is not itself a defect: ags and tidal_ags genuinely drift
# here (3.5e-4), a hundred times baseline's.
#
# Worth noting against hernquist, whose tidal variant IS secular at 9.5e-4 with no sign
# changes: plummer's tidal is bounded at 3.2e-6 with four. That 300x split between a cored and
# a cuspy profile is the Grudic & Hopkins (2020) result -- the tidal criterion doing better on
# Plummer and worse on Hernquist -- showing up as integrator character rather than a tolerance.
ENERGY_SECULAR_RATE = {
    "baseline": 4.643e-06,
    "ags": 3.504e-04,
    "ewald": 4.351e-06,
...
def _secular_energy_rate(times, energies):
    """Secular drift rate of the relative energy error: least-squares slope of
    (E(t)-E0)/|E0| against t, per unit time. Signed, so a bounded oscillation fits ~0."""
    if len(times) < 3:
        return 0.0
    y = (np.asarray(energies, dtype=float) - energies[0]) / abs(energies[0])
    return abs(float(np.polyfit(np.asarray(times, dtype=float), y, 1)[0]))
...
def test_plummer(num_mpi_ranks, num_omp_threads, extra_config_flags, request):
    ...
    calibrated = "recorded reference run" if variant_id in ENERGY_SECULAR_RATE else "default (uncalibrated)"
    assert rate < rate_max, (
        ...
        f"over t={times[0]:g}..{times[-1]:g}. A symplectic integrator should oscillate within "
        f"a bound, not trend; see ENERGY_SECULAR_RATE."
    )
