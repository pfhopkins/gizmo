...
def make_HII_region_ics(output_file="HII_region_ics.hdf5"):
    ...
    pos_gas = (pos_glass - pos_min) / glass_size * BoxSize

    # Gas properties: mean density = 100 H nuclei/cm^3
    # rho = n_H * m_p / X_H, M_gas = rho * BoxSize^3
    # With X_H = 0.70 (solar), n_H = 100/cc: M_gas ~ 28245 Msun
    M_gas_total = 28244.6  # Msun
    ...
    # Solar metallicity: [Z_total, He, C, N, O, Ne, Mg, Si, S, Ca, Fe]
    Z_solar = np.array(
    ...
    with h5py.File(output_file, "w") as f:
        ...
        s.create_dataset("StellarAge", data=np.array([0.0], dtype=np.float32))
        # ZAMS star: ProtoStellarStage=5 (main sequence)
        s.create_dataset("ProtoStellarStage", data=np.array([5], dtype=np.int32))
        s.create_dataset("ProtoStellarRadius_inSolar", data=np.array([max(10. * M_star, 5.24 * M_star**(1./3.))], dtype=np.float32))  # proper initial protostellar radius for this mass
    ...
    print(f"  Gas mass per particle = {m_gas:.6e} Msun")
    print(f"  Mean n_H ~ 100 cm^-3 (M_gas_total = {M_gas_total:.1f} Msun)")
    print(f"  Star mass = {M_star} Msun (ZAMS)")
