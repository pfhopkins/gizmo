#!/usr/bin/env python3
"""Compare GPU vs CPU outputs for Phase 2-D (COSMIC_RAY_SUBGRID_LEBRON) in the
isodisk_mechfb_cr test under FIRE_CRS=-2.

Validates at snapshot_000:
  - Coordinates : bitwise identical
  - Velocities  : FP-order reldiff < 1e-4
  - SubGrid_CosmicRayEnergyDensity (if present) : reldiff < 1e-4
"""

import h5py, numpy as np, sys, os

CPU_DIR = "output_cpu_phase2d"
GPU_DIR = "output_gpu_phase2d"
SNAP    = 0
...
def load_snap(outdir, snap_idx):
    ...
    for ptype in ("PartType0", "PartType1", "PartType2", "PartType3", "PartType4", "PartType5"):
        ...
        for field in ("SubGrid_CosmicRayEnergyDensity",):
            if field in grp:
...
GAS_TOLS = {
    "Coordinates": 1e-12,
    "Velocities":  1e-4,
    "SubGrid_CosmicRayEnergyDensity": 1e-4,
}
OTHER_TOLS = {
    "Coordinates": 1e-12,
    "Velocities":  1e-4,
...
all_pass = True
print(f"=== isodisk_mechfb_cr Phase 2-D: snapshot_{SNAP:03d} CPU vs GPU ===")
for ptype, tols, label in [
    ...
    ("PartType4", OTHER_TOLS, "PartType4 (stars)"),
    ("PartType5", OTHER_TOLS, "PartType5 (sink)"),
]:
...
print()
if all_pass:
    print("RESULT: PASS — GPU Phase 2-D walk matches CPU within tolerances")
else:
