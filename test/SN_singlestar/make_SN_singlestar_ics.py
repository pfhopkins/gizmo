#!/usr/bin/env python3
"""Generate ICs for SN_singlestar test: a 10 Msun star with age 50 Myr (already past
its main-sequence lifetime, so it explodes on the first timestep) at the center of a
uniform gas box. Total gas mass is 50000 Msun, and the box size is set so the ambient
hydrogen number density is n_H = 100 cm^-3 assuming GIZMO's default solar metallicity
(X_H = 0.70)."""
...
MSUN_IN_G = 1.989e33
KMS_IN_CMS = 1.0e5
# Code time unit (= pc / (km/s)) in Myr
CODE_TIME_IN_MYR = (PC_IN_CM / KMS_IN_CMS) / (1.0e6 * 365.25 * 86400.0)  # ~0.9778 Myr


def make_SN_singlestar_ics(output_file="SN_singlestar_ics.hdf5"):
    ...
    # Map to [0, BoxSize) — exclude the upper face so periodic wrap doesn't
    # collide upper-face particles with lower-face ones.
    pos_gas = (pos_glass - pos_min) / (glass_size * (1.0 + 1e-6)) * BoxSize
    vel_gas = np.zeros((Ngas, 3), dtype=np.float32)

    # Star: 10 Msun, age 50 Myr (well past the ~17 Myr MS lifetime → SN on first
    # timestep). With INPUT_READ_SINKPROPS, GIZMO reads StellarAge from the IC; we
    # encode the age as a negative formation time in code units (pc/(km/s) ~ 0.978 Myr).
    M_star = 10.0
    star_age_Myr = 50.0
    StellarAge_code = -star_age_Myr / CODE_TIME_IN_MYR
    ...
    with h5py.File(output_file, "w") as f:
        ...
        s.create_dataset("StellarFormationTime", data=np.array([StellarAge_code], dtype=np.float32))
        # Main-sequence star (stage 5); SN check will then promote it to stage 6
        ...
        s.create_dataset("ProtoStellarAge", data=np.array([StellarAge_code], dtype=np.float32))
    ...
    print(f"  Star: M = {M_star} Msun, age = {star_age_Myr} Myr  (StellarAge = {StellarAge_code:.4f} code units)")


if __name__ == "__main__":
    make_SN_singlestar_ics()
