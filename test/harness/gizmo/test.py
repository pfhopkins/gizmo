...
def physical_cores():
    """Number of physical cores, which is what core binding allocates -- not cpu_count(),
    which counts hardware threads and so doubles the answer wherever SMT is on."""
    if platform.system() == "Darwin":
        try:
            return int(subprocess.run(["sysctl", "-n", "hw.physicalcpu"],
                                      capture_output=True, text=True, check=True).stdout)
        except (OSError, ValueError, subprocess.CalledProcessError):
            pass
    else:
        try:
            cores, pkg, core = set(), None, None
...
def default_mpi_ranks(max_ranks=None):
    """Return the number of MPI ranks to use. Optionally cap at max_ranks.

    Fixed rather than derived from the CPU count. A rank count that varies by machine
    makes every rank-sensitive tolerance in the suite machine-dependent: the same test
    then passes on one host and fails on another with more cores, which is a property of
    the harness rather than of the code under test. Two ranks is also the minimum that
    exercises the ghost exchange at all, so it is what the correctness bars need, and it
    leaves the rest of the machine free for other work."""
    n = DEFAULT_MPI_RANKS
...
def download_test_files(test_name: str):
    ...
    # those, and the test then failed for want of a file that was available all along.
...
def _resolve_test_timeout(timeout):
    """Effective subprocess timeout: explicit argument, else GIZMO_TEST_TIMEOUT, else the default."""
...
def _run_output_dir(test_name: str, paramsfile: str) -> str:
    """A run's OutputDir, relative to the test directory (i.e. to run_test's working directory)."""
    try:
...
def mark_run_truncated(test_name: str, reason: str, paramsfile: str = None):
...
def run_truncated_reason(test_name: str, extra_config_flags=()):
...
def run_test(test_name: str, num_mpi_ranks: int = 1, num_openmp_threads: int = 0,
    ...
    for the rare test whose subject IS a failed run, where the exit check would otherwise fire
    before the test's own assertions can run.
    No-op when GIZMO_TEST_SKIP_BUILD_RUN is set (we're validating externally produced snapshots)."""
    if environ.get("GIZMO_TEST_SKIP_BUILD_RUN"):
        return
    # Always set it. Left unset, the OpenMP runtime gives every rank the whole node: main.cc's
    # omp_get_num_threads() then reports all cores, so an 8-rank run on a 96-core node asks for
    # 8 x 96 threads, while pinning.cc reads the same unset variable and lays out cpusets
    # assuming one thread per rank. "No OpenMP requested" has to mean one thread, not all of them.
    ...
    if num_openmp_threads > 0 and environ.get("GIZMO_TEST_BIND_CORES") \
            and num_mpi_ranks * num_openmp_threads <= physical_cores():
        # Opt-in: give each rank its own set of cores and keep that rank's threads inside it.
        # Measured worth 34% on a 48x2 Genoa node, but it is off by default because this
        # machine routinely runs several independent tests at once from sibling checkouts,
        # and pinned ranks from one of them contend with every other. PE=n also needs
        # ranks*threads <= physical cores or the job fails to launch rather than merely
        # running slowly, and it cannot be combined with --use-hwthread-cpus.
    ...
    with open(outfile, "w") as out, open(errfile, "w") as err:
        # Record how the run was actually launched. Rank and thread counts and the binding
        # policy are decided here, so without this the only evidence of the geometry a given
        # result came from is gone the moment the process exits.
...
def _test_dir(test_name: str, caller: str):
    ...
    The stash and variant moves below must not depend on the working directory: ~25 tests
    chdir into their own directory and call finalize_variant_output() from a finally block,
    and any raise inside the body skips the chdir back. run_test() raises exactly that way
    when GIZMO exceeds its timeout, so a timed-out variant reaches the finally from the test
    directory, where every relative path misses -- turning a clean skip into a RuntimeError
    and leaving the baseline stashed with the variant's output standing in for it.
...
def assert_final_time(snapshot_file: str, test_name: str, rtol: float = 1e-6, time_max: float = None):
