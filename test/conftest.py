...
def _restore_cwd():
    ...
    confusing FileNotFoundError instead of its real assertion, so a single early failure
    turns into a cascade of unrelated ones. Guarding it centrally fixes all of them
    without touching each test, and keeps working if someone adds another chdir."""
    saved = os.getcwd()
    try:
        yield
    finally:
...
def pytest_configure(config):
    # Tests too expensive for a routine full sweep. Run them with `pytest test/<name>`, or
