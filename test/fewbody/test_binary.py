...
# This problem shares a directory with the rest of its family; TEST_NAME is that
...
def _ensure_ic():
    ...
    subprocess.run(
        [sys.executable, "make_binary_ics.py", "--m1", str(M1), "--q", str(Q),
         "--a_au", str(A_AU), "--ecc", str(ECC), "--out", f"{VARIANT}_ics.hdf5"],
...
def _plot(t, energy, ecc, drift, variant_id):
    ...
    fig.tight_layout()
    fig.savefig(f"{TEST_DIR}/{VARIANT}_{variant_id}_conservation.png", dpi=120)
    plt.close(fig)
...
def _plot_convergence(etas, errs, order, variant_id, series=None):
    ...
    fig.tight_layout()
    fig.savefig(f"{TEST_DIR}/{VARIANT}_{variant_id}_convergence.png", dpi=120)
    plt.close(fig)
...
@pytest.mark.parametrize("extra_config_flags", [
    ...
    # On request only: this records how far KDK drifts, which is the comparison Hermite
    # exists to win. A non-strict xfail could neither fail nor report that it had started
    # passing, so it said nothing as a suite row.
    pytest.param(("DISABLE_HERMITE_INTEGRATION",), id="kdk", marks=pytest.mark.slow),
])
