...
case "$STAGE" in
    ...
    stage5) TESTS="shu_jets wind_singlestar SN_singlestar binary triple hernquist gmc_cooling fire isodisk_mechfb_sinks isodisk_thermalfb isodisk_dmdispersion compile_suite";; # thermalfb+dmdispersion are non-MAINTAIN GALSF: the only automated tests where the stage-5 deferred-rebuild ladder branch runs
