...
**MHD\_MODIFIED\_GRADIENT**: Activate this to use the Modified-Gradient (MG) method of Tu, Wang, Gao & Tang (2026, arXiv:2603.04077) for machine-precision divergence-free magnetic field reconstruction. Unlike the CG method above, MG formulates the divergence constraint as a symmetric positive-definite sparse linear system and solves it using an algebraic multigrid (AMG) preconditioned conjugate gradient solver (via HYPRE, if available, or Intel MKL PARDISO). This achieves true machine-precision div(B)=0 at the cost of a global sparse solve per (active) timestep. The runtime parameter `ActiveFractionForMGSweep` (default: apply every step) can be set to a value between 0 and 1 to limit the MG correction to timesteps where at least that fraction of cells are active, with the cheaper CG method used on smaller steps. This flag is **mutually exclusive** with `MHD_CONSTRAINED_GRADIENT`. Please cite Tu, Wang, Gao & Tang 2026 if you use this method.
