...
int read_agetracerlist(char *fname)
{
    ...
    while(1)
    {
      ...
      if(count == 1 || count == 2)
      {
          if(i >= NUM_AGE_TRACERS+1) {PRINT_WARNING("Too many entries in enrichment-age-tracer list. You should increase NUM_AGE_TRACERS=%d",(int)NUM_AGE_TRACERS); endrun(314);}
          i++;
      }
    }
