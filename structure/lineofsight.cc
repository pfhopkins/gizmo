...
void lineofsight_output(void)
{
  ...
  if(ThisTask == 0)
    {
      snprintf(buf, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s/los", All.OutputDir);
      mkdir(buf, 02755);
    }

  Los = (struct line_of_sight *) mymalloc("Los", sizeof(struct line_of_sight));
  LosGlobal = (struct line_of_sight *) mymalloc("LosGlobal", sizeof(struct line_of_sight));
...
void find_particles_and_save_them(int num)
{
  ...
  countlist = (int *) mymalloc("countlist", sizeof(int) * NTask);
  particles = (struct line_of_sight_particles *) mymalloc("particles", sizeof(struct line_of_sight_particles) * N_gas);
  ...
  if(ThisTask == 0)
    {
      snprintf(fname, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s/los/part_los_z%05.3f_%03d.dat", All.OutputDir, 1 / All.Time - 1, num);
...
void output_lines_of_sight(int num)
{
  ...
  snprintf(fname, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s/los/spec_los_z%05.3f_%03d.dat", All.OutputDir, 1 / All.Time - 1, num);
