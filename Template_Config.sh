...
#HYDRO_MULTIFLUID               # enable strict Lagrangian-partition multi-fluid framework: each particle carries a canonical FluidType ID (see declarations/multifluid_helpers.h::FluidID); within-fluid hydro pair operators skip cross-pairs; cross-fluid coupling physics (drag, ion-neutral collisions, charge exchange, etc.) lives as in-place cases inside the existing evaluator + backreaction prototypes (NOT a dispatcher layer). NOTE: force-implies EOS_GENERAL so per-cell SoundSpeed/Pressure/Temperature are saved per-element and FluidType branches live centralized in the EOS compute path instead of scattered through hydro readers (see declarations/precompiler_logic.h). Sub-flags HYDRO_MULTIFLUID_* gate enabled coupling cases; canonical FluidIDs are stable across Configs regardless of sub-flag state. Full design: gpu_bench_new/OPEN_hydro_multifluid_design.md.
#HYDRO_MULTIFLUID_NOOP_TEST     # TEST-ONLY validation flag: disables the hydro-corridor cross-fluid skip predicate so strict-bit-identity Test 2 can compare HYDRO_MULTIFLUID + all-FluidType=0 against the EOS_GENERAL baseline (the always-true predicate otherwise produces ULP-level codegen FP-reorder accumulating to ~1e-12 over many timesteps). NEVER set this in a real science run. Requires HYDRO_MULTIFLUID; forbidden in combination with any HYDRO_MULTIFLUID_* subflag enabling non-default fluids (hard-#error in declarations/precompiler_logic.h).
#HYDRO_MULTIFLUID_DUSTGAS       # Phase 3+ first-coupling demonstrator subflag: enables FluidType=FLUID_DUST_GRAIN(=1) particles and (in Phase 3b/3c) the FluidType-eligibility-widened generalization of the existing GRAIN_FLUID dust-gas drag (solids/grain_drag_gpu.cc evaluator) + backreaction (solids/grain_physics_functions.h::grain_backrx_pair_kernel). Implies HYDRO_MULTIFLUID (parent flag auto-set in declarations/precompiler_logic.h — users only set the subflag). REQUIRES GRAIN_FLUID (hard-#error otherwise — nothing to generalize without the parent dust-gas coupling). Phase 3a: enablement-only (init.cc allowed-set extended; no eligibility refactor yet, so building with this flag and a Type=3 dust IC currently runs the legacy unmodified GRAIN_FLUID path).
## ----------------------------------------------------------------------------------------------------
