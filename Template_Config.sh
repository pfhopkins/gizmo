...
#ENERGY_BUDGET_DIAGNOSTIC           # report the total gas energy (thermal+kinetic) at every full synchronization, i.e. when All.HighestActiveTimeBin == All.HighestOccupiedTimeBin -- the only point at which Vel and InternalEnergy share a clock and the sum is a genuine conserved quantity. In an adiabatic run this should track the injected energy exactly, so the 'Energy (synced)' line is the real conservation test. Do NOT use snapshots for this: io.cc writes Velocities from P[].Vel (kick-time) but InternalEnergy from InternalEnergyPred (drift-time), a mixed-clock bias worth 10-20% of L*t. Costs one MPI_Allreduce per synchronization. The per-phase breakdown that once accompanied this (kick1/drift/grav/hydro/mergesplit/kick2/spawn/srcterms, one Allreduce per mark) was hoisted out to scripts/energy_budget_phases.patch -- apply that to localize an error to a specific stage of the step.
#  MERGE_SPLIT_ENERGY_DIAGNOSTIC was hoisted out to scripts/merge_energy_diagnostic.patch -- apply that to get it back. It reported the cumulative energy budget of merge operations: dE (change in the true InternalEnergy+KE), dE_pred (same for the predicted variables the hydro actually integrates), KE_com (what a momentum-conserving merge must dissipate), resid (the correction merge_particles_ij computes), how often the InternalEnergyPred floor fires, and the TimeBin/Ti_current relationship of merged pairs. Setting the flag without applying the patch does nothing. The patch header records what it established, including two dead ends worth not repeating: the Pred floor never fires (0 of 5069 merges) and merged pairs always share Ti_current.
#WAKEUP_TRUNCATE_STEP_ON_DEMOTION   # AUTO-ENABLED whenever the spawn module (SINK_WIND_SPAWN, itself implied by SINGLE_STAR_FB_JETS/WINDS/SNE) is on; set it explicitly only to force it without spawning, and see WAKEUP_REVERSE_KICK_ON_DEMOTION to opt out. When the WAKEUP timestep limiter demotes a particle mid-step, truncate its current step to end at the system time instead of reversing the kick it already received. GIZMO's default calls do_the_kick with reversed time bounds to un-do part of the applied kick, but that re-derives the increment from state which has changed since, so it does not cancel and instead injects energy: measured +22% of the injected energy in the adiabatic wind_singlestar test and +4.7% in SN_singlestar, in both cases from ~100% hydro-triggered wakeups. Saitoh & Makino (2009), whose limiter this is, explicitly avoid "the backward integration of the entire system" (their eq 3) and instead set the new time consistent with the system time with the step as the difference -- which is what this does. Eliminates the error (reversal dE -> 0 to roundoff) and is inert in sedov/noh/interactblast/evrard, where the reversal never fires. CAVEAT: dt_step is then not a power-of-two bin length, so it disagrees with TimeBin; code deriving a step from TimeBin rather than integertime_step() will over-report it.
...
#MERGE_SPLIT_CONSERVE_ENERGY        # make particle merging energy-conserving. A merge sets the new velocity momentum-conservingly, so the pair's COM-frame kinetic energy (1/2)*mu*|dv|^2 leaves the kinetic budget; this returns it as heat. The residual has always been computed in merge_particles_ij but the line applying it has been commented out since the earliest revision (marked '//???'), so historically every merge silently destroyed that energy -- ~23% of the injected energy by t=0.1 in the adiabatic wind_singlestar test, where E_kin+E_th=L*t is exact (0.771 of L*t with the discard vs 0.999 with merging disabled; KE_com/L*t = 21.9% independently). AUTO-ENABLED with SINK_WIND_SPAWN (see precompiler_logic.h), alongside WAKEUP_TRUNCATE_STEP_ON_DEMOTION; MERGE_SPLIT_DISCARD_ENERGY opts back out. An earlier ~9% figure predates the truncation: the wakeup kick reversal was adding ~+18% and cancelling most of the discard, which is why the old default read a healthy-looking ~0.95 while carrying a 23% sink and an 18% source. With both fixed the adiabatic wind bubble conserves to ~0.97 of L*t; the residual ~3% is merge-associated but is not a bookkeeping error (dE, dE_pred and mass are all exact at the merge, and it is insensitive to CourantFac).
