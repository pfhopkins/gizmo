...
#SINK_DUST_HEATING_PLANCKMEAN   # proto-stellar dust heating: sum kappa_Planck(T_eff)*L/(4 pi r^2) over sources directly in the gravity tree, and solve for the dust temperature from that rate. Supersedes the default treatment under SINGLE_STAR_FB_RT_HEATING, which deposits the whole bolometric luminosity into the far-IR band (i.e. assumes an already-reprocessed cold SED) and so underestimates dust heating by ~an order of magnitude for a stellar photosphere. Assumes spatially uniform dust properties.
#SINK_DUST_HEATING_VERBOSE     # per-sink diagnostic for SINK_DUST_HEATING_PLANCKMEAN: prints rho0, r0, tau(r0), |grad rho|, the measured envelope slope n_env, R_in/R_max/R_raw, T_phot, T_star and which clamp (if any) determined the answer. Diagnostic only; noisy, one line per sink per tree build.
