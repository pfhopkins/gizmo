...
#GIZMO_GPU_GRAVTREE             # [Step 13 Phase 4 Tier 1a] enables speculative GPU gravity tree walk pre-pass. Each active particle attempts a GPU walk first; particles that hit a remote pseudo-particle are left for the existing CPU primary loop + MPI export machinery. Tier 1a supports core walk only — no PMGRID, ADAPTIVE_GRAVSOFT_*, EVALPOTENTIAL, RT/SINK/SINGLE_STAR/CR/TIDAL/JERK payloads, periodic Ewald, HERMITE, ATFU. Compile fails with #error if any of those are set. Subsequent tier commits in Phase 4 add support incrementally. Requires OPENMP_GPU_OFFLOAD.
## ----------------------------------------------------------------------------------------------------
