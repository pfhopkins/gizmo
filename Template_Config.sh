...
#GIZMO_GPU_GRAVTREE             # [Step 13] enable GPU gravity tree walk for active particles, with LET-based foreign-node walk replacing the legacy CPU primary loop + MPI export. Supported across PMGRID, ADAPTIVE_GRAVSOFT_*, EVALPOTENTIAL, RT/SINK/SINGLE_STAR/CR/TIDAL/JERK payloads, periodic Ewald, HERMITE/ATFU, FIRE_BHS MencInRcrit, etc. (see step13 phase 9-10 closeout for full coverage list). Requires OPENMP_GPU_OFFLOAD.
## ----------------------------------------------------------------------------------------------------
