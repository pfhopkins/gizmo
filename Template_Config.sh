...
####################################################################################################-
# ------------------ Flag based refinement method for zooming in on tagged particles ----------------
FLAG_BASED_REFINEMENT                     # enable a flag-based refinement method, which allows us to refine based on the distance from the COM of tagged particles (e.g. for following the collapse of a particular region). 
FLAG_BASED_REFINEMENT_MAXDENSITY          # replace COM calculation by calculating the maximum density on the fly and locking on to it. Only moves away from the max density if certain criteria are met (to avoid jumps and choosing a tiny collapsing clump). This is done so we can follow the fastest collapsing region automatically
JEANS_REFINEMENT_IN_REGION                # enable a Jeans refinement criterion within the flagged region, which can be used to ensure that the Jeans mass is always resolved within the region around the tagged particles. Only applies if FLAG_BASED_REFINEMENT is enabled.
JEANS_REFINEMENT_IN_REGION_RADIUS_PC=400  # Radius (in this case 400 pc) around the flagged particle within which to apply the Jeans refinement criterion, in pc (physical units, not comoving). Only applies if JEANS_REFINEMENT_IN_REGION is enabled.
####################################################################################################-
