 ...
 *
 * Tables are declared as function-local constexpr inside gpu_peano_and_morton_key
 * rather than file-scope static constexpr.  File-scope static constexpr arrays
 * are host-side in nvcc and generate "undefined in device code" errors even with
 * --expt-relaxed-constexpr.  Function-local constexpr arrays in KOKKOS_INLINE_FUNCTION
 * bodies are placed in read-only constant cache by the CUDA compiler with
 * --expt-relaxed-constexpr, matching the performance of __device__ __constant__ tables.
 * The 768 total bytes (384 per table) fit comfortably in any GPU's constant cache. */
...
{
    /* Function-local constexpr: nvcc places these in GPU read-only constant
     * cache with --expt-relaxed-constexpr.  File-scope static constexpr is
     * host-only in nvcc even with that flag. */
    constexpr unsigned char rottable[48][8] = {
    ...
    constexpr unsigned char subpix[48][8] = {
      {0, 7, 1, 6, 3, 4, 2, 5},
      {7, 4, 6, 5, 0, 3, 1, 2},
      {4, 3, 5, 2, 7, 0, 6, 1},
      {3, 0, 2, 1, 4, 7, 5, 6},
    ...
    for(int b = bits - 1; b >= 0; b--) {
        ...
        unsigned char pix = (unsigned char)(((x & mb) ? 4 : 0)
                                          ...
                                          | ((z & mb) ? 1 : 0));
        key = (peanokey)(key << 3);
        key = (peanokey)(key | (peanokey)subpix[rotation][pix]);
        rotation = rottable[rotation][pix];
    }
    return key;
}
