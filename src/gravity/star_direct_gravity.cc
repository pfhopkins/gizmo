 ...
 *
 *  The sum below is a serial host loop over a table gathered to every task. A direct N-body sum is
 *  one of the problems GPUs suit best, and doing it on the device would raise the practical star
 *  count here by orders of magnitude; the host version is the reference implementation, not the
 *  intended end point.
 */
