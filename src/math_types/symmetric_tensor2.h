...
struct SymmetricTensor2 {
    ...
    GIZMO_GPU_FUNCTION static constexpr int flat_index(int i, int j) noexcept {
    ...
    struct Row {
        SymmetricTensor2 &t;
        int i;
        GIZMO_GPU_FUNCTION T &operator[](int j) noexcept { return t.data[flat_index(i, j)]; }
        GIZMO_GPU_FUNCTION const T &operator[](int j) const noexcept { return t.data[flat_index(i, j)]; }
    };
    ...
    GIZMO_GPU_FUNCTION Row operator[](int i) noexcept { return {*this, i}; }
    GIZMO_GPU_FUNCTION ConstRow operator[](int i) const noexcept { return {*this, i}; }

    // Trace: sum of diagonal elements.
    GIZMO_GPU_FUNCTION T trace() const noexcept { return data[0] + data[1] + data[2]; }

    // Scalar multiply in-place.
    GIZMO_GPU_FUNCTION SymmetricTensor2& operator*=(T s) noexcept {
        for(int k = 0; k < 6; k++) { data[k] *= s; }
        return *this;
    }

    // Element-wise addition in-place.
    GIZMO_GPU_FUNCTION SymmetricTensor2& operator+=(const SymmetricTensor2& other) noexcept {
    ...
    template<typename U>
    GIZMO_GPU_FUNCTION T double_contract(const Mat3<U>& M) const noexcept {
    ...
    GIZMO_GPU_FUNCTION void set_isotropic(T val) noexcept {
...
template<typename T>
GIZMO_GPU_FUNCTION inline SymmetricTensor2<T> operator*(T s, SymmetricTensor2<T> t) noexcept { t *= s; return t; }

template<typename T>
GIZMO_GPU_FUNCTION inline SymmetricTensor2<T> operator*(SymmetricTensor2<T> t, T s) noexcept { t *= s; return t; }

template<typename T>
GIZMO_GPU_FUNCTION inline SymmetricTensor2<T> operator+(SymmetricTensor2<T> a, const SymmetricTensor2<T>& b) noexcept { a += b; return a; }
...
// Symmetric outer product v (x) v
// Storage: [0]=xx [1]=yy [2]=zz [3]=xy [4]=yz [5]=xz
template<typename T>
GIZMO_GPU_FUNCTION inline SymmetricTensor2<T> outer_product(const Vec3<T>& v) noexcept {
