...
void gpu_step_sidx_invalidate_full(void)
{
    ...
    /* The owned tile indexes are keyed on the slot layout too. */
...
/* ============================================================================
 * The owned-particle tile index (declared in gpu_neighbor_list.h).
 * ========================================================================== */

/* GX_OWNED_TILE_INDEX_MAX_RESIDENT (gpu_neighbor_list.h) indexes are resident
 * at once -- one per supply mask a fused loop has asked for recently.  Three
 * covers the masks the loops use (gas; dark matter; every type) with one to
 * spare; a fourth mask evicts the one used longest ago, between calls, and
 * pays a rebuild.  Not a knob: the cost of being wrong is one extra build per
 * step, which the accounting shows. */

struct GxOwnedTileIndex {
    sfc_tile_t      *tiles        = nullptr;   /* shared space, [ntiles] */
    tile_bvh_node_t *bvh          = nullptr;   /* shared space, [nnodes] */
    int             *pool         = nullptr;   /* shared space, [num_pool] */
    int             *tile_of      = nullptr;   /* shared space, [num_local_at_build] */
    int             *leaf_of_tile = nullptr;   /* shared space, [ntiles] */
    int             *parent       = nullptr;   /* shared space, [nnodes] */
    int ntiles = 0, nnodes = 0, bvh_root = -1, num_pool = 0;
...
static struct GxOwnedTileIndex g_owned_tile_index[GX_OWNED_TILE_INDEX_MAX_RESIDENT];
static unsigned long long      g_owned_tile_index_clock = 0;

static void gx_owned_tile_index_free(struct GxOwnedTileIndex *ix)
{
    /* Ordering, not cleanup: kokkos_free does not synchronize, and a walk may
       still be reading these (gpu_spatial_index_free states the same rule). */
    if(ix->tiles || ix->bvh || ix->pool || ix->tile_of || ix->leaf_of_tile || ix->parent) {Kokkos::fence();}
    if(ix->tiles)        {Kokkos::kokkos_free<GIZMO_KOKKOS_SHARED_SPACE>(ix->tiles);}
    if(ix->bvh)          {Kokkos::kokkos_free<GIZMO_KOKKOS_SHARED_SPACE>(ix->bvh);}
    if(ix->pool)         {Kokkos::kokkos_free<GIZMO_KOKKOS_SHARED_SPACE>(ix->pool);}
    if(ix->tile_of)      {Kokkos::kokkos_free<GIZMO_KOKKOS_SHARED_SPACE>(ix->tile_of);}
    if(ix->leaf_of_tile) {Kokkos::kokkos_free<GIZMO_KOKKOS_SHARED_SPACE>(ix->leaf_of_tile);}
    if(ix->parent)       {Kokkos::kokkos_free<GIZMO_KOKKOS_SHARED_SPACE>(ix->parent);}
    *ix = GxOwnedTileIndex{};
}
                                       ...
                                       const struct particle_data *P)
{
    ...
    if(num_local > ix->num_local_at_build) {
        ...
        if(build_sfc_supply_pool((struct particle_data *)(P + ix->num_local_at_build), appended, (int)mask, NULL) > 0) {return 0;}
    }
    return 1;
}

/* Build the index for `mask` over the owned particles.  Tiles and the BVH come
 * from the shared builders; what is added is the two maps the kick pass raises
                                     ...
                                     struct particle_data *P, const char *caller)
{
    ...
    /* Every array is at least one element long, so a request for none of
       something is not mistaken for a refusal. */
    const size_t b_tiles = (size_t)((ntiles > 0) ? ntiles : 1) * sizeof(sfc_tile_t);
    const size_t b_bvh   = (size_t)((nnodes > 0) ? nnodes : 1) * sizeof(tile_bvh_node_t);
    const size_t b_pool  = (size_t)((num_pool > 0) ? num_pool : 1) * sizeof(int);
    const size_t b_of    = (size_t)((num_local > 0) ? num_local : 1) * sizeof(int);
    const size_t b_leaf  = (size_t)((ntiles > 0) ? ntiles : 1) * sizeof(int);
    const size_t b_par   = (size_t)((nnodes > 0) ? nnodes : 1) * sizeof(int);
    ix->tiles        = (sfc_tile_t *)      ngl_alloc_shared(b_tiles, "ngl_sidx_owned_tiles");
    ix->bvh          = (tile_bvh_node_t *) ngl_alloc_shared(b_bvh,   "ngl_sidx_owned_bvh");
    ix->pool         = (int *)             ngl_alloc_shared(b_pool,  "ngl_sidx_owned_pool");
    ix->tile_of      = (int *)             ngl_alloc_shared(b_of,    "ngl_sidx_owned_tile_of");
    ix->leaf_of_tile = (int *)             ngl_alloc_shared(b_leaf,  "ngl_sidx_owned_leaf_of_tile");
    ix->parent       = (int *)             ngl_alloc_shared(b_par,   "ngl_sidx_owned_parent");
    const int ok = ix->tiles && ix->bvh && ix->pool && ix->tile_of && ix->leaf_of_tile && ix->parent;
    if(ok) {
        if(ntiles > 0)   {memcpy(ix->tiles, h_tiles, (size_t)ntiles * sizeof(sfc_tile_t));}
        if(nnodes > 0)   {memcpy(ix->bvh,   h_bvh,   (size_t)nnodes * sizeof(tile_bvh_node_t));}
        if(num_pool > 0) {memcpy(ix->pool,  h_pool,  (size_t)num_pool * sizeof(int));}
        for(int i = 0; i < num_local; i++) {ix->tile_of[i] = -1;}
        for(int t = 0; t < ntiles; t++) {
            for(int s = 0; s < h_tiles[t].count; s++) {ix->tile_of[h_pool[h_tiles[t].first + s]] = t;}
        }
        for(int no = 0; no < nnodes; no++) {ix->parent[no] = -1;}
        for(int no = 0; no < nnodes; no++) {
            if(h_bvh[no].left < 0) {ix->leaf_of_tile[-(h_bvh[no].left + 1)] = no;}
            else {ix->parent[h_bvh[no].left] = no; ix->parent[h_bvh[no].right] = no;}
        }
    }
    /* Reverse of the order the builders allocated in. */
    ...
    if(!ok) {
        gx_owned_tile_index_free(ix);
        char msg[256];
        snprintf(msg, sizeof(msg), "%s: could not allocate the owned tile index for mask 0x%x (%d tiles, %d particles)",
                 caller ? caller : "?", mask, ntiles, num_local);
...
{
    GIZMO_GPU_ENSURE_ALL_FRESH();
    struct particle_data *P = gpu_particles_arena_P();
    const int num_local = ghost_get_num_local();
    if(!P) {return 1;}

    struct GxOwnedTileIndex *ix = NULL;
    for(int k = 0; k < GX_OWNED_TILE_INDEX_MAX_RESIDENT && !ix; k++) {
        if(gx_owned_tile_index_current(&g_owned_tile_index[k], mask, num_local, P)) {ix = &g_owned_tile_index[k];}
    }
    if(!ix) {
        /* A slot for a fresh build: an invalid one, else the one used longest
         * ago among those no call is holding. */
        for(int k = 0; k < GX_OWNED_TILE_INDEX_MAX_RESIDENT && !ix; k++) {
            if(!g_owned_tile_index[k].valid) {ix = &g_owned_tile_index[k];}
        }
        if(!ix) {
            for(int k = 0; k < GX_OWNED_TILE_INDEX_MAX_RESIDENT; k++) {
                struct GxOwnedTileIndex *c = &g_owned_tile_index[k];
                if(c->held) {continue;}
                if(!ix || c->last_used < ix->last_used) {ix = c;}
            }
            if(!ix) {return 1;}   /* every slot is held by a call in progress */
        }
        if(gx_owned_tile_index_build(ix, mask, num_local, P, caller) != 0) {return 1;}
    }
    ix->held = 1;
    ix->last_used = ++g_owned_tile_index_clock;

    /* The boxes are motion bounds and are only correct widened, so the drift
     * tables are not optional here as they are for the tree walk (which falls
     * back to a sweep-certified length): without them this rank declines. */
    gx_walk_drift_tables_refresh();
    if(!g_walk_drift_tables_ok) {
        static int reported = 0;
        if(!reported) {
            reported = 1;
            printf("%s: task %d has no drift-table mirror for the owned tile index; this call falls back for every rank\n",
                   caller ? caller : "?", ThisTask);
            fflush(stdout);
        }
        ix->held = 0;
        return 1;
    ...
    *out = GxOwnedTileView{};
    out->tiles = ix->tiles; out->bvh = ix->bvh; out->pool = ix->pool;
    out->bvh_root = ix->bvh_root;
    out->local_particle_slots = num_local;
    out->drift_tables = g_walk_drift_tables;
    out->drift_tables_ok = 1;
    out->ti_now = All.Ti_Current;
    return 0;
}
...
{
    for(int k = 0; k < GX_OWNED_TILE_INDEX_MAX_RESIDENT; k++) {g_owned_tile_index[k].held = 0;}
}

int gx_owned_tile_index_raise_targets(struct GxOwnedTileRaise *out, int max_out)
{
    int n = 0;
    for(int k = 0; k < GX_OWNED_TILE_INDEX_MAX_RESIDENT && n < max_out; k++) {
        const struct GxOwnedTileIndex *ix = &g_owned_tile_index[k];
        if(!ix->valid || ix->ntiles <= 0) {continue;}
        out[n].tiles = ix->tiles; out[n].bvh = ix->bvh; out[n].tile_of = ix->tile_of;
        out[n].leaf_of_tile = ix->leaf_of_tile; out[n].parent = ix->parent;
        out[n].num_local_at_build = ix->num_local_at_build;
        n++;
    }
    return n;
}
