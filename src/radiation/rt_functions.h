...
double rt_kappa_adaptive_IR_band(int i, double T_dust, double Trad, int do_emission_absorption_scattering_opacity, int dust_or_gas_opacity_only_flag, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    if(dust_or_gas_opacity_only_flag <= 0) // non-dust (e.g. gas-phase) IR opacities
    {
        ...
        double Tgas=1. + cell[i].gas_temperature_at_u(cell[i].InternalEnergyPred), rho_cgs = cell[i].Density*All.cf_a3inv*UNIT_DENSITY_IN_CGS; /* crude estimate of gas temperature to use with scalings below, and gas density in cgs */
...
double rt_kappa(int i, int k_freq, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    {
        double T_eff=cell[i].gas_temperature_at_u(cell[i].InternalEnergyPred), rho=cell[i].Density*All.cf_a3inv*UNIT_DENSITY_IN_CGS; // temperature from the composition the cooling solve stored, or the ionized fallback where none was, nothing fancy, to get the temperature //
...
double rt_absorb_frac_albedo(int i, int k_freq, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    if(k_freq==RT_FREQ_BIN_FREEFREE)
    {
        double T_eff=cell[i].gas_temperature_at_u(cell[i].InternalEnergyPred), rho=cell[i].Density*All.cf_a3inv*UNIT_DENSITY_IN_CGS, kappa_abs = 1.e30*rho*pow(T_eff,-3.5);
...
KOKKOS_INLINE_FUNCTION void rt_get_lum_gas(int target, double *je, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    double t_eff = cell[target].gas_temperature_at_u(cell[target].InternalEnergyPred); // temperature from the composition the cooling solve stored, or the ionized fallback where none was, nothing fancy, to get the temperature //
