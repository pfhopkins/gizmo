...
extern int MaxForeignNodes;  /*!< LET: the CEILING of the foreign-node INDEX range, published by force_treeallocate -- derived there for a fresh tree, or restored verbatim from the restart file for a serialized one, whose node pointers encode the ceiling the writer used.  0 on non-GPU builds.  Index map: foreign nodes occupy [TreeNodeIndexBase+MaxNodes, TreeNodeIndexBase+MaxNodes+MaxForeignNodes); pseudo-particles shifted to [TreeNodeIndexBase+MaxNodes+MaxForeignNodes, ...).  This is an index ceiling, NOT the amount of foreign-node storage that exists: see AllocatedForeignNodes. */
