...
#endif
#if !(defined(RT_OTVET) || defined(RT_FLUXLIMITEDDIFFUSION) || defined(RT_M1) || defined(RT_LOCALRAYGRID))
/* AGN-source counterparts of the FIRE sub-grid stellar RT modules. They share
   the LEBRON/gravity-tree numerical infrastructure (RT_USE_GRAVTREE,
   valid_gas_particle_for_rt accumulators, etc.) and are therefore mutually
   exclusive with explicit RT solvers (RT_M1, RT_OTVET, RT_FLUXLIMITEDDIFFUSION,
   RT_LOCALRAYGRID), exactly as their stellar GALSF_FB_FIRE_RT_* counterparts. */
...
/* POST_COOLING_DEVICE_EOS_SUPPORTED — narrow compile-time gate for the
   ...
   Both are table lookups and are the last two names here; they leave when their
   tables gain device mirrors.

   EOS_TILLOTSON and HYDRO_GENERATE_TARGET_MESH USED to be excluded and no longer
   are: calculate_tillotson_eos is GPU-marked in declarations/cell_data.h, and the
   two return_user_desired_target_* user-edit bodies are KOKKOS_INLINE_FUNCTION in
   eos/eos_functions.h, so both branches run on the device pass.

   NOTE for a COMBINED EOS_TILLOTSON + EOS_ANEOS build: Tillotson being device-ready
   does NOT make that build device-routed. EOS_ANEOS is still excluded above, so the
   gate stays undefined and the whole post-cooling EOS stays on the host, Tillotson
   cells included. Such a build becomes device-routed only when ANEOS leaves too.
...
#if !defined(EOS_HELMHOLTZ) && !defined(EOS_ANEOS)
...
/* Sub-block offsets within Metallicity[] / yields[] / Z_injected[] for the dustchem layout.
   Dustchem block begins at ISMDUSTCHEM_SPECIES_OFFSET_IN_METALLICITY and contains, in order:
     Dust_Metal      (NUM_ISMDUSTCHEM_ELEMENTS)
     Dust_Source     (NUM_ISMDUSTCHEM_SOURCES)
     Dust_Species    (NUM_ISMDUSTCHEM_SPECIES)
     Dust_NumberInBin (NUM_ISMDUSTCHEM_SPECIES * NUM_ISMDUSTCHEM_SIZE_BINS)  [GRAINSIZEEVO only]
     Dust_SlopeInBin  (NUM_ISMDUSTCHEM_SPECIES * NUM_ISMDUSTCHEM_SIZE_BINS)  [GRAINSIZEEVO only]
   These are ABSOLUTE indices into the Metallicity-sized arrays (NOT offsets to add to NUM_METAL_SPECIES;
   NUM_METAL_SPECIES already includes the dustchem block). Always use these macros for dustchem indexing. */
#define ISMDUSTCHEM_DUST_METAL_OFFSET_IN_METALLICITY       (ISMDUSTCHEM_SPECIES_OFFSET_IN_METALLICITY)
#define ISMDUSTCHEM_DUST_SOURCE_OFFSET_IN_METALLICITY      (ISMDUSTCHEM_DUST_METAL_OFFSET_IN_METALLICITY      + NUM_ISMDUSTCHEM_ELEMENTS)
#define ISMDUSTCHEM_DUST_SPECIES_OFFSET_IN_METALLICITY     (ISMDUSTCHEM_DUST_SOURCE_OFFSET_IN_METALLICITY     + NUM_ISMDUSTCHEM_SOURCES)
#define ISMDUSTCHEM_DUST_NUMBERINBIN_OFFSET_IN_METALLICITY (ISMDUSTCHEM_DUST_SPECIES_OFFSET_IN_METALLICITY    + NUM_ISMDUSTCHEM_SPECIES)
#define ISMDUSTCHEM_DUST_SLOPEINBIN_OFFSET_IN_METALLICITY  (ISMDUSTCHEM_DUST_NUMBERINBIN_OFFSET_IN_METALLICITY + NUM_ISMDUSTCHEM_SPECIES*NUM_ISMDUSTCHEM_SIZE_BINS)
