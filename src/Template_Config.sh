...
#TREE_LEAF_BUCKET_SIZE=8           # stop subdividing a tree node once it holds this many particles or fewer, so a leaf may hold several particles which the walk then evaluates directly, instead of splitting down to one particle per leaf. A larger value gives a much smaller and shallower tree, cheaper to build and to store, and bounds the depth that near-coincident particles can force, at the cost of more direct particle-particle tests per walk. There is no universally good value: useful settings range from 1 to several thousand depending on the problem, on how aggressively the opening criterion descends, and on the machine, so it is worth tuning if used. Default 1, which reproduces the historical one-particle-per-leaf tree exactly.
