...
#ENERGY_BUDGET_DIAGNOSTIC           # energy diagnostic, no behaviour change. Reports the total gas energy (thermal+kinetic) at every full synchronization, i.e. when All.HighestActiveTimeBin == All.HighestOccupiedTimeBin -- the only point at which Vel and InternalEnergy share a clock and the sum is a genuine conserved quantity, so the 'Energy (synced)' line is the real conservation test and in an adiabatic run should track the injected energy exactly. Do NOT substitute snapshots for it: io.cc writes Velocities from P[].Vel (kick-time) but InternalEnergy from InternalEnergyPred (drift-time), so a snapshot sum is mixed-clock and not conserved. Costs one MPI_Allreduce per synchronization.
#GIZMO_STALE_FORCE_DUMP=8           # diagnostic, off by default; the value is a dump count. Writes per-rank text files (staleforce_NNN.rank: id, active flag, mass, position, GravAccel) on the first N steps where NO tree rebuild happened -- the only steps where a stale tree is observable, since snapshots land on full-sync points where a decomposition has just run and therefore always score a fresh tree. Score the dumps offline against a direct sum on these same positions to isolate tree error from drift. See run.cc.
#GIZMO_GPU_KEY_AUDIT                # diagnostic, off by default. After the device topology build, recompute every particle's Peano key and topleaf on the HOST through domain.cc's own helpers (a different translation unit, so it also catches the compiler treating two copies of the position->key expression differently) and report any particle whose build-side topleaf disagrees or that landed under a topleaf this rank does not own. Closes the blind spot in the root-N_part integrity check, which only sees particles missing from EVERY rank's tree -- one mis-keyed into the WRONG topleaf is still counted but sits under a node whose bounds do not contain it, giving intermittent force errors with no warning. O(NumPart) host work per tree build; for debugging, not production. See gpu_topology_build.cc.
#GIZMO_LET_MOTION_MARGIN=1.0        # accuracy/tuning option (NOT a diagnostic), off by default; the value multiplies the type-1 softening support radius. Inflates the LET cover boxes so the essentiality test provisions for the LET's lifetime rather than the instant it is packed: boxes are built from receiver positions at PACK time, and a target drifting closer before the next rebuild can want resolution that was never shipped. Off/0 reproduces the historical decision exactly; measured +8.1% essential LET nodes at 1.0. Largely superseded by the per-step refresh of imported foreign moments (let_pack.cc), which is why nothing enables it -- kept as a knob for configurations where refresh cadence and pack provisioning need to be separated. See let_pack.cc.
...
#MERGE_SPLIT_ALLOW_KINETIC_DISSIPATION   # opt back out of the MERGE_SPLIT_LIMIT_KINETIC_DISSIPATION auto-enable. For reproducing older results only.
#SINK_SPAWN_MERGE_WHEN_AMBIENT   # retire spawned jet/wind cells only once they have joined the ambient medium, rather than as soon as their relative velocity dips below one neighbor's sound speed. Requires (a) the target to be a normal cell, never another spawned cell, and (b) the cell to be subsonic AND thermally equilibrated wrt non-spawned gas in its kernel. AUTO-ENABLED by SINGLE_STAR_FB_JETS/WINDS.
#SINK_SPAWN_MERGE_MAX_THERMAL_CONTRAST=(1.0)  # under SINK_SPAWN_MERGE_WHEN_AMBIENT, max |ln(u_cell/u_ambient)| for a spawned cell to count as having joined the ISM. Subsonic alone is not sufficient: a contact discontinuity is in pressure equilibrium with no velocity jump, so outflow material at the contact passes every kinematic test while still being a distinct phase. Deliberately u and not an entropic function, which is only a state function for a single-gamma ideal gas.
#SINK_SPAWN_MERGE_ANY_NEIGHBOR   # opt back out of the SINK_SPAWN_MERGE_WHEN_AMBIENT auto-enable, restoring the legacy retire-when-subsonic-wrt-target behaviour. For reproducing older jet/wind runs.
