 ...
 * The body migrated to hydro/hydro_force_loop.cc::HydroForceSpec::
 * apply_active_writeback, called by the runner per active after the
 * neighbor-loop reduces all per-pair contributions into AccumData. */
/* --------------------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------------------- */
/* hydro_force() retired from this file in hydro corridor commit 8 (closes the
 * corridor — HydroForceSpec). The new toplevel + the host out2particle_hydra
 * scatter both live in hydro/hydro_force_loop.cc. Dispatch goes through
