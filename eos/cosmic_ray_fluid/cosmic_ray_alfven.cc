...
double CosmicRay_Update_DriftKick(int i, double dt_entr, int mode, struct particle_data *pp, struct gas_cell_data *cell)
{
...
#ifdef COOLING 
    temperature = ThermalProperties(u0, rho, i, &mu_meanwt, &ne, &nh0, &nhp, &nHe0, &nHeII, &nHepp, cell); // get thermodynamic properties
    ...
    vA_code = Get_Gas_ion_Alfven_speed_i(i, cell); // include ionization appropriately for small-scale modes
