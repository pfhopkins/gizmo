...
{
    double vol_inv = CellP[target].Density*All.cf_a3inv / P[target].Mass, fturb_multiplier=1, f_QLT=1, R_CR_GV, Z_charge_CR, M_cr_mp, b0[3]={0}, p0[3]={0};
...
#ifdef MAGNETIC
    for(k=0;k<3;k++) {b0[k]=CellP[target].BPred[k]*vol_inv*All.cf_a2inv; B2+=b0[k]*b0[k];}
    e_B=0.5*B2; B2=1./sqrt(B2+MIN_REAL_NUMBER); for(k=0;k<3;k++) {b0[k]*=B2;} // calculate B-field energy and bhat vector
#else
    for(k=0;k<3;k++) {b0[k]=CellP[target].Gradients.CosmicRayPressure[k_CRegy][k]; B2+=b0[k]*b0[k];} // just assume equilibrium here, convert to physical units, pick arbitrary direction here //
    e_B=CellP[target].Pressure*All.cf_a3inv; for(k=0;k<3;k++) {b0[k]/=sqrt(B2+MIN_REAL_NUMBER);} // calculate B-field energy and bhat vector
#endif
...
double CosmicRay_Update_DriftKick(int i, double dt_entr, int mode)
{
    ...
    for(k_CRegy=0;k_CRegy<N_CR_PARTICLE_BINS;k_CRegy++)
    {
        ...
        for(k=0;k<3;k++)
        {
            if(mode==0) {B0[k]=CellP[i].B[k]/V_i;} else {B0[k]=CellP[i].BPred[k]/V_i;}
        ...
        if(mode==0) {for(k=0;k<3;k++) {flux[k]=CellP[i].CosmicRayFlux[k_CRegy][k];}} else {for(k=0;k<3;k++) {flux[k]=CellP[i].CosmicRayFluxPred[k_CRegy][k];}}
#ifdef MAGNETIC // do projection onto field lines
