...
MyFloat f_Cplus(int i, MyFloat temp, MyFloat x_elec, MyFloat shieldfac, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    MyFloat k_gr = alpha_recomb_grain(i, temp, x_elec, shieldfac, "C+", pp, cell);
...
MyFloat return_electron_fraction_from_Cplus(int i, MyFloat temp, MyFloat x_elec, MyFloat shieldfac, struct particle_data *pp, struct gas_cell_data *cell){
    MyFloat x_Cplus = pp[i].Metallicity[2]/All.SolarAbundances[2] * 1.6e-4 * f_Cplus(i, temp, x_elec, shieldfac, pp, cell); // Assumes gas-phase C abundance 1.6e-4 (Sofia 2004)
    return x_Cplus;
}
...
MyFloat return_electron_fraction_from_Oplus(int i, MyFloat nHp, struct particle_data *pp, struct gas_cell_data *cell){
    MyFloat x_Oplus = pp[i].Metallicity[4]/All.SolarAbundances[4] * 3.2e-4 * f_Oplus(nHp); // Assumes gas-phase O abundance 3.2e-4 (Savage & Sembach 1996)
    return x_Oplus;
}
...
MyFloat return_electron_fraction_from_alkali(int i, MyFloat temp, struct particle_data *pp, struct gas_cell_data *cell){
    ...
    MyFloat xe = 6.47e-13 * sqrt(x_K/1e-7) * sqrt(sqrt(temp*temp*temp/1e9))  * sqrt(2.4e15 / cell[i].nHcgs()) * exp(-25188/temp)/1.15e-11; // low-ionization approximation
