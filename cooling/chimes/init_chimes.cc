...
{  
  ...
  // Open main data file 
  snprintf(fname, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s", myGlobalVars->MainDataTablePath); 
...
{   
  ...
  if (myGlobalVars->redshift_dependent_UVB_index >= 0) 
    {
      snprintf(fname, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s/redshifts.hdf5", myGlobalVars->PhotoIonTablePath[myGlobalVars->redshift_dependent_UVB_index]);
      file_id = H5Fopen(fname, H5F_ACC_RDONLY, H5P_DEFAULT);
  ...
  if (myGlobalVars->redshift_dependent_UVB_index == 0) 
    snprintf(fname, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s/z%.3f_cross_sections.hdf5", myGlobalVars->PhotoIonTablePath[0], chimes_table_redshift_dependent_UVB.redshift_bins[0]); 
  else
    snprintf(fname, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s", myGlobalVars->PhotoIonTablePath[0]); 
  ...
  for (i = 0; i < myGlobalVars->N_spectra; i++) 
	...
	continue; 
      ...
      snprintf(fname, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s", myGlobalVars->PhotoIonTablePath[i]); 
      file_id = H5Fopen(fname, H5F_ACC_RDONLY, H5P_DEFAULT);
...
void init_chimes(struct globalVariables *myGlobalVars)
{
  ...
  if (myGlobalVars->use_redshift_dependent_eqm_tables == 1) 
      ...
      if (myGlobalVars->redshift_dependent_UVB_index < 0) 
	{
      ...
      snprintf(fname, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s/z%.3f_eqm.hdf5", myGlobalVars->EqAbundanceTablePath, chimes_table_redshift_dependent_UVB.redshift_bins[0]); 
    ...
    {
      snprintf(fname, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s", myGlobalVars->EqAbundanceTablePath); 
      allocate_eqm_table_memory(fname, &chimes_table_eqm_abundances, myGlobalVars); 
...
void load_redshift_dependent_UVB(ChimesFloat redshift, int bin_index, struct globalVariables *myGlobalVars) 
{
  ...
  snprintf(fname, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s/z%.3f_cross_sections.hdf5", myGlobalVars->PhotoIonTablePath[spectrum_index], redshift);
  ...
  if (myGlobalVars->use_redshift_dependent_eqm_tables == 1) 
    {
      snprintf(fname, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s/z%.3f_eqm.hdf5", myGlobalVars->EqAbundanceTablePath, redshift); 
