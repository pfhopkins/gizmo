...
#TREE_QUERY_PACKET_SIZE=8       # walk the gravity tree for this many adjacent active elements at once on the host: they share one traversal while every element keeps its own opening decisions and evaluates its own accepted elements, so forces are unchanged and the traversal cost of a deep tree is divided among them. Problem-dependent, worth tuning if you care about speed; 1 walks every element alone.
