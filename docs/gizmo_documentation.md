...
**GALSF\_ISMDUSTCHEM\_MODEL**: Enables live ISM dust evolution, tracking the formation, destruction, and processing of multiple dust species self-consistently with stellar evolution, feedback, and gas-phase chemistry. The value is set as a sum of bitflags selecting the model variant: 1=elemental (tracks dust abundance per element), 2=species-based (tracks silicates, carbonaceous, SiC, metallic iron, and free-flying iron nanoparticles separately), 4=include free-flying iron nanoparticles, 8=include an oxygen reservoir for silicate formation. Formation channels include stellar sources (SNe Type Ia/II, AGB winds) with composition-dependent yields; destruction channels include sputtering in hot gas and destruction by SNe shocks; growth channels include gas-phase accretion in the ISM. The recommended default for most applications is `GALSF_ISMDUSTCHEM_MODEL=(2+4+8)`. Users should cite Choban et al. 2022 (arXiv:2112.09141) and subsequent papers for the methods and calibration. **GALSF\_ISMDUSTCHEM\_GRAINSIZEEVO** can be additionally enabled to track grain size evolution with logarithmically-spaced size bins (set the value to the number of bins, e.g. 16), enabling coagulation, shattering, accretion, and sputtering as a function of grain size; this requires `GALSF_ISMDUSTCHEM_MODEL` with model flags 16 or 32. Cite Choban et al. for the grain size evolution methods.
