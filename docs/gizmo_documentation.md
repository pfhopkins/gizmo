...
7. [Modules in Config.sh (Setting compile-time options, i.e. Physics)](#config)
    ...
    + [Additional Fluid Physics](#config-fluids)
        ...
        + [Multi-Fluid (Lagrangian-Partition) Framework](#config-fluids-multifluid)
    ...
    + [SuperMassive Black Hole particle Options](#config-bh)
...
**Modularity and cross-compatibility.** Almost every physics module in the code is designed to be independently toggled on or off via a single flag in `Config.sh`, and modules are designed to be cross-compatible wherever physically meaningful. A simple periodic-box hydrodynamics simulation can be transformed into a simulation of star formation in a magnetized giant molecular cloud, live thermonuclear burning in a white dwarf merger, elastic solid-body collisions and giant planetary impacts, dust particulate dynamics with aerodynamic drag and Lorentz forces, cosmic-ray magneto-hydrodynamics with anisotropic transport and streaming, radiation-driven ionization fronts and photo-dissociation regions, or any combination of the above -- with just a few compile-time switches in `Config.sh`, often without modifying a single line of source code. This modularity means that improvements to any one module (e.g. a more accurate cooling network, or a GPU-accelerated neighbor-list hydro solver) automatically benefit every science application that uses it, and entirely new physics can be grafted in without risk of breaking existing functionality. The code is designed with the explicit philosophy that physical and numerical flexibility should be a first-class feature, not an afterthought: the same codebase that runs a pure dark matter cosmological simulation should, with a handful of flags, become a multi-physics star and planet formation code or a nuclear astrophysics simulation engine.
...
When using specific modules, the relevant code papers must also be cited: for example, papers using the MHD version of the code should cite the MHD methods paper: Hopkins & Raives 2016, MNRAS, 455, 51 ("Accurate, Meshless Methods for Magneto-Hydrodynamics," [arXiv:1505.02783](https://arxiv.org/abs/1505.02783)). Anisotropic diffusion operators (conduction, viscosity, radiation, cosmic rays, non-ideal MHD, passive scalar diffusion) should cite Hopkins 2017, MNRAS, 466, 3387 ("Anisotropic Diffusion in Mesh-Free Numerical Magnetohydrodynamics," [arXiv:1602.07702](https://arxiv.org/abs/1602.07702)). The citations required for the various code modules are all given in the "Config.sh (Setting Compile-Time Options)" section of this Guide, where the modules are described. 
...
If you are interested in additional comparisons, see the [methods paper](https://arxiv.org/abs/1409.7395) for a more technical description specific to **GIZMO**, or see [my notes here](http://www.tapir.caltech.edu/~phopkins/public/notes_pfh_numerical_methods.pdf) from a summer-school lecture for a broader overview of different methods (SPH, AMR, MFM/MFV, Voronoi-meshes).
...
First, get a copy of the source code from the [Public Code Site](https://github.com/pfhopkins/gizmo-public) or the [development (private) Repository](https://github.com/pfhopkins/gizmo).
...
+ **Kokkos** (required for GPU acceleration) - the Kokkos performance portability library ([`https://github.com/kokkos/kokkos`](https://github.com/kokkos/kokkos)). Kokkos provides a portable abstraction for CUDA, HIP, and OpenMP backends, allowing the same code to run on NVIDIA, AMD, or CPU-only systems without modification. On HPC systems, Kokkos is often available as a module (e.g. `module load kokkos`); otherwise it can be installed as a CMake subproject or from source. See the Makefile for machine-specific Kokkos linking examples (e.g. the "Vista" or "MacBookCellar_Kokkos" configurations). CPU-only builds (no GPU hardware) can use the Kokkos OpenMP backend; builds without Kokkos entirely are no longer supported.

+ **HYPRE** (optional, for `MHD_MODIFIED_GRADIENT`) - the HYPRE linear solvers library ([`https://github.com/hypre-space/hypre`](https://github.com/hypre-space/hypre)), developed at LLNL. Required only when `MHD_MODIFIED_GRADIENT` is enabled in `Config.sh`, which uses HYPRE's algebraic multigrid (BoomerAMG) preconditioned conjugate gradient solver to enforce machine-precision $\nabla\cdot{\bf B}=0$ via a global sparse linear solve. On HPC systems this is often available as a module (e.g. `module load hypre`); on macOS it can be installed via Homebrew (`brew install hypre`). The Makefile auto-detects a Homebrew installation, a TACC module (environment variables `TACC_HYPRE_DIR` or `TACC_HYPRE_GPU_DIR`), or an OLCF module (`OLCF_HYPRE_ROOT`, e.g. `module load hypre/3.0.0-mpi` on Frontier); see the Makefile for details and examples. If HYPRE is not available, `MHD_MODIFIED_GRADIENT` can instead be built with the `MHD_MODIFIED_GRADIENT_CG_ONLY` flag, which selects the built-in SSOR-preconditioned CG solver (slower but no external dependency). A GPU-enabled HYPRE build (compiled with `--with-cuda`) is needed to offload the sparse solve to GPU; standard CPU builds work correctly with the CPU HYPRE libraries.
...
+ **Makefile**: this actually controls the code compilation. A number of systems have been pre-defined here, you should follow these examples to add other system-types. Slight adjustments of the makefile will be needed if any of the above libraries is not installed in a standard location on your system. Also, compiler optimisation options may need adjustment, depending on the C-compiler that is used. The provided makefile is compatible with GNU- make, i.e. typing make or gmake should then build the executable. For example, for the "Frontera" system at TACC, the Makefile includes the machine-specific set of options: 
        ...
        ifeq ($(SYSTYPE),"Frontera") # checks if we are on Frontera according to the Makefile.systype file
...
+ **Makefile.systype**: This is where you actually specify the machine on which you are running. Simply uncomment your system and comment out all other lines. For example, for the same "Frontera" system described above, we would simply un-comment the line
...
5. **Gravity walk** (device): *speculative* Barnes-Hut walk for the active set; force, optional potential, tidal tensor, jerk; adaptive softening; PMGRID short-range table. The walk is "speculative" in the sense that all leaves are descended optimistically with the opening criterion checked at the multipole level (rather than partitioning the tree into pre-decided open/closed nodes on the host first); this maps efficiently to GPU warps/wavefronts where divergent control flow is expensive, at the cost of doing slightly more work per particle compared to a host-side adaptive walker. The net result is a substantial wall-clock speedup over the legacy export-loop CPU tree walk, especially for full-active steps where every particle's walk runs concurrently.
...
**MEAN\_MOLECULAR\_WEIGHT\_DEFAULT**: Sets the mean molecular weight assumed wherever no chemistry is actually being solved. The default is the fully-ionized value 0.59; set it to e.g. 2.3 for a run whose gas is cold and molecular, where that default would badly mis-state the temperature corresponding to a given internal energy.
...
**EOS\_HELMHOLTZ**: The gas will use the Timmes & Swesty 2000 equation of state (for e.g. stellar or degenerate gas equations of state). The electron fraction and mean atomic weight are taken as input parameters (specified in the IC file or calculated on-the-fly if the appropriate physics is included); the EOS then takes these, the density, and internal energy, and returns the pressure, temperature, and entropy of the system, needed for the hydrodynamic solver. This is a standard Helmholtz EOS approach, and standard parameters of the EOS can be edited in the files in the `helmholtz` folder. Necessary tables are provided [here](http://www.tapir.caltech.edu/~phopkins/public/helm_table.dat) or in the wiki section of the GitHub site.

**EOS\_TILLOTSON**: The gas particles (Type=0) will use a Tillotson EOS (Tilloson 1962, "Metallic equations of state for hypervelocity impact," Tech. rep., General Dynamics, San Diego CA, General Atomic Div.). This is designed for solid or liquid bodies and solid/liquid+vapor mixtures, particularly popular for simulating solid bodies and their impacts (e.g. asteroid collisions, moon-impact simulations, comet impacts in water, etc). When this flag is active, all particles of Type=0 have an assigned material composition specified by the parameter "CompositionType" (which should be specified in the initial conditions file, or otherwise hard-coded somehow into the start-up operations). This will invoke pre-tabulated equations of state for different compositions: CompositionType=1,2,3,4,5,6 corresponds to granite,basalt,iron,ice,olivine/dunite,or water, respectively (with parameters compiled in Reinhardt+Stadel 2017,MNRAS,467,4252, Table A1). If CompositionType=0, a custom EOS will be used specified by the parameters in the parameterfile (if no particles have CompositionType=0, then these parameters will be ignored). (Exception: under the **EOS\_TYPES\_DEFAULTGAS\_AND\_SOLIDS** option -- automatically enabled by **GRAIN\_FLUID\_PROMOTION** -- CompositionType=0 instead denotes the standard gas fluid, with solid materials using CompositionType=1..6.) Per-particle CompositionType is read from the initial-conditions file by default and preserved (values outside the valid material range are reset to 0); set **IO\_COMPOSITIONTYPE\_NOT\_IN\_ICFILE** if your IC has no CompositionType block and composition is assigned at runtime. The Lagrangian methods in GIZMO are particularly well-suited to these simulations; for detailed demonstrations and example simulations, see Deng et al., arXiv:1711.04589. Note because of the composition assumptions, this should only be used with fixed-mass methods (MFM or SPH). Users of this module should cite Reinhardt+Stadel for the equations-of-state (unless a new one is used/implemented), and Deng et al. for the numerical implementation.

**EOS\_ANEOS**: The gas particles (Type=0) will use a tabulated ANEOS/SESAME equation of state, read from pre-computed lookup tables in (density, temperature) space. This is designed for planetary science applications such as giant impacts, asteroid collisions, and other problems involving realistic material equations of state with phase transitions. Multiple materials can be loaded simultaneously (up to 7), with each particle assigned a material via the "CompositionType" field in the initial conditions file (0-indexed, so CompositionType=0 uses the first table, CompositionType=1 the second, etc.; under **EOS\_TYPES\_DEFAULTGAS\_AND\_SOLIDS** CompositionType=0 instead denotes the standard gas fluid and the tables start at CompositionType=1). Per-particle CompositionType is read from the IC and preserved (out-of-range values reset to 0); set **IO\_COMPOSITIONTYPE\_NOT\_IN\_ICFILE** if the IC has no CompositionType block. The tables should be in SESAME ASCII format with columns for density, temperature, pressure, specific internal energy, specific entropy, and optionally sound speed and phase flag. The number of materials and file paths for each table are specified at runtime in the parameterfile via `AneosNumMaterials` and `AneosTable0`, `AneosTable1`, etc. Tables in the Stewart group SWIFT format (e.g. M-ANEOS forsterite from Stewart et al. 2019 or iron from Stewart 2020) can be converted using the provided `convert_stewart_table.py` script in the test suite. The code performs bilinear interpolation in log(density)-log(temperature) space and uses Newton-Raphson iteration (with bisection fallback) to invert the energy-temperature relation at each timestep. Pressure, sound speed, temperature, entropy, specific heat, Gruneisen parameter, and material phase are all computed from the tables. Note that `EOS_ANEOS` and `EOS_HELMHOLTZ` are mutually exclusive. This should only be used with fixed-mass methods (MFM or SPH).
...
#MHD_NON_IDEAL_CORRECTIONTERMS  # add anomalous-resistivity and Epstein-like drift/slip cross-section corrections to the non-ideal MHD coefficients computed in eos.cc. Refines Ohmic, Hall, and ambipolar coefficients in regimes where the strict-equilibrium and thermal-velocity approximations break down. Cite Hopkins & Squire 2024, arXiv:2405.06026
#MHD_BATTERY_MECHANISMS=1       # in-situ generation of seed magnetic fields from non-ideal EMFs. Bitfield: bit 0 (=1) electron Biermann battery (Kulsrud 1997); bit 1 (=2) radiative-ionization battery (Harrison 1973 / Durrive & Langer 2015); bit 2 (=4) charged-dust battery in TVA / fluid-limit (Soliman, Hopkins & Squire 2025 Eq. 17, mostly-neutral form, driven by dust-vs-gas radiation-pressure differential acceleration); bit 3 (=8) charged-dust battery with explicit dust current J_d summed from grain super-particles (Soliman+ 2025 Eq. 9 with alpha_O/alpha_H/alpha_A coefficients). Bits combine by addition (e.g. =3 = Biermann + RI). Requires MAGNETIC. Bit 1 requires RT_EVOLVE_FLUX + RT_CHEM_PHOTOION. Bits 2 and 3 require GRAIN_FLUID; bit 3 additionally requires RT_OPACITY_FROM_EXPLICIT_GRAINS. Bits 2 and 3 are mutually exclusive. Cite Soliman, Hopkins & Squire 2025, ApJ, 985, 55, when using the dust bits.
...
#MHD_MODIFIED_GRADIENT          # use Modified-Gradient (MG) method for machine-precision div(B)=0 via sparse matrix solve. Mutually exclusive with MHD_CONSTRAINED_GRADIENT. cite Tu, Wang, Gao & Tang 2026, arXiv:2603.04077
##-----------------------------------------------------------------------------------------------------
```
...
**MHD\_NON\_IDEAL**: Enables non-ideal MHD terms, specifically Ohmic resistivity, ambipolar diffusion, and the Hall effect. These are solved operator-split from the ideal MHD Reimann problem, as anisotropic diffusion terms appearing in the induction equation. Currently the numerical implementation of the induction equation terms is well-tested; the developer (PFH) asks that you cite the methods paper Hopkins 2017, MNRAS, 466, 3387, in addition to the GIZMO MHD methods paper, if you use these. However, the coefficients, which the code attempts to calculate based on the physical state of the gas, require careful calibration and testing for physical problems of interest, and authors should explore these for themselves. Some quite detailed modules are already in-code for problems of e.g. star and planet formation and proto-stellar/planetary disk dynamics as well as molecular cloud dynamics: details are in the `notes_nonidealmhd_in_ppds` file ([here](http://www.tapir.caltech.edu/~phopkins/public/notes_nonidealmhd_in_ppds.pdf)).

**MHD\_NON\_IDEAL\_CORRECTIONTERMS**: Adds approximate corrections to the non-ideal MHD coefficients (Ohmic resistivity $\eta_{\rm O}$, Hall coefficient $\eta_{\rm H}$, ambipolar coefficient $\eta_{\rm A}$) computed self-consistently from local gas state when `MHD_NON_IDEAL` is active. The default coefficients assume thermal velocities and strict charge/momentum-balance equilibria; the corrections here account for (1) anomalous resistivity at very large $|\nabla {\bf B}|$ where the drift speed of the current-carrying species exceeds the local thermal speed, and (2) Epstein-like drift/slip-dependent cross sections that modify the collision integrals when the relative drift between charged species and neutrals is non-negligible compared to thermal speeds. These can be important in cool dense neutral gas (proto-stellar disks, dense molecular cores) where the default coefficients can otherwise be off by factors of a few. The scalings used here are derived and presented in Hopkins & Squire 2024 ([arXiv:2405.06026](https://arxiv.org/abs/2405.06026)); please cite this paper in addition to the MHD methods paper if you enable this module.

**MHD\_BATTERY\_MECHANISMS**: Enables in-situ generation of seed magnetic fields from non-ideal EMFs that source the magnetic field even when ${\bf B} = 0$ initially. This is a bitfield, where each bit enables a physically distinct battery mechanism that can be combined via addition. Bit 0 (=1) is the standard electron Biermann battery, ${\bf E}_{\rm B} \propto \nabla P_{e} \times \nabla n_{e} / (n_{e}^{2} e)$, the conventional source term in fully-ionized plasmas (Kulsrud 1997). Bit 1 (=2) is the radiative-ionization battery from photoionization momentum deposition, where photons deliver excess momentum to electrons relative to ions (Harrison 1973; Durrive & Langer 2015), implemented per-cell as $\boldsymbol{\alpha}_{k} = (\sigma_{k,{\rm HI}} n_{\rm HI} + \sigma_{k,{\rm HeI}} n_{\rm HeI} + \sigma_{k,{\rm HeII}} n_{\rm HeII}) / (n_{e}\,e\,c)$ summed over the radiation bands $k$ and dotted into the local radiation flux ${\bf F}_{{\rm rad},k}$. Bit 2 (=4) is the charged-dust battery in the small-stopping-time (terminal-velocity-approximation, TVA) "mostly-neutral" form from Soliman, Hopkins & Squire 2025 Eq. 17: the dust battery EMF is driven by the dust-vs-gas differential acceleration from radiation pressure (per-band $\delta {\bf a}_{dg} = \sum_{k} (\kappa_{{\rm dust},k} - \kappa_{{\rm gas},k}) {\bf F}_{{\rm rad},k}/c$, with the IR band's dust/gas split taken from the existing `rt_kappa_adaptive_IR_band` helper, photoionizing bands assumed gas-dominated, and other bands assumed dust-dominated), then closed via the cool-plasma mobility weighting ${\bf E}'_{{\rm bat},d} = -(m_{d}\,\delta{\bf a}_{dg}/q_{d}) \mu_{d} \omega_{en} \omega_{pn} / [\mu_{d}\omega_{en}\omega_{pn} + \mu_{e}\omega_{pn}\omega_{dn} + \mu_{p}\omega_{en}\omega_{dn}]$, using the same collision frequencies (Pinto & Galli 2008 e-n and i-n rate coefficients, Epstein dust-neutral coefficient) and Draine & Sutin 1987 grain-charge equilibrium already used by the existing non-ideal MHD coefficient calculation in `eos.cc` (the SHS25 §2.4 proof shows the existing Wardle $\alpha_{\rm O,H,A}$ coefficients are equivalent to the SHS25 forms in the $J_{d} \to 0$ limit). Bit 3 (=8) is the same dust battery in the explicit-current form (SHS25 Eq. 9), where the per-cell dust current ${\bf J}_{d} = -\sum_{\rm grain} q_{d} n_{d} ({\bf v}_{d} - {\bf v}_{g})$ is summed in `solids/grain_physics_loop.cc` from the live `GRAIN_FLUID` super-particles (piggybacking on the existing grain $\to$ gas pass so no duplicate neighbor walk is required) and then dotted through the SHS25 $\alpha_{\rm O}/\alpha_{\rm H}/\alpha_{\rm A}$ tensor as ${\bf E}'_{{\rm bat},d} = -\alpha_{\rm O}\,{\bf J}_{d} - \alpha_{\rm H}\,({\bf J}_{d}\times\hat{\bf b}) - \alpha_{\rm A}\,({\bf J}_{d}\times\hat{\bf b})\times\hat{\bf b}$ — the explicit form is preferred when grain super-particle dynamics are actually resolved, while the TVA form is recommended for cosmological / CNM regimes where the dust population is sub-resolution. The two dust paths (bits 2 and 3) are mutually exclusive. All bits require `MAGNETIC`; bit 1 additionally requires `RT_EVOLVE_FLUX` and `RT_CHEM_PHOTOION`; bits 2 and 3 require `GRAIN_FLUID`, and bit 3 additionally requires `RT_OPACITY_FROM_EXPLICIT_GRAINS`. The battery EMF from all enabled bits is accumulated into `cell.E_battery_T2_cell` per cell and combined into the divergence-cleaned induction equation in the standard MHD flux loop. Please cite Soliman, Hopkins & Squire 2025, ApJ, 985, 55 (and the underlying Biermann, Harrison, and Durrive-Langer papers as appropriate) for any use of these modules.
...
**MHD\_MODIFIED\_GRADIENT**: Activate this to use the Modified-Gradient (MG) method of Tu, Wang, Gao & Tang (2026, arXiv:2603.04077) for machine-precision divergence-free magnetic field reconstruction. Unlike the CG method above, MG formulates the divergence constraint as a symmetric positive-definite sparse linear system and solves it using an algebraic multigrid (AMG) preconditioned conjugate gradient solver (via HYPRE, if available, or Intel MKL PARDISO). This achieves true machine-precision div(B)=0 at the cost of a global sparse solve per (active) timestep. The runtime parameter `ActiveFractionForMGSweep` (default: apply every step) can be set to a value between 0 and 1 to limit the MG correction to timesteps where at least that fraction of cells are active, with the cheaper CG method used on smaller steps. This flag is **mutually exclusive** with `MHD_CONSTRAINED_GRADIENT`. Please cite Tu, Wang, Gao & Tang 2026 if you use this method. When the MG sweep is skipped on a small step (active fraction below `ActiveFractionForMGSweep`), the previous MG solution is *retained* and the cheaper `MHD_CONSTRAINED_GRADIENT`-style local face correction runs *on top of* the stale MG state, correcting only the small drift accumulated for those cells that have advanced since the last MG solve (rather than starting from scratch). In test problems, this MG+CG combination with adaptive `ActiveFractionForMGSweep` has been shown to reduce $|\nabla\cdot{\bf B}|$ by ~4 orders of magnitude compared to CG alone (which itself reduces $|\nabla\cdot{\bf B}|$ by ~2 orders of magnitude compared to pure Dedner cleaning) — so while not formally achieving machine-precision $\nabla\cdot{\bf B}=0$ on every step, the adaptive scheme captures most of the divergence-control benefit of the full MG solve at a small fraction of the cost.


<a name="config-fluids-twotemperature"></a>
### _Two-Temperature Plasma (Independent Electron Temperature)_

```bash
##-----------------------------------------------------------------------------------------------------
#--------------------------------------- Two-Temperature Plasma
#TWO_TEMPERATURE_PLASMA=1       # evolve T_e independently from the ion/gas temperature T_i. Bitfield: bit 0 (=1) electron-ion Coulomb (Spitzer) equilibration + radiative cooling/Compton routed to T_e [core]; bit 1 (=2) electron-neutral elastic coupling [reserved, not yet implemented]; bit 2 (=4) route the Spitzer conduction deposition into u_e instead of u_total (requires CONDUCTION). Combine bits with addition. Requires COOLING. Mutually exclusive with CHIMES, EOS_HELMHOLTZ, and COOLING_OPERATOR_SPLIT (v1).
##-----------------------------------------------------------------------------------------------------
```

**TWO\_TEMPERATURE\_PLASMA**: Enables an independently-evolved electron temperature $T_{e}$ separate from the ion/gas temperature $T_{i}$, for problems where the electron-ion Coulomb equilibration timescale becomes comparable to or longer than other timescales of interest (e.g. shocks in dilute hot plasma, intracluster medium, supernova-remnant interiors, hot accretion flows). The primary state carried per cell is the specific electron internal energy per unit gas mass, $u_{e,{\rm cell}} = (3/2) n_{e} k_{\rm B} T_{e} / \rho$; the electron temperature $T_{e,{\rm cell}}$ is a derived cache. The *total* internal energy `InternalEnergy` remains the single conserved hydrodynamic variable -- the module only locally re-partitions it into electron and ion sub-energies (sub-energy mode), so the Riemann solver, snapshot consumers, and downstream couplings (including the Biermann battery in `MHD_BATTERY_MECHANISMS`, the gradient pass, and snapshot I/O) see the same total $u$ as in the single-temperature case. This is a bitfield: bit 0 (=1, required core) activates electron-ion Coulomb (Spitzer 1962) thermal equilibration with rate $\alpha_{ei} \propto n_{e} n_{i} \Lambda_{ei} / T_{e}^{3/2}$ and routes the radiative cooling and Compton-heating components of the cooling-function update to $T_{e}$ alone (rather than the total), so the cooling rate inside the rootfind is physically evaluated as a function of $T_{e}$ as it should be; bit 1 (=2) is reserved for electron-neutral elastic coupling, not yet implemented (enabling it will hard-fail at compile); bit 2 (=4) routes the Spitzer-Härm thermal conduction deposition (when `CONDUCTION` is active) into $u_{e}$ rather than $u_{\rm total}$, since electron heat conduction in a magnetized plasma is physically an electron-channel energy transport. By default, the fraction of hydro dissipation deposited into electrons at shocks is controlled by the runtime parameter `TwoTemp_ShockElectronFraction` (default $f_{e}=0$, the collisionless-shock limit following Vink+15, Ghavamian+13, where ions absorb all dissipation and electrons heat only via Coulomb equilibration; setting $f_{e}=1$ recovers strong-coupling single-fluid behavior). The initial $T_{e}/T_{i}$ ratio at startup is controlled by `TwoTemp_InitialTeOverTgas` (default 1.0 = LTE seed; set $\neq 1$ to initialize in a non-equilibrium state, e.g. for relaxation regression tests). Requires `COOLING`. Mutually exclusive in v1 with `CHIMES` (which evolves $T_{e}$ internally already), `EOS_HELMHOLTZ` (which owns $T$ directly via the tabulated EOS), and `COOLING_OPERATOR_SPLIT` (the current implementation uses the unsplit cooling rootfind path). Mach-dependent $f_{e}(\mathcal{M})$ (Ghavamian-style ramp) is deferred to a future refinement.
...
#GRAIN_COLLISIONS               # model collisions between grains (super-particles; so this is stochastic). Hard-sphere scattering across pi*(R_i+R_j)^2, weighted by how many grains each super-particle represents (see the scattering-rate equations in the DM_SIDM section); options for inelastic or velocity-dependent terms. Approved users please cite papers above and Rocha et al., MNRAS 2013, 430, 81
...
**IO\_DUST\_NOT\_IN\_ICFILE**: Needed when restarting from a snapshot (flag=2) which contains no dust fields, with `GALSF_ISMDUSTCHEM_MODEL` active. The dust species are then set from the parameterfile arguments and an MRN size distribution is assumed, instead of being read from the file.
...
<a name="config-fluids-multifluid"></a>
### _Multi-Fluid (Lagrangian-Partition) Framework_

GIZMO supports a generic multi-fluid framework in which `Type==0` (gas) particles are partitioned into Lagrangian sub-fluids by an integer tag `P[].FluidType`. Each fluid evolves its own hydrodynamics inside the existing hydro corridor — same density / gradient / pressure-force machinery — but pair operators skip cross-`FluidType` pairs, so different fluids do not directly exchange mass/momentum/energy through the hydro scheme. Cross-fluid coupling (drag, charge exchange, radiation feedback, etc.) is added by physics-specific sub-modules that explicitly walk cross-`FluidType` pairs. The base framework is designed primarily as a scaffold for users adding their own new physics: a clean partition of Type=0 into multiple co-evolved fluids, with all the structural plumbing (IC reader, snapshot writer, restart, domain exchange, ghost packing, merge/split inheritance) handled, so the user only writes their cross-fluid coupling. Several reference sub-modules are included to demonstrate the pattern.

```bash
## ----------------------------------------------------------------------------------------------------
# --------------------------------------- Multi-Fluid Framework (Lagrangian-partition multi-fluid)
#HYDRO_MULTIFLUID               # base scaffold: Type=0 particles carry a FluidType ID; hydro pair operators skip cross-FluidType pairs (intra-fluid hydro only). Force-implies EOS_GENERAL.
#HYDRO_MULTIFLUID_DUST_DRAG     # cross-fluid drag coupling for Type=0+FluidType=FLUID_DUST_GRAIN dust-as-fluid against Type=0+FluidType=FLUID_DEFAULT gas; reuses the GRAIN_FLUID drag/backreaction kernels. Mutually exclusive with GRAIN_FLUID / HYDRO_MULTIFLUID_IONNEUTRAL.
#HYDRO_MULTIFLUID_IONNEUTRAL    # two-fluid ion-neutral ambipolar drag: Type=0+FluidType=FLUID_ION ions coupled to Type=0+FluidType=FLUID_DEFAULT neutrals via the Draine ambipolar form. Mutually exclusive with GRAIN_FLUID / HYDRO_MULTIFLUID_DUST_DRAG.
#HYDRO_MULTIFLUID_DM            # dark-fluid placeholder: Type=0+FluidType=FLUID_DM particles get a dedicated trivial adiabatic EOS and dedicated do_dark_cooling_for_particle hook. Gas->star/sink promotion redirected to inert Type=3. Baryonic feedback kernels skip FLUID_DM neighbors; RT radiation pressure on FLUID_DM gas zeroed.
#HYDRO_MULTIFLUID_DM_COOLING    # opt-in reference implementation of atomic+molecular ADM cooling, distilled from Roy et al. (cite below). Requires HYDRO_MULTIFLUID_DM. Adds four runtime parameters: ADM_FineStructure, ADM_ProtonMass, ADM_ElectronMass, ADM_MolecularFraction.
##-----------------------------------------------------------------------------------------------------
```

**HYDRO\_MULTIFLUID**: The base scaffold. Adds an `unsigned char FluidType` field to `P[]` (stored in existing struct padding, so no per-particle storage cost on most Configs), and an enum of canonical fluid IDs (`FLUID_DEFAULT=0`, `FLUID_DUST_GRAIN=1`, `FLUID_ION=2`, `FLUID_NEUTRAL=3`, `FLUID_DM=4`, with values 5..255 reserved). The HDF5 IC reader looks for a per-particle `FluidType` dataset on `PartType0` and reads it as an integer (defaults to 0 when absent), and snapshots write the field on output. The hydro corridor (density, gradient, cell-corrections, hydro-force loops) adds a top-of-pair-body skip `if (!same_lagrangian_fluid_id(ft_i, ft_j)) return;` so different fluids do not directly mix hydro state. Cross-fluid mergers in `merge_split.cc` are hard-refused. When this flag is on but no sub-module (DUST_DRAG / IONNEUTRAL / DM) is enabled, the framework is a no-op: the FluidType field is carried passively but the corridor skip prevents inter-fluid hydro interaction, so users can build their own coupling physics by adding cross-fluid pair body inserts in the relevant loops. The flag force-implies `EOS_GENERAL` so per-cell EOS state (Pressure, SoundSpeed, Temperature) is saved per-cell rather than re-computed each read — this localizes per-fluid EOS variation to one site (the EOS compute path), which is required for any sub-module that needs a fluid-specific equation of state. Citation: this scaffold is general and lightweight (no specific publication is required for the base flag itself); sub-modules below have their own citations.

**HYDRO\_MULTIFLUID\_DUST\_DRAG**: Cross-fluid drag coupling between a Type=0 dust-as-fluid (tagged FluidType=`FLUID_DUST_GRAIN`) and a Type=0 ambient gas (tagged FluidType=`FLUID_DEFAULT`). Internally, this reuses the existing `GRAIN_FLUID` aerodynamic drag and backreaction kernels (`grain_drag_kernel`, `grain_backrx_pair_kernel`) but widens their source-side eligibility predicate (`IS_PARTICLE_DRAGVALID`) to also accept Type=0+`FLUID_DUST_GRAIN` particles. The dust-as-fluid case is physically distinct from the `GRAIN_FLUID` (Type=3, collisionless dust-as-PIC) case — dust-as-fluid is appropriate when grain stopping times are short compared to the orbital/dynamical timescale (strongly-coupled limit) such that the grain population behaves as a separate fluid rather than as a collisionless PIC sample of grain orbits. The implementation reuses the existing dust drag formula (Epstein + Stokes corrections, with optional Lorentz force under `GRAIN_LORENTZFORCE`) since these are mechanically identical between Type=3 grain and Type=0 dust-fluid; users implementing more sophisticated dust-fluid microphysics (size-spectrum-resolved fluid mixtures, etc.) can extend the kernel branch directly. Compile-time dependencies: requires `HYDRO_MULTIFLUID`, `GRAIN_FLUID`, and `GRAIN_BACKREACTION` since the kernel/storage are reused; this is an *implementation* dependency, not a physics dependency — the dust-fluid does not modify or activate Type=3 grain physics. Cite Hopkins & Squire 2018 (MNRAS, 480, 2813) and Soliman et al. 2025 (ApJ, 985, 55) for the underlying dust-MHD framework whose drag/backreaction kernels this re-uses.

**HYDRO\_MULTIFLUID\_IONNEUTRAL**: Two-fluid ion-neutral ambipolar drag coupling. Ions are tagged `FluidType=FLUID_ION`, neutrals `FluidType=FLUID_DEFAULT`. Ions carry the magnetic field; neutrals carry $B=0$ in the IC and remain at zero by induction since the intra-fluid corridor skip prevents cross-fluid MHD pair contributions. Cross-fluid drag is computed via the Draine ambipolar form: $1/t_{\rm stop} = \gamma_{\rm AD}\,\rho_{\rm neutral}$ with $\gamma_{\rm AD} = \langle\sigma v\rangle_{in} / (m_i + m_n)$. Hardcoded HI/H${}^+$ values are used by default: $\langle\sigma v\rangle_{in} = 1.9\times 10^{-9}\,{\rm cm}^3/{\rm s}$ (Draine 1980, ApJ, 241, 1021) and $m_i + m_n = 2\,m_p$, giving $\gamma_{\rm AD} \approx 5.7\times 10^{14}\,{\rm cm}^3\,{\rm g}^{-1}\,{\rm s}^{-1}$ for atomic ISM. Users in other regimes (e.g. HCO${}^+$/H$_2$ in molecular clouds, where $\gamma_{\rm AD} \approx 4\times 10^{13}\,{\rm cm}^3\,{\rm g}^{-1}\,{\rm s}^{-1}$; Mouschovias 1991, ApJ, 373, 169) edit two transparent constants at the top of the ion-neutral drag branch in `solids/grain_drag_gpu.cc`. Temperature dependence (weak, scaling as $T^{0.5}$) is neglected in this default implementation. The existing Epstein drag integrator is retained for the velocity update — it is physically appropriate even for ion-neutral drag, since ambipolar drag becomes nonlinear in an Epstein-like fashion in the supersonic regime (the linear approximation only holds at subsonic relative velocities). Mutually exclusive with `GRAIN_FLUID` and `HYDRO_MULTIFLUID_DUST_DRAG` (compile-time enforced by macro-redefinition sentinel). Cite Draine 1980, Hopkins & Squire 2018, and the references in their §2 for the underlying ambipolar drag framework.

**HYDRO\_MULTIFLUID\_DM**: Dark-fluid placeholder. Particles tagged `FluidType=FLUID_DM` get a dedicated trivial adiabatic EOS ($P = (\gamma-1)\rho u$ with $\gamma=5/3$) and a dedicated cooling entry point `do_dark_cooling_for_particle`. By default the cooling hook is a no-op (dark fluid evolves strictly adiabatically); the opt-in `HYDRO_MULTIFLUID_DM_COOLING` sub-flag activates a reference atomic+molecular ADM cooling implementation (see below). The framework also handles the cross-coupling of dark fluid with baryonic physics in the standard FIRE/STARFORGE physics stack: (1) gas-to-star and gas-to-sink promotion in `galaxy_sf/sfr_eff.cc` redirects FLUID_DM particles to inert collisionless Type=3 instead of Type=4 stars or Type=5 sinks, so dark gas that would have formed stars in a standard run becomes a collisionless dark-star-analog tracer; (2) RT radiation pressure on FLUID_DM gas is zeroed in `gravity/gravtree.cc` (no full RHD coupling — this would have to be added separately for a full mirror-RT model); (3) baryonic stellar feedback kernels (mechanical FB, thermal FB, sink wind/kick, HII heating, local RT) skip FLUID_DM neighbors so dark gas does not feel SNe / wind / radiation feedback from baryonic stars; (4) helper functions `ThermalProperties` and `Get_Gas_Molecular_Mass_Fraction` short-circuit for FLUID_DM particles so the full thermochemistry call-stack is bypassed in typical (e.g. FIRE) Configs. The minimum set of edits needed to add a new dark-species sub-module on top of this is small: this single base flag handles the structural plumbing; adding e.g. mirror-RT or DM-DM scattering only requires adding the physics-specific dispatch where relevant. Citation is not specifically required for the base flag; cite the cooling sub-flag references below if the cooling extension is used.

**HYDRO\_MULTIFLUID\_DM\_COOLING**: Opt-in reference implementation of atomic + molecular cooling for the dark fluid, requiring `HYDRO_MULTIFLUID_DM`. Implements collisional excitation, collisional ionization, radiative recombination (line + free-bound), and free-free (Bremsstrahlung) cooling for a dark-hydrogen-like fluid with user-supplied fine-structure constant $\alpha_{\rm ADM}$, proton mass $m_{p,{\rm ADM}}$, and electron mass $m_{e,{\rm ADM}}$, plus optional molecular dark-H$_2$ rovib cooling with a user-supplied initial dark-H$_2$ fraction. The implementation lives in `sidm/dm_cooling_functions.h` (the dark-fluid EOS hook stays in the EOS-only `sidm/dm_fluid_functions.h` to keep the cooling-table dependency out of broad EOS consumers); the consolidated table struct + builder declaration are in `sidm/dm_cooling_tables.h`; the `__managed__` table instance is owned by `cooling/cooling.cc` next to `CoolTables`. The chain uses GIZMO's in-tree 20-point Gauss-Legendre quadrature for the cross-section rate integrals (no external library dependency). Photoionization is absent by design (no cosmic-microwave-background analog is assumed for the dark fluid; users implementing a dark-sector radiation background should add the term themselves). The cooling chain (implicit bisection solver, ionization-equilibrium balance, ADM-scaled Glover & Abel 2008 / Glover 2015 H$_2$ rovib curves) is largely a cleaned-up consolidation of code originally developed by Roy et al. (see also Ryan et al. 2024 arXiv:2403.05624 and Roy, Hopkins, & Squire in prep) for atomic dark-matter cosmological simulations. **This is placeholder reference machinery, not extensively validated against an analytic ADM benchmark in the present code base** — users implementing real ADM science should re-validate (a natural first sanity check is to set all `ADM_*` parameters to their Standard Model values and verify equivalence to the standard `COOLING` cooling rates). Adds four runtime parameters that must be specified in the parameterfile: `ADM_FineStructure` (= $\alpha_{\rm ADM}$; SM value $\approx 7.2974\times 10^{-3}$); `ADM_ProtonMass` (= $m_{p,{\rm ADM}}$ in g; SM value $1.6726\times 10^{-24}$); `ADM_ElectronMass` (= $m_{e,{\rm ADM}}$ in g; SM value $9.1094\times 10^{-28}$); `ADM_MolecularFraction` (initial dark-H$_2$ mass fraction, 0..1). Cite Roy et al. and the underlying Glover & Abel 2008 (MNRAS, 388, 1627), Glover 2015 (MNRAS, 451, 2082) for the molecular cooling tables. **Users should also note that the base `HYDRO_MULTIFLUID_DM` scaffold is intentionally minimal — the cooling sub-module is one example of how to extend it, but users with more sophisticated dark-sector microphysics (multi-species, DM-DM scattering, mirror radiation transport, ADM nucleosynthesis, etc.) should treat this as a template to extend, not as a production-ready ADM cooling library.**

A general note for users adding their own multifluid sub-modules: the framework intentionally exposes a clean two-loop separation between own-fluid hydro (handled by the existing corridor with the same-FluidType skip) and cross-fluid coupling (handled by sub-module pair bodies inside the existing kernels, dispatched on `FluidType` predicates). When adding a new sub-module, the typical pattern is (a) reserve a new `FluidID` enum value in `declarations/multifluid_helpers.h`; (b) add a new sub-flag to `Template_Config.sh` (mutually exclusive with other drag-source sub-flags via the existing `IS_PARTICLE_DRAGVALID` macro-redefinition sentinel if it is a new drag source, orthogonal otherwise); (c) add the cross-fluid pair body insert in the relevant existing kernel(s), gated on the new sub-flag and the appropriate FluidType pair predicate; (d) if the sub-module needs new per-particle state, add it to `particle_data` gated on the new sub-flag, and extend `init.cc` / IC reader / snapshot writer / restart / domain exchange / ghost packing as needed (follow the existing `Gas_Density` / `Gas_Velocity` / `Grain_AccelTimeMin` pattern). The framework is deliberately *not* a generic n-species coupling matrix — each sub-module is its own physics, written to fit the case at hand, with shared infrastructure but separate code paths.
...
#GRAIN_FLUID_AND_PIC_BOTH_DEFINED #- this tells the code that both GRAIN_FLUID (dust) and PIC_MHD (for e.g. cosmic rays or other applications) are simultaneously active. cite Ji, Squire, & Hopkins, arXiv:2112.00752
## --------------------------------------------------------------------------------------------------
```
...
Some additional details of the modules are described in the `notes_cosmicrays` file ([here](http://www.tapir.caltech.edu/~phopkins/public/notes_cosmicrays.pdf)), but a more formal and accurate description is in the methods papers cited above.

The specific flags include:

```bash
## ----------------------------------------------------------------------------------------------------
#COSMIC_RAY_FLUID               # top-level switch to evolve the distribution function (continuum limit) of a population of CRs. includes losses/gains, coupling to gas, streaming, diffusion. this uses the multi-moment formulation correct in arbitrarily anisotropic and free-streaming or fluid-like limits. all operators will be anisotropic as it should unless MHD is turned off. cite Hopkins et al. arXiv:2103.10443,2002.06211,2202.05283
...
## -----------------------------------------------------------------------------------------------------
# --------------------------------------- Tree-Walk Precision (performance optimization)
#GIZMO_MIXED_PRECISION_GRAVITY  # use single-precision (float) for the gravity force-kernel accumulators and tree-node moment storage, while keeping particle positions in double precision. Cuts gravity-tree memory and bandwidth ~2x at sub-percent force accuracy cost for typical multi-physics simulations. Default OFF (all gravity quantities in double precision). Follows pkdgrav3 / Bonsai conventions.
##-----------------------------------------------------------------------------------------------------
```
...
#ADAPTIVE_GRAVSOFT_FORGAS       # allows variable softening length (=KernelRadius) for gas particles
#ADAPTIVE_GRAVSOFT_FORALL=2     # enable adaptive gravitational softening lengths for designated particle types (ADAPTIVE_GRAVSOFT_FORGAS will be enabled). the softening is set to the distance
...
#DM_HEATING                     # Continuous gas heating from DM annihilation and/or decay (rho_DM-driven source term added to DtInternalEnergy). Run-time params control the two channels independently. Cite Hopkins et al., 2026 (internal).
##-----------------------------------------------------------------------------------------------------
...
​                    
**DM\_SIDM**: Enables self-interacting dark matter, following M. Rocha, James Bullock, and Mike Boylan-Kolchin. The per-pair scattering probability is the kernel-weighted form given under "Scattering rate used for `DM_SIDM` and `GRAIN_COLLISIONS`" below; it replaces the tabulated overlap integral of the original implementation, and gives the same ensemble-averaged rate with a somewhat different pointwise normalization. Like `PM_PLACEHIGHRESREGION`, the parameter needs to be set to an integer that encodes the particle types that make up the SIDM particles in the form of a bit mask. For example, if types 1 and 2 are the SIDM particles, then the parameter should be set to `DM_SIDM` = 6 = 2 + 4 (i.e. $2^{1} + 2^{2}$). This allows for arbitrary combinations of SIDM particle types. Note that the legacy flag `DMDISK_INTERACTIONS` is now implicit in this (its identical to setting `DM_SIDM=2+4`). This full module as-written in the development code is proprietary and users must obtain permissions from the developers (JB and MBK) for use in scientific products. But a slightly simpler version is part of the public code; moreover if users wish to use the implemented architecture and replace the actual self-interaction kernel with their own, they are free to do so provided they include the appropriate citations. If this is active, the user must set several parameterfile options that specify the dark matter self-interaction cross-section, its dependence on relative velocity, and the degree of exo-or-endo-thermality (i.e. dissipation or gain of kinetic energy per interaction). See the parameterfile description below for description of these. Users should cite Rocha et al., MNRAS 2013, 430, 81 and Robles et al, 2017 (arXiv:1706.07514).

**Scattering rate used for `DM_SIDM` and `GRAIN_COLLISIONS`**: both modules decide, for each neighbor pair, whether the two elements scatter in this timestep. The probability is built from the adaptive-softening kernel each element already carries, rather than from an overlap integral of two extended "clouds". For self-interacting dark matter, with macro-particle masses `M_i`, `M_j`, cross-section per unit mass `sigma/m` (`DM_InteractionCrossSection`), relative velocity `|dv|`, timestep `dt`, and the usual kernel `W(r,h)`:

    p_ij = |dv| * dt * a^-3 * (sigma/m) * [ M_i*M_j / (M_i+M_j) ] * [ W(r,h_i) + W(r,h_j) ]

For grain-grain collisions the elements represent *numbers* of grains rather than masses: a particle of mass `M` and grain radius `R` stands for `N = M/mu` grains of mass `mu = (4*pi/3) * rho_grain * R^3` (`Grain_Internal_Density`). Two particles of equal mass therefore hold very different numbers of grains if their grain sizes differ, and the collision rate is set by the side holding more. With hard spheres meeting across `pi*(R_i+R_j)^2`:

    p_ij = |dv| * dt * a^-3 * [ N_i*N_j / (N_i+N_j) ] * pi*(R_i+R_j)^2 * [ W(r,h_i) + W(r,h_j) ]

Both expressions are symmetric, so the two elements of a pair compute the same probability and agree on whether they scatter, and an accepted event gives them equal and opposite momentum kicks. Summed over a particle's neighbors, each one-sided term returns the density that particle moves through, so the rate is normalized by construction. Because `W(r,h)` vanishes for `r >= h`, the probability is non-zero exactly when `r < max(h_i,h_j)`: **the neighbor criterion is the ordinary symmetric one**, with no widened search radius and no separate overlap test. The velocity-dependent suppression (`DM_InteractionVelocityScale`, `Grain_InteractionVelocityScale`) multiplies the expressions above unchanged.
...
### _Continuum Vlasov (6D Phase-Space Integration) Methods_ 
...
These flags implement a Lagrangian, finite-volume method for directly integrating the continuum collisionless (or weakly-collisional) Boltzmann/Vlasov equation in $6+1$-dimensional phase space, in place of the traditional Monte-Carlo/N-body sampling of point-mass "particles." Each cell (mesh-generating point) carries a set of `CBE_INTEGRATOR` velocity-space basis functions -- Gaussian-like components with independently evolved mass, mean velocity, and (optionally) anisotropic velocity dispersion -- which are advanced in time via a Godunov-type scheme: at each face between neighboring cells, a basis on one side is paired with the most similar basis on the other side (by a Wasserstein-like velocity-space cost, with a "free-slot" fallback that routes mass into the least-populated basis when no good match exists), a one-sided Riemann-type flux is computed between the paired bases, and the mass/momentum/stress moments of each basis are updated conservatively. The method is designed to couple transparently to the existing $N$-body/tree gravity, adaptive softening, and domain-decomposition machinery in GIZMO -- so a `CBE_INTEGRATOR`-active particle type still has a well-defined position and mass for gravity purposes, but its velocity-space content is a continuum DF rather than a $\delta$-function. This can represent multiple crossing streams, phase-mixed/thermalized distributions, and everything in between, with a modest, bounded cost multiplier relative to standard $N$-body methods (rather than the exponential blow-up of grid-based Vlasov solvers). A methods paper describing the algorithm, its validation on standard collisionless test problems, and example applications to dark matter, stellar dynamics, and weakly-collisional gas/dust is in preparation (Hopkins 2026); cite this if the module is used.

**CBE\_INTEGRATOR**: Top-level switch turning on the CBE integrator for the particle types specified by `CBE_INTEGRATOR_PARTICLETYPES`. The value sets the number of basis functions $N_{\rm basis}$ carried per particle/cell (e.g. `=8`); more bases can represent more complex, multi-modal, or strongly anisotropic DFs at increased memory and compute cost. Enabling this automatically implies adaptive gravitational softening for the same particle types (as `ADAPTIVE_GRAVSOFT_FORALL`), since the volume-partition on which the CBE fluxes are defined requires a well-defined, adaptive smoothing/interaction length. This module is still under active development and validation; discuss with PFH about the current status and appropriate use cases before using it in a production/publication run.

**CBE\_INTEGRATOR\_PARTICLETYPES**: Bitmask (identical convention to `PM_PLACEHIGHRESREGION`/`DM_SIDM` above) specifying which particle types carry the CBE basis-function representation. Defaults to type 1 (the standard collisionless/dark-matter type) if not set.

**CBE\_INTEGRATOR\_SECONDMOMENT**: Evolves the second moment (symmetric stress tensor) of each basis in addition to its mass and mean velocity -- the "10-moment" scheme (mass + $D$-component momentum + $D(D+1)/2$-component symmetric stress, in $D$ spatial dimensions). This lets each basis carry its own anisotropic velocity dispersion, rather than assuming a single width/softening for the whole DF, and is the recommended default for any use of `CBE_INTEGRATOR`.

**CBE\_INTEGRATOR\_HEATFLUX**: Extends the moment hierarchy to 13 moments by additionally evolving a contracted third-moment (heat-flux) vector for each basis (implies `CBE_INTEGRATOR_SECONDMOMENT`). This can improve convergence and accuracy for DFs close to a local Maxwellian, at the cost of requiring more careful realizability/positivity handling as the DF departs further from equilibrium (see the discussion of 10- vs. 13-moment trade-offs in the method paper). Still experimental.

**CBE\_INTEGRATOR\_WITHGRADIENTS**: Enables second-order spatial reconstruction: each basis' primitive variables (density, mean velocity, and -- if `CBE_INTEGRATOR_SECONDMOMENT` is on -- stress) are extrapolated to the cell faces via slope-limited gradients (a Barth-\&-Jesperson-style limiter, component-separated between density, velocity, and stress to avoid one noisy component spuriously damping the others), rather than using the first-order, piecewise-constant cell-centered values directly. Strongly recommended for production runs; first-order (this flag off) is useful mainly for basic testing/debugging.
...
These flags enable group finding via an on-the-fly FOF finder which is run at intervals set by the run-time parameters. This is most commonly used for halo finding in cosmological simulations, to build on-the-fly halo catalogues, or to simply track halos for purposes of seeding sink particles and other on-the-fly physics. However it has also been modified in previous applications to track things like star clusters, planet-forming clumps, or really any type of self-gravitating objects on-the-fly. The current defaults assume halo-finding, however, by defining structures as above a traditional virial overdensity. Users of *any* of these modules should cite Springel et al., MNRAS, 2001, 328, 726 for the numerical methods.
...
#GALSF_SFR_VIRIAL_SCALING=2   # instead of a threshold, implements a semi-continuous SF efficiency as a function of alpha_vir. set 0=step function between 1 and 0.01; 1=Padoan 2012 prescription; 2=multi-free-fall model, as in e.g. Federrath+Klessen 2012/2013 ApJ 761,156; 763,51 (similar to that implemented in e.g. Kretschmer+Teyssier 2020), based on the analytic models in Hopkins MNRAS 2013, 430 1653, with correct virial parameter
...
#GALSF_FB_FIRE_AGE_TRACERS      # add explicit tracking of tracer species for arbitrary stellar yields as a function of age from different stellar populations. cite Wetzel et al. 2023 (arXiv:2203.00040)
##-----------------------------------------------------------------------------------------------------
#################################################################################################### 
```

These flags control the feedback models used in the series of FIRE papers, explicitly tracking of multiple forms of feedback physics (without free or tuned parameters). As noted above, the examples of these modules which are only in the development code and not in the public version of GIZMO are proprietary and are not to be used without the explicit permission of the FIRE collaboration core development team, consisting of PFH, D. Keres, C.A. Faucher-Giguere, and E. Quataert. I (PFH) do not have the ability to grant permissions alone, so permission to use the development code does not constitute permission to use all of the modules below. Projects using these modules are subject to the FIRE collaboration standard policies (projects must be pre-approved and posted to the collaboration, authorship offered, collaboration citation policies followed, etc.). For versions of these modules in the public code, you should cite the appropriate FIRE methods papers, namely Hopkins et al. Hopkins et al. FIRE1/2/3 methods: arXiv:1311.2073, 1702.06148 and 2203.00040. Using those modules in your own work does not constitute a FIRE simulation or collaboration paper, you should refer to these numerical methods as following the implementation from FIRE in (cite appropriate FIRE version papers), as provided in the public GIZMO code, similar to citation policy for public FIRE simulation snapshots. If you wish to join the FIRE collaboration to participate in the FIRE project, write FIRE papers, and access non-public datasets, please reach out to members of the collaboration to discuss joining. 
...
**GALSF\_FB\_FIRE\_AGE\_TRACERS**: Enables explicit tracking of tracer species for arbitrary stellar yields as a function of age from different stellar populations. When enabled, additional passive scalar fields are appended to each particle's metallicity array, each corresponding to a discrete stellar age bin. As star particles age, the feedback (mass, energy, metals) deposited by each age bin is tracked separately in nearby gas. By default, the age bins are logarithmically spaced between `AgeTracerBinStart` (default 1 Myr) and `AgeTracerBinEnd` (default 14000 Myr), with `AgeTracerEventsPerTimeBin` (default 10) controlling the deposition rate normalization. If `GALSF_FB_FIRE_AGE_TRACERS_CUSTOM` is also set, a custom list of bin edges can be specified via `AgeTracerListFilename`. This is enabled by default with `FIRE_PHYSICS_DEFAULTS` for newer FIRE versions. Users should cite Wetzel et al. 2023 (arXiv:2203.00040).
...
**GALSF\_ISMDUSTCHEM\_MODEL**: Enables live ISM dust evolution, tracking the formation, destruction, and processing of multiple dust species self-consistently with stellar evolution, feedback, and gas-phase chemistry. The value is a sum of bitflags selecting which dust species are followed: 1=silicates and carbonaceous dust, 2=metallic iron dust, 4=oxygen-bearing dust species (an oxygen reservoir), 8=metallic iron nanoparticles carried as inclusions within silicates. Bit 1 is the base set and is always required -- silicates and carbon are tracked in every variant, and omitting it is a compile-time error -- and bit 8 requires bit 2, since the iron inclusions are drawn from the metallic iron. Formation channels include stellar sources (SNe Type Ia/II, AGB winds) with composition-dependent yields; destruction channels include sputtering in hot gas and destruction by SNe shocks; growth channels include gas-phase accretion in the ISM. A typical choice is `GALSF_ISMDUSTCHEM_MODEL=(1+2)`. Users should cite Choban et al. 2022 (arXiv:2112.09141) and subsequent papers for the methods and calibration. **GALSF\_ISMDUSTCHEM\_GRAINSIZEEVO** can be additionally enabled to track grain size evolution with logarithmically-spaced size bins (set the value to the number of bins, e.g. 16), enabling coagulation, shattering, accretion, and sputtering as a function of grain size; this requires `GALSF_ISMDUSTCHEM_MODEL`=1 or (1+2) only, together with `GALSF_ISMDUSTCHEM_SILICATE_COMPOSITION`. Cite Choban et al. for the grain size evolution methods. **GALSF\_ISMDUSTCHEM\_SILICATE\_COMPOSITION** sets the chemical composition assumed for the silicate dust, which in turn changes the production, growth and destruction rates of the silicate species and the maximum depletions of Mg, Fe, Si and O permitted in the gas phase; the value is a sum of bitflags: 1=an olivine-pyroxene mix, which is the default, 2=add two extra oxygen atoms to better match the observed oxygen depletions, 4=add one extra iron atom to better match the iron depletions, 8=remove all iron from the silicates, which is meant to be used together with the metallic iron species. This is read as a plain integer rather than tested bit by bit, so it is always defined whenever the dust model is active, defaulting to 1 if it is not set explicitly; `(1+2+8)` is a common choice. It is required by the grain size evolution above.
...
# -------------------- (unlike GALSF options, these sinks are individual accretors, not populations). Much in common with black hole-centric modules below.
...
**SINGLE\_STAR\_SINK\_DYNAMICS**: Top-level switch. Must be on for any of the other modules within this block to work. These are essentially different 'skins' or options for the sink particle modules, which are described below, so make sure to read both sections if you are planning on using these. More details of the generic sink particle modules (including e.g. accretion and dynamics options) are found in the `notes_blackholes` file ([here](http://www.tapir.caltech.edu/~phopkins/public/notes_blackholes.pdf)). Additional details that are unique to the sink modules in this section will be described in the (currently in-progress) `notes_singlestar` file ([here](http://www.tapir.caltech.edu/~phopkins/public/notes_singlestar.pdf)), until the methods paper is published.
...
**DISABLE\_HERMITE\_INTEGRATION**: Negates the `HERMITE_INTEGRATION` which `SINGLE_STAR_STARFORGE_DEFAULTS` would otherwise switch on, leaving the sinks on the ordinary KDK leapfrog. This is useful as a convergence control: leapfrog is 2nd order where the Hermite scheme is 4th, so running the same problem both ways separates the errors which come from the integrator from those which do not.

**SINGLE\_STAR\_RT\_DEFAULTS**: Enables the radiation-transport settings which the STARFORGE radiative modules have in common -- the M1 solver, the comoving frame, the reduced speed of light, and photon injection from sinks -- without the radiative-feedback band set which would ordinarily accompany them. `SINGLE_STAR_FB_RAD` switches this on for you, so you only need to set it directly if you want that transport configuration without the feedback bands.

**JET\_DIRECTION\_FIXED\_Z**: For testing only. Launches spawned jets along $\pm z$ rather than along the accreted angular-momentum axis, so that the launch geometry can be checked against a known direction. This is not physical and should not be enabled for science runs.

**WIND\_MDOT**: For testing only. Hard-codes the stellar wind mass-loss rate, in ${\rm M_{\odot}\,yr^{-1}}$, instead of taking it from the stellar evolution model, so that wind injection can be exercised at a controlled and known rate.

**WIND\_LUMINOSITY**: For testing only. Hard-codes the wind luminosity in erg/s; together with `WIND_MDOT` this fixes the wind velocity through $L_{w}=(1/2)\dot{M}_{w}v_{w}^{2}$.

**IO\_HERMITE\_SYNC**: Writes `HermiteSyncCoordinates` and `HermiteSyncVelocities` into snapshots: a mutually consistent position-velocity pair evaluated at the output time. The ordinary snapshot fields are not this, since the positions are at the drift time while the velocities are at the kick time. That mismatch is harmless for most purposes, but it is wrong if you want to compute orbital elements, apply vis-viva, or take a kinetic energy directly from a snapshot, so enable this if you need any of those.
...
**SINGLE\_STAR\_SINK\_FORMATION**: With this enabled, sink particles can be created 'on the fly' as opposed to simply in the ICs. You set the value to a bitflag which allows users to specify any arbitrary combination of criteria for star formation (provided they are physically compatible): the bitflag is set to the sum of the values corresponding to each criterion you want to enable, including: **(0)** A density threshold (set by the `CritPhysDensity` parameter in the params file). Note because the value is `0` here, this is effectively always on, but you can essentially disable it by simply setting `CritPhysDensity` very low or zero. **(1)** A local virial criterion (self-gravitating at the resolution scale including kinetic, thermal, and magnetic energy (see the description above for details of what this entails and for the proper references if you use it). **(2)** Requires the gas be in a convergent flow ($\nabla\cdot{\bf v}<0$). **(4)** Require the gas cell sit at a local density and potential extremum (evaluated within kernel). **(8)** Require that there are no other sinks inside three times the gas kernel/neighbor radius, and the gas is not a valid 'interacting neighbor' of any other sink particle. **(16)** Require the timescale for the gas element to free-fall onto (using a 2-body orbit) or intercept ($\sim 1/(\Delta{\bf v}\cdot \Delta {\bf r})$) the nearest sink is longer than the local dynamical collapse time. Cite Grudic et al. arXiv:2010.11254 for the algorithm. **(32)** Enforce a Hill-type criterion that the local tidal tensor is negative definite (forces are converging along all three principle axes) and that the gas cell lives inside its own Hill sphere. This is a much stronger and more rigorous (Galilean-invariant) version of a local maximum-type criterion. Cite Grudic et al. arXiv:2010.11254 for the algorithm. **(64)** Require that the thermal+magnetic Jeans mass be less than the minimum of 1e4 solar masses or 100x the gas cell mass (but no smaller than 1e-3 solar masses). Cite Grudic et al. arXiv:2010.11254. **(128)** Require that the flow be converging along all three principle axes by diagonalizing the velocity shear tensor. This is a much stricter version of the converging-flow criterion, though not necessarily always more physical. **(256)** Require that the gas be self-shielding (molecular) and scale the star formation rate to the molecular fraction in the gas cell. **(512)** Scale the star formation efficiency per free-fall time not to a constant (the default behavior), but to a multi-free-fall model (this is generally only relevant in galaxy-scale simulations; see the description in `GALSF_SFR_CRITERION` above). **(1024)** Adds a purely-numerical 'catch' where the more complicated kinematic criteria are ignored if the gas kernel size approaches the minimum force softening, where the forces become non-Keplerian and so some of the criteria above (like the virial criteria) are fundamentally ill-defined. **(2048)** Uses a time-averaged version of the virial criterion (see description above again). If all of the chosen criteria are met, a gas element is immediately converted into a sink. Each of these options involves papers that should be cited: please see the code and documentation for sink modules to cite the appropriate paper[s]. Cite Grudic et al., arXiv:2010.11254, for the methods here.

**SINGLE\_STAR\_ACCRETION**: Enables accretion onto sinks after they form. The value chooses the model, but all are described in the sinks documentation (please read that for details of how the different modules work in detail). Values 0-10 choose 'continuous accretion rate' models, where the accretion rate onto the sink is continuously evaluated from the gas properties of the neighbors through which the sink moves. Within this, values 0-10 specifically are identical to the corresponding value of `SINK_GRAVACCRETION`: 0=estimator from Hopkins and Quataert 2010, for accretion in a gravitationally-unstable disk, 1=revision to 0 from Angles-Alcazar et al 2011, 2=fixed accretion per free-fall time from accretion radius, 3=gravito-turbulent scaling from Gammie et al., 4=fixed accretion per free fall within radius where sink dominates potential, 5=hybrid Bondi plus self-gravitating disk model, 6=fixed inflow per free-fall in collisionless Hernquist sphere, 7=hybrid model interpolating between Shu-type isothermal sphere accretion and Bondi-Hoyle, 8=Hubber-type disk-free-fall interpolation, 9=pure Bondi-Hoyle (ignoring the possibility of gas angular momentum or cooling), 10=pure Bondi (ignoring gas velocity). Setting this flag =11 or =12 uses the `SINK_GRAVCAPTURE_GAS` flag, where accretion is not continuous but discrete, swallowing gas elements which are bound within the neighbor radius and meet some other basic criteria. =11 sets the default behavior for this flag, =12 sets it to behave like the Bate-style sinks (with a fixed, instead of adaptive, accretion radius, and an angular momentum criterion for accretion). Each of these options involves papers that should be cited: please see the code and documentation for sink modules to cite the appropriate paper[s]. Cite Grudic et al., arXiv:2010.11254, for the methods here.
...
#SINGLE_STAR_FB_WINDS           # enable continuous main-sequence mechanical feedback from single stellar sources accounting for OB/AGB/WR winds. with STARFORGE parent flag[s] enabled, this will following STARFORGE methods (Grudic+ arXiv:2010.11254). Otherwise, this will follow a simpler Castor, Abbot, & Klein scaling, for type=4 particles representing single stars, using the standard GALSF_FB_MECHANICAL algorithms in code, for which you should cite Hopkins et al. 2018MNRAS.477.1578H
...
#SINGLE_STAR_FB_SNE             # enable supernovae from single stellar sources at end of main-sequence lifetime. with STARFORGE flag[s] enabled this will use particle spawning in shells following STARFORGE methods (Grudic+ arXiv:2010.11254). Otherwise, this will act uniformly and in a single timestep at the end of the stellar main-sequence lifetime for type=4 particles representing single stars, using the standard GALSF_FB_MECHANICAL algorithms in code, for which you should cite Hopkins et al. 2018MNRAS.477.1578H
...
Again a reminder some of the STARFORGE-specific modules are under-development, though they are all described in the methods paper Grudic et al., arXiv:2010.11254. This should be considered to update and replace the older  `notes_singlestar` file ([here](http://www.tapir.caltech.edu/~phopkins/public/notes_singlestar.pdf)). If you are interested in co-development, please contact Mike Grudic and David Guszejnov.
...
**SINGLE\_STAR\_FB\_RT\_HEATING**: Adds feedback from accreting proto-stars in the form of heating via their accretion luminosity. By default this performs a LEBRON-method radiation hydrodynamic simulation, with the stellar luminosity coming out in the IR. If the radiation hydro module is changed, whatever radiation hydro method is chosen will be used. The accretion luminosity scales with the accretion rate given the radiative efficiency, set by the run-time parameter `SinkRadiativeEfficiency`, unless protostellar evolution is enabled, in which case the appropriate proto-stellar evolution tracks are used to determine the accretion and contraction luminosities (both are then used for feedback). See description of the radiative feedback modules in the sink notes for more details of the numerical methods. Importantly, if you are using the public code, then by enabling explicit radiation-hydrodynamics (also in the public code), this is not needed, and the user can treat full radiative feedback in the public code without needing any proprietary modules here.

**SINGLE\_STAR\_FB\_JETS**: Adds feedback from proto-stellar jets. This acts as as kinematic outflows with a specified outflow rate and velocity set in the same manner as black hole-driven accretion-disk outflows. This leverages the `SINK_WIND_SPAWN` module, see the description in the sink particle notes for the details of the numerical methods. The run-time parameters `Sink_accreted_fraction` or `Sink_f_accretion` (both set the same variable, your choice of the convention to use), `Sink_outflow_velocity` or `Sink_v_outflow`, `Sink_outflow_temperature` or `Cell_Spawn_Temperature`, and `Cell_Spawn_Mass_ratio`, are used, as is standard with this module (see the parameterfile notes below for the sink/sink-particle modules) to determine the mechanical and thermal energy-loadings (as well as e.g. optional magnetic or cosmic-ray loading) for the jets. The jets, by default, are collimated with a very small opening angle at launch (they almost immediately will expand as they propagate), along the direction of the sink accreted specific angular momentum. Cite Su et al, arXiv:2102.02206, and Grudic et al., arXiv:2010.11254, for the methods here.

**SINGLE\_STAR\_FB\_WINDS**: Adds continuous main-sequence mechanical feedback from single stellar sources accounting for OB/AGB/WR winds. This adds the parameter `Cell_Spawn_Mass_ratio_MS` which can be set for the cell-spawning for this mode of feedback analogous to `Cell_Spawn_Mass_ratio`, but allowing you to specify these separately. This has two possible algorithmic channels it can adopt, depending on other flags that are set. With STARFORGE parent flag[s] enabled, this will following STARFORGE methods (Grudic+ arXiv:2010.11254). Otherwise, this will follow a simpler Castor, Abbot, & Klein scaling, for type=4 particles representing single stars, using the standard `GALSF_FB_MECHANICAL` algorithms in code, for which you should cite Hopkins et al. 2018MNRAS.477.1578H. Users interested in modifying the relevant scalings or active particle types should modify the code appropriately in the `stellar_evolution.c` file.

**SINGLE\_STAR\_WIND\_MODE**: Pins the channel through which jets and winds are injected, instead of letting the code select it by comparing momentum rates. Set =1 to force winds to spawn as discrete cells, or =2 to force them to be injected continuously. This exists to exercise one injection path on its own; the automatic comparison is what you want otherwise.

**SINK\_SPAWN\_MERGE\_WHEN\_AMBIENT**: Retires a spawned jet or wind cell only once it is both subsonic and thermally equilibrated with the non-spawned gas around it, rather than merely subsonic with respect to the cell it is being merged into. The distinction matters because a spawned cell can be subsonic relative to its merge target while remaining far hotter than the medium, in which case retiring it deposits that thermal energy somewhere it does not belong. This is set automatically with the STARFORGE protostellar model.

**SINK\_SPAWN\_MERGE\_MAX\_THERMAL\_CONTRAST**: Sets how nearly a spawned cell must match the ambient medium before the criterion above will retire it, as the largest allowed $|\ln{(u_{\rm cell}/u_{\rm ambient})}|$. The default is 1.0.

**SINK\_SPAWN\_MERGE\_ANY\_NEIGHBOR**: Opts out of `SINK_SPAWN_MERGE_WHEN_AMBIENT`, restoring the older behavior in which a spawned cell is retired as soon as it is subsonic with respect to its merge target, whatever its temperature relative to the surrounding gas.

**MERGE\_SPLIT\_LIMIT\_KINETIC\_DISSIPATION**: Refuses to retire a spawned cell into a target if doing so would thermalize the majority of the energy involved. Merging two cells carrying a large relative velocity converts their relative kinetic energy into heat in a single step, which is an unresolved shock rather than a physical one, and this prevents it. Set automatically with the STARFORGE protostellar model.

**MERGE\_SPLIT\_MAX\_KINETIC\_DISSIPATION\_FRACTION**: Sets the fraction permitted by the criterion above, as $KE_{\rm com}/(KE_{\rm com}+E_{\rm light})$. Lower values are stricter; roughly 0.34 corresponds to Mach 1 on the lighter of the two cells. The default is 0.5.

**MERGE\_SPLIT\_ALLOW\_KINETIC\_DISSIPATION**: Opts out of `MERGE_SPLIT_LIMIT_KINETIC_DISSIPATION`, permitting merges however much kinetic energy they would thermalize.

**SINGLE\_STAR\_FB\_SNE**: Adds supernovae from single stellar sources at end of main-sequence lifetime. Again this has two possible behaviors, depending on the other flags that are set. With STARFORGE flag[s] enabled this will use particle spawning in shells following STARFORGE methods (Grudic+ arXiv:2010.11254). Otherwise, this will act uniformly and in a single timestep at the end of the stellar main-sequence lifetime for type=4 particles representing single stars, using the standard `GALSF_FB_MECHANICAL` algorithms in code, for which you should cite Hopkins et al. 2018MNRAS.477.1578H. Users interested in modifying the relevant scalings or active particle types should modify the code appropriately in the `stellar_evolution.c` file.
...
There are a variety of flags for additional special behaviors. Here are several most commonly used for the types of simulations above. However users should also carefully read the section on super-massive sink particles below, as almost all of those flags are cross-compatible here: in fact the modules here and there are often the same, simply with different operator logic for the sake of user convenience.
...
## ----------------------------------------------------------------------------------------------------
#SINK_RETURN_ANGMOM_TO_GAS        # BH/sink particles return accreted angular momentum to surrounding gas (per Hubber+13) to represent AM transfer (loss in accreting material)
...
**SINGLE\_STAR\_FB\_RT\_HEATING**: Adds proto-stellar heating with a simple non-explicit-RHD approximation, where the luminosity determined by SinkRadiativeEfficiency (typical might be 5e-7 or so). This particular module used without radiation-hydrodynamics uses FIRE modules, so permissions follow those. But by enabling explicit radiation-hydrodynamics, this is not needed, and the user can treat full radiative feedback in the public code.
...
All seed models require certain parameters in the parameterfile: (1) `SeedSinkMass` = the actual seed BH mass, in code units. (2) `SeedSinkMinRedshift` = the minimum redshift where seeding is allowed -- at redshifts below this, no seeds will be created. (3) `SeedSinkMassSigma` = logarithmic dispersion in BH mass, in dex -- BHs will be spawned with a random mass drawn from a lognormal with this width.
...
#SINK_REPOSITION_ON_POTMIN         # reposition sink particle on potential minimum (requires EVALPOTENTIAL). moves smoothly with damped velocity to most-bound particle in kernel. Cite Wellons et al. arXiv:2203.06201
####################################################################################################
```

**SINK\_DYNFRICTION\_FROMTREE**: Compute dynamical friction forces on BH following the discrete dynamical friction estimator in Linhao Ma et al., arXiv:2101.02727 and L. Ma et al., arXiv:2208.12275. This is a more flexible, general, and less noisy and more accurate version of the traditional Chandrasekhar dynamical friction formula, and is recommended over that implementation for any simulations which wish to track the un-resolved dynamical friction forces in such a manner. Care is needed to avoid double-counting in the high-resolution limit. While in the public code, users are encouraged to contact Linhao Ma to discuss use, if desired, since some testing is still ongoing for certain applications. Please cite both Ma et al. papers if using this flag.

**SINK\_REPOSITION\_ON\_POTMIN**: This is the most extreme option, and it simply anchors the BH particle to the potential minimum (it will re-position every timestep to the location of the potential minimum within the kernel). This uses the modern smoother version, obtained in older code versions by setting it equal to =2, which finds the most deeply-bound particle (not just potential  minimum), and moves continuously with a velocity-damping term, all of which combines to give the most stable centering. The options are explained in more detail in the `notes_blackholes` file. The new option is detailed in Wellons+Angles-Alcazar+PFH et al., arXiv:2203.06201, which should be cited for its use.


<a name="config-bh-accretion"></a>
### _BH Accretion Models_ 

sink particles accrete, but this accretion is generally taking place at scales (the Schwartzschild radius) well below the resolution of the simulations. Therefore different sub-grid models are present to mimic accretion both into an un-resolved accretion disk around the BH, and from that onto the BH itself. All of the accretion models below should enable the SINK\_SWALLOWGAS flag.

The accretion models involve several parameterfile settings. `SinkAccretionFactor` arbitrarily multiplies the calculated accretion rate by a constant factor (so its default value is =1). `SinkEddingtonFactor` caps the maximum accretion rate at this multiple of the Eddington limit (so Eddington-limited accretion sets this =1, un-limited accretion sets this to some huge value). `SinkNgbFactor` multiplies the standard kernel neighbor number -- it is recommended this value be set to a few, since some of these estimators rely on things like bulge-disk decomposition inside the kernel (which requires a decent number of elements). `SinkMaxAccretionRadius` sets the maximum radius (in code units) out to which the BH will search for neighbors to achieve this neighbor number (so that it doesnt go indefinitely far, if there is little gas within the radius around the BH). `SinkRadiativeEfficiency` sets the radiative efficiency, important for both feedback and for determining what fraction of mass actually ends up in the BH versus is lost to radiation.
...
**SINK\_SWALLOWGAS**: Allow the BH to actually consume gas particles. Required for any accretion model. Unless set otherwise by a specific compile flag, the accretion is stochastic, as in Springel et al. 2005 MNRAS 361 776 and Di Matteo et al. 2005 Nature 433 604 (see those papers for more details). The "sink" (or "black hole/star/central object") mass is separately tracked from the particle mass (according to whatever accretion model). If the sink mass becomes larger than the particle mass (possible for some choices of parameters below), then nearby neighbor particles are immediately "swallowed" with some probability until the mass is balanced. This is done so that total particle mass in the simulation is properly conserved, without generating arbitrarily large mis-matches in the masses of different particle types.

**SINK\_ALPHADISK\_ACCRETION**: Enables a sub-grid model for an "alpha-disk" (a traditional Shakura & Sunyaev viscous disk) around the BH particle. If this is off, accretion into the BH will be instantaneous (whenever the large-scale accretion model determines there is accretion). If this is on, mass is accreted into the viscous disk, then accreted continuously out of that disk onto the BH at a rate following Shakura & Sunyaev (crudely this approaches Eddington when the mass in the disk approaches the pre-existing BH mass); both the BH mass and the viscous disk mass are separately and continuously followed. Note that although BH growth may be strictly limited at Eddington, accretion into the outer accretion disk is not. This smooths out the actual BH accretion rate over the viscous timescale. If you use this module, cite the GIZMO methods paper for now, since this has not been actually published.

**SINK\_SUBGRIDBHVARIABILITY**: This enables a model for AGN variability on very small timescales, corresponding to internal variability in e.g. the un-resolved accretion disk. This is 'smoothed over' by finite resolution; however, we can crudely approximate it by including an explicit power-spectrum of fluctuations in dM/dt, integrated from frequencies of infinity down to 1/dt where dt is the simulation timestep (typically ~100 yr in the galaxy centers). We do this following Hopkins & Quataert 2011: assuming fluctuations in ln(dM/dt) follow a Gaussian random walk with equal power per logarithmic time interval from tmin (the orbital time at the innermost stable circular orbit for a non-rotating BH) to tmax (the dynamical time at the resolved BH kernel/accretion radius). If you use this module, cite Hopkins & Quataert 2011, MNRAS, 415, 1027 (the prescription is presented in their footnote 5). `GALSF` must also be enabled.

**SINK\_GRAVCAPTURE\_GAS**/**SINK\_GRAVCAPTURE\_NONGAS**: `SINK_GRAVCAPTURE_GAS` enables gas accretion via explicit resolution of direct gravitational capture of particles: if gas particles are located within the accretion radius (the minimum resolved softening around the BH), then this uses their position and relative velocity with respect to the BH particle we check whether (a) it is gravitationally bound to the BH, and (b) whether the apo-centric radius of the particle about the BH is also within the accretion radius. If both are true, we consider the particle "captured" and immediately add its mass to the BH accretion disk (if `SINK_ALPHADISK_ACCRETION` is enabled, otherwise it is added directly to the BH). Use this only if you want to force the code to resolve accretion into the viscous disk (i.e. this is for very high-res simulations). Otherwise de-activate this and use a "sub-grid" accretion rate estimator. `SINK_GRAVCAPTURE_NONGAS` does the same, for non-gas particle types (stars/dark matter; note that BH-BH mergers are always allowed, and handled separately from these flags). If this is on, dark matter is always added directly to the BH, while stars add to the accretion disk if `SINK_ALPHADISK_ACCRETION` is on, otherwise to the BH (but does not contribute towards feedback or the Eddington limit, i.e. acting as if it is not tidally disrupted). Note that `SINK_GRAVCAPTURE_NONGAS` can be turned on with other models (below) for gas accretion. If you use this module, cite Hopkins et al., 2016, MNRAS, 458, 816 (where this was developed and studied in detail).

**SINK\_GRAVACCRETION**: Enables the "unresolved limit" accretion rate estimator based on gravitational torques from Hopkins & Quataert 2011, MNRAS, 415, 1027. This is a general family of accretion rate estimators based on the assumption that the rate-limiting step for accretion is angular momentum transport (as opposed to the traditional Bondi-Hoyle-type estimators, which do not include any accounting for angular momentum and/or cooling). The fundamental assumptions of the default models here (values =0/1) are tested and designed to reproduce the accretion rates through annuli ranging from accretion disk scales <~0.01 pc through galactic ~1000 kpc scales, with a primary focus on scales ~1-100 pc. The dominant mechanism of angular momentum transfer on larger scales are torques due to inhomogeneities (e.g. due to instabilities, clumps, etc) in a gas+stellar disk. See that paper for details. In general this accretion rate estimator can be more accurate by a factor of $\sim 10^{4} - 10^{8}$ compared to e.g. the Bondi estimators below (on scales outside the inner accretion disk, which this is not designed to reproduce). Quantities such as the gas angular momentum around the sink are calculated on the fly and used in the accretion rate estimation. Two variants here (setting =0 or =1) use two slightly different estimators to calculate the angular momentum and disk-mass-fraction around the sink (which enter the accretion rate determination), and evaluate at either the adaptive kernel radius or a fixed physical radius, as stated. For [=2] the scaling is reduced to simply assume a constant efficiency per dynamical time. For [=3], this uses the simpler classic gravito-turbulent scaling for the properties of the outer disk, which is reasonable for some circumstellar disks and can apply if the medium is gas-dominated in an external potential, thin, has toomre Q above unity, and can be approximated by a beta-cooling model without cooling too fast. For [=4], the fixed efficiency is again assumed but now set to fixed at the sink radius of influenced defined in the usual fashion from the surrounding material kinematics. For [=5] a hybrid model is employed, where the circularization radius of gas is explicitly calculated based on its angular momentum content, and if this is sufficiently low, where the Bondi-Hoyle limit could actually apply, the Bondi formula is used. [=6] and [=7] employ scalings which interpolate between fixed accretion per dynamical time for bound gas, and Bondi-Hoyle, appropriately for a density profile scaling as $\rho\propto r^{-1}$ or $r^{-2}$, respectively, in the self-gravitating component. [=8] Uses the estimator from Hubber et al. 2013 for a toy alpha-disk model which is regulated by bondi-accretion, valid if the bondi limit applies to large radii and the disk is a thermal-pressure-dominated alpha disk in steady state with an approximately constant thermal scale-height-to-radius ratio throughout. Values [=9,10,11] adopt the classic Bondi-Hoyle formula, i.e. neglect angular momentum and assume spherical accretion and no cooling. Note that a large number of studies have shown this is not a good approximation to periods of high sink or stellar accretion, since it assumes the gas has no angular momentum (when, in fact, understanding gas accretion onto sinks from large scales is primarily an angular momentum problem). Contrary to some claims in the literature, there is no Bondi-Hoyle formula that accounts for angular momentum -- the actual scalings in the angular-momentum dominated regime resemble the gravito-turbulent and gravitational-torque accretion models discussed above, which have qualitatively different dimensional scalings (nearly independent of sink mass and sound speed, for example, where Bondi-Hoyle depends strongly on both of these). Still, the Bondi-Hoyle limit is potentially relevant for either situations (1) where the sink is accreting smoothly from a hot, hydrostatic, pressure-supported atmosphere, or (2) where a 'seed' sink is moving through the ISM (on scales where it does not strongly influence the potential), closer to the regime the Bondi-Hoyle accretion theory was designed to represent. The `notes_blackholes` file has additional details of the scalings adopted. The [=9] implementation is standard Bondi-Hoyle scaling, accounting for sink-gas relative velocities (the denominator is $(c_{s}^2 + |{\bf v}_{\rm gas}-{\bf v}_{\rm BH}|^2)^{3/2}$). Set [=10] to ignore the relative velocity and use just the sound speed. Set (=11) to slightly modify the pre-factor in front of the Bondi rate (giving it a power-law density dependence), which will exactly Booth & Schaye 2009, and subsequent papers (OWLS, EAGLE, etc.) -- that particular sub-version requires `GALSF` also be set.

**SINK\_GRAVACCRETION\_STELLARFBCORR**: Multiple the accretion rate by the appropriate function to account for additional acceleration-dependent retention from stellar feedback in Mdot. This is relevant in simulations where the accretion rate onto the BH is estimated from scales which are still star-forming (larger than 0.01 pc or so, for super-massive black holes). Stellar feedback will expel some of that material, suppressing the accretion rate relative to a model neglecting stellar feedback, in a way that depends on the acceleration scale. If you are using this flag, cite Hopkins et al., arXiv:2103.10444, for both the analytic derivation of these scalings and the numerical methods/implementation.
...
#SINK_THERMALFEEDBACK             # constant fraction of luminosity coupled in kernel around BH. cite Springel, Di Matteo, and Hernquist, 2005, MNRAS, 361, 776
# -- mechanical (wind from accretion disk/BH with specified mass/momentum/energy-loading relative to accretion rate)
#SINK_WIND_KICK=1                 # gas in kernel given stochastic 'kicks' at fixed velocity. (>0=isotropic, <0=collimated, absolute value sets momentum-loading in L/c units). cite Angles-Alcazar et al., 2017, MNRAS, 464, 2840
#SINK_WIND_SPAWN=2                # spawn virtual 'wind' particles to carry BH winds out. value=min number spawned per spawn-step. Cite Torrey et al 2020MNRAS.497.5292T, Su et al., arXiv:2102.02206, and Grudic et al. arXiv:2010.11254 for use and numerical methods and tests
...
#COOL_MOLECFRAC=1               # choose how to track molecular H2 fraction (1-6, see docs for details) 
##---------------------------------------
```
...
**CHIMES\_STELLAR\_FLUXES**: Tracks stellar fluxes in different age bins through the luminosity tree, using the LEBRON method from FIRE, and couples these to the CHIMES chemistry routines. Note that this should be used instead of FIRE's standard UV heating routine, if those options are desired with CHIMES.
...
#RT_FLUXLIMITEDDIFFUSION                # RT solved using the moments-based 0th-order flux-limited diffusion approximation (constant, always-isotropic Eddington tensor; fastest, but cannot capture shadows or beaming). cite Hopkins & Grudic, 2018, arXiv:1803.07573
#RT_M1                                  # RT solved using the moments-based 1st-order M1 approximation (solve fluxes and tensors with M1 closure; captures shadows and beaming; more expensive than FLD; currently only compatible with explicit diffusion solver). cite Hopkins & Grudic, 2018, arXiv:1803.07573
#RT_OTVET                               # RT solved using the moments-based 0th-order OTVET approximation (optically thin Eddington tensor, interpolated to thick limit; intermediate between FLD and M1; good for problems dominated by a few point sources). cite Hopkins & Grudic, 2018, arXiv:1803.07573
...
**DOMAIN_SEGMENTS_SCALE**: The domain decomposition hands each MPI task its share of the simulation volume as several separate contiguous pieces of the Peano-Hilbert curve, rather than one. Having more pieces makes the decomposition far less dependent on spatial co-location and lets it balance the work much more finely, at the cost of more top-level tree refinement, more communication (each piece exchanges its own pseudo-particle data), and a more spatially fragmented territory per task. The code chooses the number of pieces automatically from the particle count and the number of tasks, aiming for roughly ten thousand particles per piece. It is set when the initial conditions are read and revisited at each full domain decomposition, so a run whose particle count grows keeps a granularity matched to its load; the value is rounded to a power of two, so in practice it only moves when the load genuinely doubles. That target is measured to be about where the balancing gain stops paying for those costs, and it holds across problems spanning more than an order of magnitude in size. This setting multiplies that automatic choice, so the default of 1 leaves it alone; the result is rounded to a power of two and capped, so raising it has no further effect once the cap is reached. Raise it for problems whose cost is dominated by load imbalance; lower it for problems that are memory- or communication-limited. Note the number of simultaneous communication requests is the task count times the number of pieces per task, which is a common source of trouble if it is pushed very high.

**DOMAIN_TOPTREE_REFINEMENT_SCALE**: Multiplies how finely the top-level tree is refined, relative to the number of domain pieces that refinement has to fill. The default of 1 asks for one leaf per piece; refinement splits eight ways at a time, so what is achieved overshoots what is asked for, and both numbers are reported at each decomposition. Values below 1 are clamped, because a top tree with fewer leaves than pieces cannot be assigned at all. This has a much weaker effect on run time than DOMAIN_SEGMENTS_SCALE, and is rarely worth changing.

**DOMAIN\_TIMEBINS**: Chooses how the cost of each domain is weighted when the decomposition balances the load. Set =0 to weight by how frequently each element is active, or =1 for full per-timebin balancing following the Gadget-4 scheme. Omit the flag entirely to leave the costs unweighted.

**DOMAIN\_NO\_LIGHTWEIGHT\_REPARTITION**: Forces a full domain decomposition every time one is triggered. By default the code will instead re-balance the load while re-using the existing top-level tree, whenever a full decomposition was not actually required. That is much cheaper, and on large runs -- most of all on GPUs, where a full decomposition is disproportionately expensive -- the difference is substantial. Set this only if a run genuinely needs the top-level tree rebuilt every time.

**GPU\_PARTICLE\_STORAGE\_PLACEMENT**: Overrides where the particle storage prefers to live on an AMD/HIP GPU. These arrays are demand-paged between host and device, so each page ends up wherever it was last touched, and by default the code chooses a placement from the per-rank size of that storage: above roughly a gigabyte it asks for the pages to prefer host memory with the device mapped in as an accessor, and below it leaves them to settle wherever they are used. The size dependence is the point -- on a small problem the repeated device walks give each page enough re-use that it settles in device memory and stays there, while on a large one nothing gets that re-use, host and device each sweep the whole array every cycle, and the migration never converges. The difference is worth a factor of several on the largest runs, and a few percent the other way on small ones. Set =1 to force the host-preferred placement or =0 to force the default, for instance to measure the crossover on a problem near it. This has no effect on NVIDIA GPUs, where the equivalent advice has not been tested, or on CPU-only builds.
...
**OUTPUT\_ADDITIONAL\_RUNINFO**: This enables various outputs that are disabled by default. This includes outputs like 'cpu.txt' every timestep instead of only on the super-steps, far more output in the stdout outputs, checking for 'stop' files for checkpointing every timestep, outputing the extended `sink_details.txt` files, etc. On machines with slow I/O, this can be a major slow-down and bottleneck for the code. On machines with fast I/O, this is much less of an issue. It depends strongly on the run. But most of the extra output is only needed for de-bugging.
...
**OUTPUT\_SINK\_DISTANCES**: Saves the distance to the nearest sink, if `SINK_CALC_DISTANCES` is enabled, to snapshots.
...
**INPUT\_READ\_KERNELRADIUS**: Read the initial guess for KernelRadius from the ICs file. In any case the density routine must be called to determine the "correct" KernelRadius, but this can be useful if the ICs are irregular so the "guess" the code would normally use to begin the iteration process might be problematic. In general though it is redundant.

**INPUT\_READ\_SINKPROPS**: Forces the sink properties -- sink radius, ZAMS mass, luminosity, age and the rest -- to be read from the initial conditions file, for an IC which contains sink particles and was built for use with the single-star modules. Without this those properties are initialized by the code rather than taken from the file.
...
#TREE_QUERY_PACKET_SIZE=8       # walk the gravity tree for this many adjacent active elements at once on the host: they share one traversal while every element keeps its own opening decisions and evaluates its own accepted elements, so forces are unchanged and the traversal cost of a deep tree is divided among them. Default 8; problem-dependent, worth tuning if you care about speed; 1 walks every element alone.
...
# --------------------
# ----- sink particle/black hole/star special options
...
#SINK_RIAF_SUBEDDINGTON_MODEL       # enable an arbitrary modular variation in the radiative efficiency of BHs as a function of eddington ratio or other particle properties
...
# --------------------
# ----- Load-Balancing
#ALLOW_IMBALANCED_GASPARTICLELOAD # retired and has no effect: the gas-cell capacity always follows the particle capacity, so the load-balancing freedom this used to enable is unconditional. A run with no gas allocates no gas-cell storage at all.
####################################################################################################
```
...
**CONTINUE\_BELOW\_MINTIMESTEP**: Opts out of the above. `STOP_WHEN_BELOW_MINTIMESTEP` is on by default, because a run whose timestep has collapsed to the floor has almost always gone unstable, and allowing it to continue will burn an entire allocation making no progress. Setting this instead clamps the stepsize at `MinSizeTimestep` and carries on. Use it only for a problem where reaching the floor is both expected and survivable -- if you are not certain that it is, you want the default.
...
**WAKEUP**: Sets the timestep-limiter factor: a cell is 'woken' onto a shorter timestep, out of turn, when a neighbor's timestep is shorter than its own by more than this factor. This is the limiter of Saitoh & Makino 2009, and it is what stops a cold cell sitting on a long timestep from being run over by a blastwave it never saw coming. Smaller values are more accurate and more expensive -- their Table 1 gives a smaller energy error for f=2 than f=4 on the Sedov problem, at roughly twice the cost. If you do not set this explicitly the code follows `SLOPE_LIMITER_TOLERANCE`, taking 4.1 when that is positive and 2.1 otherwise.
...
**HYDRO\_VOLUME\_CORRECTIONS**: Applies a higher-order partition-of-unity volume correction to the meshless cell volumes via a dedicated extra neighbor pass (the `cellcorrections` Spec). Unlike `HYDRO_PARTITION_UNITY_IMPROVE_FD` (which corrects only the leading $\nabla H$ term, accumulated inline with the density loop), this pass evaluates a second-order quadrature correction over the full neighbor stencil. More accurate than the FD-only correction at the cost of an extra neighbor pass per (active) timestep; the two are complementary and can in principle be combined. Like `HYDRO_PARTITION_UNITY_IMPROVE_FD`, only meaningful for meshless (MFM/MFV) methods.

**HYDRO\_PARTITION\_UNITY\_IMPROVE\_FD**: Applies the first-derivative (FD) partition-of-unity volume correction from Massaro Acha, Alonso Asensio & Dalla Vecchia 2026, which corrects the meshless cell-volume estimates for the spatial gradient of the kernel support size $H({\bf x})$. The default meshless volume partition $V_{i} = 1 / \sum_{j} W(|{\bf x}_{i} - {\bf x}_{j}|, H_{i})$ is exact only in the limit of constant $H$; the FD correction subtracts the leading $\nabla H$ term, $V_{i} \to V_{i} (1 + \alpha_{\zeta} {\bf \nabla} H \cdot \langle {\bf x}_{j} - {\bf x}_{i}\rangle / H + \ldots)$, with kernel-dependent coefficients $\alpha_{\zeta}$ tabulated in their Table 1 (defined as $\alpha_{\zeta} = -(2\pi/3) \int_{0}^{1} \xi^{5} (d\zeta/d\xi)\,d\xi$ where $\zeta$ is the kernel function in their notation). This typically improves cubature-rule accuracy by a factor $\sim 3$--$4\times$ at essentially zero extra cost, since the required gradient $\nabla H$ is accumulated as part of the existing density neighbor loop (no separate neighbor pass). Only relevant for meshless (MFM/MFV) methods; ignored under SPH and regular-grid modes. Complementary to (but distinct from) `HYDRO_VOLUME_CORRECTIONS`, which uses a separate neighbor loop for a higher-order quadrature correction. Please cite Massaro Acha, Alonso Asensio & Dalla Vecchia 2026 if you use this module.
...
**TURB\_DRIVING\_UPDATE\_FORCE\_ON\_TURBUPDATE**: Updates the turbulent driving force only as often as the driving phases themselves are recomputed, at the interval set by `TurbDrive_TimeBetweenTurbUpdates`, rather than every timestep. Enable this only as an optimization, when evaluating the turbulent force is itself expensive: to avoid large errors `TurbDrive_TimeBetweenTurbUpdates` must then be set by hand to be much smaller than $\lambda_{\rm min}/V$, where $V$ is the typical turbulent velocity and $\lambda_{\rm min}$ the smallest driven wavelength.
...
**TREE\_LEAF\_BUCKET\_SIZE**: This sets the number of elements at which the code stops subdividing a tree node, so that a 'leaf' of the tree can hold several elements which the tree walk then evaluates directly, instead of splitting every branch all the way down to one element per leaf. Larger values give a smaller and shallower tree: cheaper to build, cheaper to store, cheaper to communicate between processors, and much less vulnerable to the pathologically deep branches that nearly-coincident elements would otherwise force. The trade-off is that more of the gravity and neighbor work is then done as direct element-element interactions, instead of being absorbed into the multipole moments of nodes. Note this does not reduce the force accuracy -- if anything slightly the reverse, since a node the walk would have accepted as a multipole is now opened and summed exactly -- the cost is purely in the number of interactions evaluated. Where the optimum lies is problem-dependent: strongly clustered, high-dynamic-range problems (deep trees, large numbers of nearly-empty nodes) favor larger values, while problems whose trees are already shallow gain little and can lose slightly. It is worth tuning if you care about performance. Very large values are limited in practice not by the extra interactions but by memory, since the portions of the tree exported to other processors grow as leaves are sent element-by-element. Setting this to 1 recovers exactly the historical behavior of one element per leaf.
...
**SINGLE\_STAR\_DIRECT\_GRAVITY**: Computes star-star (type 5) gravity exactly at -all- separations, by brute-force summation over a table of every star broadcast to every task, with the tree dropping star mass for star targets so that nothing is counted twice. Unlike the `SINGLE_STAR_DIRECT_GRAVITY_RADIUS` option, there is no cutoff, so the tree never approximates one star's pull on another at any distance. This costs $O(N_{\rm star}^{2})$ per gravity step and stores $O(N_{\rm star})$ per task, so it is intended for runs with a modest number of stars. It is incompatible with `SINGLE_STAR_TIMESTEPPING`>0, whose binary search reads exactly the star-star tree interactions that this removes.
...
**MHD\_CONSERVE\_B\_ON\_REFINEMENT**: After a refinement or de-refinement operation, redefines ${\bf B}$ following the density step so that ${\bf B}$ itself is exactly conserved across the operation. The default instead conserves the code's volume-integrated quantity ${\bf VB}$ between the steps, which is the natural conserved variable for the transport but does not hold the field strength itself fixed when the cell volume changes.
...
**RT\_REINJECT\_ACCRETED\_PHOTONS**: When sink particles are used, photons which would otherwise be lost with a gas cell when it is accreted are instead re-injected into the lowest-energy frequency bin at the next photon injection from that sink. Without this the radiation carried by accreted cells simply disappears, which is a small but systematic photon sink in runs where accretion is rapid.

<a name="config-debug-sinks"></a>
### _Black Hole/Sink Particle Special Options_ 

**SINK\_WIND\_SPAWN\_SET\_BFIELD\_POLTOR**: Set the poloridal and toroidal magnetic field for spawned jet cells (should work for all cell spawning). Cite Su et al., arXiv:2102.02206, for methods.

**SINK\_WIND\_SPAWN\_SET\_JET\_PRECESSION**: Manually set jet precession in parameter file (does not work for cosmological simulations). Will make the jet direction precess according to a user-specified precession angle and period. Cite Su et al., arXiv:2102.02206, for methods.

**SINK\_RIAF\_SUBEDDINGTON\_MODEL**: Enable an arbitrary modular variation in the radiative efficiency of BHs as a function of eddington ratio or other particle properties. This is determined by the function `evaluate_sink_radiative_efficiency` in `sink_util.c`, which the user should set to scale according to the desired model.

**SINK\_CR\_SUBEDDINGTON\_MODEL**: Enable an arbitrary modular variation in the CR acceleration efficiency from BHs as a function of mass, eddington ratio, spin, or other particle properties. This is determined by the function `evaluate_sink_cosmicray_efficiency` in `sink_util.c`, which the user should set to scale according to the desired model.
...
**SINGLE\_STAR\_AND\_SSP\_HYBRID\_MODEL\_DEFAULTS**: Sets a bundle of defaults for the hybrid model above, taken from the zoom-in experiments around special particles, so that such a run can be configured from a single flag; the value is passed through as `SINGLE_STAR_AND_SSP_HYBRID_MODEL` itself, i.e. the mass resolution below which the single-star routines are used. Beyond that it enables `SINGLE_STAR_STARFORGE_DEFAULTS` and `GALSF_SFR_IMF_SAMPLING` (so the intermediate-mass regime is at least approximately handled), turns on `COOLING`, `MAGNETIC`, `CONDUCTION` and `CONDUCTION_SPITZER`, `VISCOSITY` and `VISCOSITY_BRAGINSKII`, enables protostellar jets, continuous winds, SNe and radiative feedback (`SINGLE_STAR_FB_JETS`, `SINGLE_STAR_FB_WINDS`, `SINGLE_STAR_FB_SNE`, `SINGLE_STAR_FB_RAD`), and configures the transport with `RT_COMOVING` and `RT_SOURCES=(16+32)` so that both single-star and stellar-population particles emit, a reduced speed of light of 0.1 unless one is set explicitly, and `ADAPTIVE_TREEFORCE_UPDATE=0.0625`. Set the individual flags yourself if you want any combination other than this one.
...
As a result of this, a 'minimal' parameterfile can be as short as the following:
    ...
    PartAllocFactor             2           % max particle-load imbalance
    ...
    DomainBuild_ActiveFraction  0.01        % fraction active for decomposition
    TreeRebuild_ActiveFraction  0.01        % fraction active for tree build
...
**UnitLength\_in\_cm** (or `UnitLength_in_centimeters`): This sets the internal length unit in centimeters. For cosmological simulations, the convention is to express these in "little-h" units (i.e. the value you set corresponds to cm$/h$, where $H_{0} = 100\,h$ km/s/Mpc, so that the code's internal representation is $h$-independent). The example value 3.085678e21 sets the length unit to 1.0 kpc$/h$ (i.e. 1 kpc when $h=1$). For non-cosmological simulations the $h$ factor is irrelevant (set `HubbleParam=1`) and this is simply the length unit in cm. (Default: 1, only for simulations which do not need any dimensions).

**UnitMass\_in\_g** (or `UnitMass_in_grams`): This sets the internal mass unit in grams (with the same little-$h$ convention as above for cosmological runs). The example value 1.989e43 sets the mass unit to $10^{10}\,M_{\odot}\,h^{-1}$. (Default: 1, only for simulations which do not need any dimensions).
...
### _Memory & Load-Balancing (Domain) Parameters_ 
    ...
    %----- Memory allocation
    PartAllocFactor   2.0   % particle-load imbalance the decomposition may reach

**WorkingMemoryPoolSize** (or `Working_Mem_Pool_Per_Task_in_MB`): Sets the size, in megabytes per MPI task, of the working memory pool: a single block reserved at start-up that the code hands short-lived allocations out of and returns them to in reverse order. Reserving it once avoids repeated large allocations during the run, and it is accounted, so exhausting it produces a message naming what asked for the memory rather than a crash. It is **not** a limit on the task's total memory: the particle data, the gravity tree, the buffers used to exchange data between tasks, and any memory used on an accelerator are allocated separately and are not counted against it. **You should not normally set this.** The code works out a size from the things that actually draw on the pool, once it knows the particle count, and prints the size it chose. Set it only to override that deliberately -- for example if a run stops with a message naming this parameter, which means something needed more of the pool than the code predicted. (Older parameter files called this `MaxMemSize`; that name is no longer recognised, and the code says so at start-up.)

**PartAllocFactor** (or `ParticleNumberMemoryImbalance_Limit`): The largest particle-load imbalance the domain decomposition is allowed to produce, as a multiple of the balanced count: a task may be assigned up to `PartAllocFactor` times `TotNumPart/NTask` particles. Some imbalance has to be allowed, because insisting on an equal particle count per task would force a poor *work*-load balance. Values below 1 leave the decomposition with no solution and the run will stop. **Bigger is not better.** This is a ceiling on what a task may be *assigned*; it is not a memory reserve, and room for particles imported from neighbouring tasks is no longer taken out of it -- that is measured per task from the imports that actually occur, and added on top. Raising it therefore buys no room for imported data, while it does inflate the gravity tree's node arrays and the decomposition's own limits, so a large value costs memory and gains nothing. Values a little above 1 are usually right; the default is 2. The case for setting it higher is a problem whose particle count grows substantially as it runs, through cell splitting or particle spawning.

**CommChunkSize** (or `MPI_Comm_Chunk_Size_in_MB`): The size, in megabytes, of the chunk the code sends particle data between tasks in -- when writing and reading snapshot and initial-conditions field blocks, in the general-purpose all-to-all exchange, and in the neighbour loops' communication rounds. A larger chunk means fewer, larger rounds; a smaller one means more, smaller rounds. It trades communication rounds against transient memory, and cannot by itself cause a failure. **You should not normally set this**: the code works out a size from what a round has to hold. (Older parameter files called this `BufferSize`; that name is no longer recognised, and the code says so at start-up.)

    %---- Rebuild domains, and the gravity tree, when >this fraction of particles active
    DomainBuild_ActiveFraction   0.005	% 0.0005-0.05, dept on core+particle number
    TreeRebuild_ActiveFraction   0.005	% defaults to DomainBuild_ActiveFraction

**DomainBuild\_ActiveFraction** (or `TreeDomainUpdateFrequency`): Neither the domain decomposition nor a full tree construction is carried out every single timestep; instead, properties of tree nodes can be dynamically updated as needed. This can improve performance a great deal when many timesteps advance only a very small fraction of particles (below a percent or so). This parameter says how often the particles are redistributed between tasks: a value of 0.05, for example, decomposes on any step where at least $0.05\,N$ particles are active, where N is the total particle number. A decomposition always builds a new tree with it. If you find that imbalance (bad load-balancing) dominates runtime (see the cpu.txt file), try decreasing the value here: values from 0.0005-0.05 can work well. (Default: 0.05, but we stress this is rather ad-hoc and users should experiment with the choice.)

**TreeRebuild\_ActiveFraction**: The same thing for the gravity tree alone -- the tree is rebuilt from scratch on any step where at least this fraction of the system is active, keeping the existing decomposition. A value of zero rebuilds the tree every timestep. Redistributing particles is a cost and load-balancing question, while how long a tree may be reused before its geometry stops describing where the particles actually are is an accuracy one, which is why the two are set separately: lower this one to buy accuracy without paying for a decomposition you have no other use for. Because a decomposition always brings a new tree with it, setting this **above** `DomainBuild_ActiveFraction` has no effect. (Default: whatever `DomainBuild_ActiveFraction` ends up being, which reproduces the older behaviour where a single parameter controlled both. A parameterfile written for the older code, which set only `TreeDomainUpdateFrequency`, therefore behaves exactly as it did before.)
...
**MinGasTemp** (or `Minimum_Gas_Temperature`): The minimum allowed gas temperature in Kelvin (this is converted by the code to a minimum thermal energy per unit mass assuming the mean molecular weight of neutral gas). This may be set to zero in principle, but often it is desirable to prevent the gas from becoming too cold, e.g. for resolution reasons or because of lower limits in the implemented cooling function. (Default: `0`).
    ...
    MaxKernelRadius                 1.0e10  % maximum gas kernel length (some very large value to prevent errors)
    MinGasKernelRadiusFractional    0       % minimum kernel length relative to gas force softening (<= 1)
...
**Note on configuring adaptive softening**: If you use any of `ADAPTIVE_GRAVSOFT_FORGAS`, `ADAPTIVE_GRAVSOFT_FORALL`, or `ADAPTIVE_GRAVSOFT_FROM_TIDAL_CRITERION`, you must carefully set three parameterfile values together: (1) `Softening_TypeX` sets the **minimum** allowed softening for each particle type (the softening will never go below this); (2) `MaxKernelRadius` sets the **maximum** allowed softening (it will never exceed this); and (3) `AGS_DesNumNgb` controls the target neighbor number used to set the adaptive softening for non-gas particles. If any of these are misconfigured (e.g., minimum too large, maximum too small, or neighbor number inappropriate), the adaptive softening may not behave as intended.
...
Alternatively, for historical reasons, you can choose to use the naming convention for the force-softening from GADGET and AREPO. This looks like the following, in your parameterfile:
    ...
    SofteningBndry     0.001    % sink particles (if active), or collisionless (type=5)
    %---- if these are set in cosmo runs, SofteningX switches from comoving to physical
    %------- units when the comoving value exceeds the choice here
    %------- (these are ignored, and *only* the above are used, for non-cosmo runs)
...
**BiniX/BiniY/BiniZ** (or `B_initial_x`, `B_initial_y`, `B_initial_z`): If `MHD_B_SET_IN_PARAMS` is turned on at compile-time, then when starting from an initial condition (not a restart), the B-fields of all gas are initialized to these X/Y/Z values (given here in Gauss, these will be converted to code units). If `MHD_B_SET_IN_PARAMS` is not active, the initial B-fields will be read from the ICs file instead (these values are ignored).
    ...
    %--- Modified-Gradient div(B) correction (MHD_MODIFIED_GRADIENT on)
    ActiveFractionForMGSweep   0.0    % fraction of active cells required to trigger MG global solve (0=always, 1=never)

**ActiveFractionForMGSweep**: If `MHD_MODIFIED_GRADIENT` is enabled, this controls how frequently the global sparse-matrix div(B) correction is applied. At each sync-point, the code computes the fraction of all gas cells that are active at that timestep. If this fraction is greater than or equal to `ActiveFractionForMGSweep`, the full MG solve (via HYPRE BoomerAMG or the built-in SSOR-CG solver) is executed; otherwise the step is skipped and the cheaper CG-only correction is applied instead. Setting this to `0.0` (or omitting it entirely, which defaults to `0.0`) applies the MG solve every sync-point, giving maximum div(B) control at the highest cost. Setting it to a value like `0.1` applies the global solve only on the larger timestep intervals (when ≥10% of cells are active), which can recover most of the accuracy benefit with substantially lower overhead in multi-timestep simulations.
...
**CosmicRay\_SNeFraction** (or `CosmicRay_SNeEnergyFraction`): If `COSMIC_RAY_FLUID` and `GALSF_FB_MECHANICAL` are both on with the default FIRE physics implementation active as well, then a fraction (typically calculated and observationally inferred to be $\sim 0.1$) of the initial supernova kinetic energy of each explosion is deposited into cosmic rays.
    ...
    %-------------- parameters for grain-grain collisions/scattering/sticking/shattering (GRAIN_COLLISIONS on)
...
**Grain\_Absorbed\_vs\_Total\_Extinction**: fraction of incident radiation absorbed (1-albedo), assumed for grains, if `RT_OPACITY_FROM_EXPLICIT_GRAINS` and `RT_GENERIC_USER_FREQ` are set.
    ...
    %---- Nuclear Reaction Networks (NUCLEAR_NETWORK on)
    NuclearNetworkDataFile    ""              % path to network data directory (empty = built-in aprox13; required for SkyNet/Torch)
    NuclearBurningFloor_T     1.0e8           % minimum temperature [K] below which nuclear burning is not computed (default 1e8 K)
    NuclearBurningFloor_rho   0               % minimum density [code units] below which burning is not computed (default 0 = no floor)
    NuclearNSE_T_threshold    6.0e9           % temperature [K] above which NSE table used instead of integrating network (default 6e9 K)

**NuclearNetworkDataFile**: Path (as a string) to the directory containing reaction rate data files needed by the chosen nuclear network solver. For the built-in `aprox13` network (`NUCLEAR_NETWORK_SOLVER=0`), this parameter is ignored and can be left as an empty string or omitted entirely -- no external data files are needed. For external solvers (`NUCLEAR_NETWORK_SOLVER=1` for SkyNet, `=2` for Torch), this must be set to the directory containing the network's data tables (e.g. nuclear partition functions, reaction rate coefficients). See the documentation of the respective library for what files are required and where they should live.

**NuclearBurningFloor\_T**: Minimum temperature threshold (in Kelvin) below which nuclear burning reactions are not computed for a given particle. Particles with temperature below this value are assumed to be in a regime where the nuclear timescales are far longer than the hydro timestep and the energy release is negligible. The default value of $10^8$ K is appropriate for most applications; lowering it will increase computational cost by integrating burning for cooler particles that contribute negligibly to the overall energetics. This parameter has no effect if `NUCLEAR_NETWORK` is not enabled.

**NuclearBurningFloor\_rho**: Minimum density threshold (in code units) below which nuclear burning is not computed. Combined with `NuclearBurningFloor_T`, this allows you to exclude low-density regions (e.g. unbound ejecta) from the burning calculation for efficiency. The default value of 0 disables the density floor (i.e. burning is evaluated for any particle above the temperature floor regardless of density). For problems like white dwarf detonations where the outer, low-density layers are unimportant, setting this to a small positive value (e.g. the density of the initial surface layers) can significantly reduce compute time.

**NuclearNSE\_T\_threshold**: Temperature (in Kelvin) above which the code switches from integrating the full nuclear reaction network to a tabulated nuclear statistical equilibrium (NSE) lookup. At temperatures above this threshold, nuclear reactions proceed on timescales much shorter than the hydrodynamic timestep, so the composition can be assumed to be in instantaneous NSE, which is fast to evaluate from a pre-computed table. The default of $6\times10^9$ K is a standard choice for thermonuclear burning applications; higher values will integrate the network to higher temperatures (more accurate but slower), while lower values will use NSE sooner (faster but less accurate for the approach to NSE). This parameter is only used when `NUCLEAR_NETWORK_NSE_TABLE` is enabled in `Config.sh`.
...
Note that *all* of these FIRE parameters have sensible default values (the values in the parameter block above), which will be automatically used if the relevant code modules are turned on but the parameters are not specifically set by the user.
    ...
    SeedReservoirMass       0.0        % initial mass in the alpha disk (SINK_ALPHADISK_ACCRETION)
    ...
    SeedSinkPerUnitMass     1.e-4      % stellar mass (code unit) per BH seed (SINK_SEED_FROM_LOCALGAS)
    ...
    SinkMaxAccretionRadius  0.02	    % max radius for BH neighbor search/accretion (code units)
    SinkRadiativeEfficiency 0.1        % radiative efficiency (for accretion and feedback)
    %--- feedback 
    ...
    Sink_accreted_fraction  0.5        % fraction of gas swallowed by sink (SINK_WIND options; alt name Sink_f_accretion)
    Sink_outflow_velocity   30000	    % velocity (code units) of outflow from sink (SINK_WIND options; alt name Sink_v_outflow)
    Sink_outflow_temperature 1.0e4      % internal temperature (K) of BAL outflow (SINK_WIND_SPAWN; alt name Cell_Spawn_Temperature)
    Sink_outflow_particlemass 1.e-10     % mass of 'virtual wind particles' in code units (SINK_WIND_SPAWN; use Cell_Spawn_Mass_ratio if SINGLE_STAR or SINK_SCALE_SPAWNINGMASS_WITH_INITIALMASS is active)
    Sink_CosmicRay_Injection_Efficiency 0.1   % fraction of accretion energy into cosmic rays (SINK_COSMIC_RAYS; alt name Sink_CosmicRay_Injection_Efficiency)
...
**SeedReservoirMass**: Initial mass in the viscous alpha-disk (in code units), when `SINK_ALPHADISK_ACCRETION` is enabled. Usually zero, unless you want to initialize BHs rapidly accreting.

**SeedSinkMinRedshift**: Minimum redshift, below which seed BHs will not be spawned on-the-fly (regardless of the seed model in the simulations). 

**SeedSinkMassSigma**: In the seed models which spawn BHs on the fly, this parameter allows them to have random masses drawn from a log-normal distribution. Then **SeedSinkMass** becomes the median/peak of the log-normal, and this specifies with width (in dex) of the distribution function. To fix the BH mass, simply set this to zero.

**SeedSinkPerUnitMass**: If `SINK_SEED_FROM_LOCALGAS` is enabled, this determines the seed probability (probability is dp=dmformed/this mass), set in code units

**MinFoFMassForNewSeed**: This sets the minimum mass of an on-the-fly FOF group which will get a BH seed (if a BH isn't already present), when using the `SINK_SEED_FROM_FOF` flag[s]. The mass is in code units, and refers to either the total mass (if `SINK_SEED_FROM_FOF=0`, where total halo mass is used to identify groups) or stellar mass (if `SINK_SEED_FROM_FOF=1`, where BHs are seeded in stellar groups specifically). 
...
**SinkAccretionFactor**: Multiplies the accretion rate relative to the 'default' for whatever chosen accretion model is given by the compile-time choices. It can always be set =0 to prevent BH accretion.

**SinkEddingtonFactor**: Fraction of the Eddington limit at which to cap the maximum BH accretion rate. Set to some arbitrarily large value to effectively remove the Eddington limit. 

**SinkNgbFactor**: The BH particle searches for neighbors to determine things like its accretion, BH-BH mergers, some feedback calculations. Depending on the fueling/feedback model (which may require estimating things like whether the gas is in a disk), you may want the number of neighbors to be several times larger than the 'default' kernel search. This multiplies DesNumNgb, specifically for the search around the BH particles.

**SinkMaxAccretionRadius**: This sets a hard maximum radius (in physical code units) out to which the BH particle will search. This is included to prevent cases where a poorly-resolved, gas-poor galaxy has a BH which then searches out to some very large distance until it finds gas (potentially leading to unphysical accretion or mergers from very large separations).

**SinkRadiativeEfficiency**: Radiative efficiency of the BH (defined in the standard manner, $L =$ SinkRadiativeEfficiency $*\,(dM/dt) * c^{2}$). Used to calculate luminosities for feedback, eddington accretion rates, and correct the actual accreted mass.

**SinkFeedbackFactor**: Multiplier for the strength of all BH feedback mechanisms (whatever is enabled, the energy/momentum will be scaled by this factor relative to its 'default' which corresponds to values =1). You can set this =0 to turn off BH feedback.

**SINK\_FluxMomentumFactor** or **SINK\_Rad_MomentumFactor**: (Either name here produces the same functionality). This serves the same role as SinkFeedbackFactor, specific to the radiation pressure terms from the BH. Currently this is separated just for de-bugging purposes.

**BAL\_f\_accretion**/**BAL\_v\_outflow** (or **Sink\_f\_accretion**/**Sink\_v\_outflow**): These control the model for accretion disk or broad-absorption line winds from the BH (compiler flags beginning with `SINK_WINDS`). `Sink_accreted_fraction` (or `Sink_f_accretion`) controls the mass-loading of the winds: a fraction `Sink_accreted_fraction` of the mass in the inner accretion disk is assumed to be accreted, while (`1-Sink_accreted_fraction`) is blown out in the wind, with launch velocity `Sink_outflow_velocity` (or `Sink_v_outflow`) in km/s. See the Config.sh discussion, or the `notes_blackholes` file, for more details. 

**BAL\_internal\_temperature**/**BAL\_wind\_particle\_mass** or (**Cell\_Spawn_\Temperature**/**Cell\_Spawn\_Mass\_ratio**): These options are specific to the module `SINK_WIND_SPAWN`, and set the internal temperature (in K) and virtual particle mass (in code units) of the virtual "wind particles" spawned by the BH wind. Note that as described above, if the `SINGLE_STAR` modules are active or one sets the flag `SINK_SCALE_SPAWNINGMASS_WITH_INITIALMASS`, then the spawn mass is not fixed to a single absolute value but a value equal to `Cell_Spawn_Mass_ratio` times the initial formation mass of the sink particle doing the spawning.

**Sink_CosmicRay_Injection_Efficiency**: This determines the fraction of the accretion energy (not radiated energy) which is deposited into cosmic rays when `SINK_COSMIC_RAYS` is active
...
### _Non-Standard Dark Matter, Dark Energy, Gravity, or Expansion_ 
    ...
    %-------------- Parameters for DM annihilation/decay heating (DM_HEATING on)
    DM_AnnihilationSigmaV_over_mChi 0   % <sigma v>/m_chi in cm^3 s^-1 g^-1 (self-conjugate convention, no extra 1/2)
    DM_AnnihilationHeatingFraction  0   % f_h^ann in [0,1] -- fraction of rest-mass-energy thermalized locally
...
**DM\_AnnihilationSigmaV\_over\_mChi**: If `DM_HEATING` is on, this is the velocity-averaged annihilation cross section divided by the DM particle mass, $\langle\sigma v\rangle/m_\chi$, supplied in cgs (cm$^3$ s$^{-1}$ g$^{-1}$). The single combined parameter avoids two-knob coupling; setting it to zero disables the annihilation channel entirely. Convention: self-conjugate DM with no extra factor of $1/2$ in the annihilation rate (volumetric rate $\tfrac{1}{2}n_\chi^2\langle\sigma v\rangle$ combined with energy $2 m_\chi c^2$ per event gives $\rho_{\rm DM}^2 (\langle\sigma v\rangle/m_\chi) c^2$). Dirac/non-self-conjugate DM users fold the appropriate factor of two into the supplied scalar. v-dependence (e.g. Sommerfeld) is also folded by the user into this scalar — there is no in-code velocity-callback in this version.

**DM\_AnnihilationHeatingFraction**: If `DM_HEATING` is on, the fraction $f_h^{\rm ann} \in [0,1]$ of the released rest-mass-equivalent energy that thermalizes locally in the gas (the remainder escapes as e.g. neutrinos or radiation that streams freely on the local scale). Hard-aborts at startup if outside $[0,1]$. Set to zero (along with `DM_AnnihilationSigmaV_over_mChi`) to disable the annihilation channel.
...
These parameters are used if the `EOS_TILLOTSON` and/or `EOS_ELASTIC` flags are used to activate the equations-of-state and elastic dynamics for different solid+vapor bodies (for e.g. impact simulations or solid-body interactions). When this flag is active, all particles of Type=0 have an assigned material composition specified by the parameter "CompositionType". This will invoke pre-tabulated equations of state for granite, basalt, iron, ice, olivine/dunite, or water (as described above), but if CompositionType=0, a custom EOS will be used, specified by the parameters in the parameterfile (if no particles have CompositionType=0, then these parameters will be ignored). These are specified above, in cgs units, following the notation convention in Reinhardt+Stadel 2017,MNRAS,467,4252. The example given is for iron, but any additional species can be added this way. Note that if `EOS_ELASTIC` is used without `EOS_TILLOTSON`, the shear modulus (`mu`) and elastic limit (`Y0`) from the custom CompositionType=0 slot are applied as universal constants to all particles.

If `EOS_ANEOS` is active instead of (or in addition to) `EOS_TILLOTSON`, the following runtime parameters specify the ANEOS/SESAME tabulated equation of state:

```bash
%-------------- Parameters for ANEOS/SESAME tabulated equation-of-state (EOS_ANEOS on)
AneosNumMaterials               2                       % number of ANEOS material tables to load (max 7)
AneosTable0                     forsterite.sesame       % file path for material 0 (CompositionType=0)
AneosTable1                     iron.sesame             % file path for material 1 (CompositionType=1)
```

`AneosNumMaterials` sets how many SESAME table files to load at startup. `AneosTable0`, `AneosTable1`, etc. specify the file path for each material's table. Only the first `AneosNumMaterials` table paths are required; unused slots are ignored. Each particle's `CompositionType` field (set in the initial conditions) selects which table to use (0-indexed). The tables should be in ASCII SESAME format with a header line containing the material ID, number of density points, number of temperature points, and number of columns, followed by the density grid, temperature grid, and 2D data rows. Tables in the Stewart group SWIFT format can be converted with the `convert_stewart_table.py` utility provided in the test suite.
...
+ ISM, star formation, and proto-stellar disks: [MakeCloud](https://github.com/mikegrudic/MakeCloud) is a public code to generate ICs for turbulent clouds (hydro or MHD), with arbitrary initial rotation, written by Mike Grudic at Caltech. Many of the "isolated galaxy" codes above (particularly one such as [DICE](https://ascl.net/1607.002)) can also be fairly easily scaled to generate equilibrium proto-stellar or proto-planetary disks.
...
from initial_conditions.eos_tools.shell_placement  import glass_relax, atmosphere_shell, merge_placements
...
First note that there are many, many public codes and libraries (for python, IDL, C, and other languages) which can read and interact with GIZMO snapshots (or GADGET snapshots, which follow the same format). This includes for example [YT](http://yt-project.org/), [PynBody](https://ascl.net/1305.002), [PyGadgetReader](https://ascl.net/1411.001), [SPHGR](https://ascl.net/1502.012), [PyGad](https://ascl.net/code/v/1569), and more general tools like [MESHOID](https://github.com/omgspace/meshoid), [vaex](https://vaex.io/), [Glue](http://glueviz.org/en/stable/), [TOPCAT](http://www.star.bris.ac.uk/~mbt/topcat/). Even more examples are given in the "Additional Resources" section below. But it is important to understand directly what is in your snapshots, in case these codes are not appropriate for your analysis (or you are running simulations with different physics). 

For most snapshot analysis we **recommend** [MESHOID](https://github.com/mikegrudic/meshoid), a dedicated mesh-free analysis package. GIZMO also ships lightweight built-in snapshot readers in the top-level `analysis/` directory: `load_from_snapshot.py` and `readsnap.py` (plus the legacy IDL `readsnap.pro` in `analysis/legacy_idl/`). To use them, put `analysis/` on your Python path and import directly, e.g. `from load_from_snapshot import load_from_snapshot`. `load_from_snapshot` is the most up-to-date reader and includes fixes for unit conversions and modern field names.

The older routines `readsnap.py` and `readsnap.pro` (Python and IDL, respectively) are not very fast or efficient, but demonstrate how to read a variety of file types in full generality, including the legacy pure-binary format. The more modern `load_from_snapshot.py` is far more efficient and can read any field in the snapshots by name, but only works for HDF5 outputs. Note that these routines are specific to snapshots from simulations run including certain specific physics, and may have to be modified for the specific physics and conditions under which you run your simulation.
...
**KernelMaxRadius** (P['h']): [N]-element array, kernel radius of compact support for interaction face-element definition (called the "smoothing length" for the sake of historical compatibility with SPH in older versions of GIZMO, even though there is no smoothing within the kernel for the MFM/MFV methods).
...
Some quantities are also specific to sink particle particles (Type=5), including: 

**SINK\_Mass** (P['mbh']): [N]-array; the mass of the BH (not necessarily the same as the mass of the particle)

**SINK\_Mdot** (P['mdot']):[N]-array; the accretion rate of the BH evaluated at this instant (in code units MASS/TIME)

**SINK\_Mass\_Reservoir**: [N]-array (only included if `SINK_ALPHADISK_ACCRETION` is active); the mass in the sub-grid viscous BH accretion disk 


**Additional physics-dependent output fields** (only present when the relevant compile-time flags are active):

*Equation-of-State fields* (when `EOS_GENERAL`, `EOS_TILLOTSON`, `EOS_ANEOS`, or `EOS_HELMHOLTZ` is active): **Temperature** (gas temperature in K), **SoundSpeed** (adiabatic sound speed), **Pressure** (gas pressure). When `EOS_TILLOTSON` or `EOS_ANEOS` is active: **CompositionType** (integer material index). When `EOS_ANEOS` is active: **PhaseID** (material phase flag: solid/liquid/vapor/mixed). When `EOS_HELMHOLTZ` is active: **Ye** (electron fraction), **Abar** (mean atomic weight).

*Cosmic ray fields* (when `COSMIC_RAY_FLUID` is active): **CosmicRayEnergy** (CR energy per bin), **CosmicRayDiffusionCoeff** (effective CR diffusion/streaming coefficient), **CosmicRayAlfvenEnergy** (Alfven wave energy, if `CRFLUID_EVOLVE_SCATTERINGWAVES`), **CosmicRaySlopes** (spectral slopes, if multi-bin CR spectrum evolved).

*Radiation fields* (when `RT_EVOLVE_INTENSITIES` or similar RT flags are active): **Rad\_E\_gamma** (radiation energy density per frequency bin), **Rad\_Flux** (radiation flux vector per bin), **Rad\_Kappa\_RP** (radiation opacity), **Dust\_Temperature** (dust temperature, if dust thermodynamics enabled).

*Sink/compact object fields* (PartType=5, when sink physics enabled): **Sink\_Mass** (sink gravitational mass), **Sink\_Mdot** (accretion rate), **Sink\_FormationMass** (mass at formation), **Sink\_AngMom** (angular momentum vector). When protostellar evolution is active: **ProtoStellar\_Radius**, **ProtoStellar\_Stage**, **ProtoStellar\_Age**, **ProtoStellar\_Luminosity**, **ZAMS\_Mass**.
...
The code will write to standard error in general only when there is an error which is not handled by a "clean" exit/code termination within the code (so it you see a bunch of outputs to stderr, you should try to find out whats happening!). The code will output a very large amount of information to stdout. This is generally channeled through a single 'lead' process ("Task 0"), to avoid slowing down processes, but all processes are capable of writing to stdout. It is recommended that you route this to a named file so that you can track it. An example from a single timestep might look like:
    ...
    Sink operations begin...
     ..closing sink operations
...
**sinks**: sink particle physics (neighbor finding, accretion, and feedback routines), if `SINK_PARTICLES` active.
...
Models with the short-range local radiation pressure (`GALSF_FB_FIRE_RT_LOCALRP`) will write 'MomWinds.txt'
    ...
    0.256422469 1478 96 18173 2.48201 0.0134358 0.00263988 0.0298646 4.49967e-06  

Columns are (1) simulation time, (2) number of star particles active this timestep and checked against the emission criterion, (3) number of those which actually emit, after the stochastic criterion which lets the routine fire rarely and couple a correspondingly larger momentum when it does (so this is typically a small fraction of the previous column), (4) total number of gas cells which receive momentum from those stars, (5) mean radius searched around each emitting star (code units), which is where the cost of the routine comes from, since the work scales with the number of cells inside it, (6) total photon momentum ("$dt\,L / c$") of the emitting stars for which the radiation pressure is being calculated (code units) for the timestep, (7) total momentum actually coupled to gas -- note this can and usually does -exceed- the previous column, because the infrared term couples the same photon momentum repeatedly where the gas is optically thick to its own re-radiation, (8) average velocity of the discretized "kicks" assigned to gas particles (acceleration $*$ dt) from the momentum, (9) mean infrared multiple-scattering boost -per coupled cell-: for each cell, the infrared kick it is offered against the bare single-scattering momentum per unit mass, averaged over cells and weighted by the momentum each one receives. This last is a per-cell mean and not the total boost to the coupled momentum in column (7); the momentum weighting pulls it toward the cells nearest the source, so it sits well above the median cell and should not be read as a characteristic optical depth. 
...
**Step 1: Install GIZMO as a Python package.** From the GIZMO source directory, run `pip install .` (or `pip install -e .` for an editable/development install). This installs the `gizmo` Python package, which includes test utilities and snapshot-reading tools. Dependencies include `pytest`, `h5py`, `numpy`, `scipy`, `matplotlib`, `astropy`, and `meshoid`.

**Step 2: Ensure your build environment works.** Make sure `Makefile.systype` has the correct system type uncommented for your machine (e.g. `SYSTYPE=MacBookCellar` for a typical macOS homebrew setup).

**Step 3: Run tests.** From the GIZMO source directory:

    # Run all tests
    pytest

    # Run a specific test
    pytest test/shocktube

    # Run several tests by pattern
    pytest test/sedov test/noh test/shocktube

    # Verbose output
    pytest test/sedov -v

    # Show stdout during the test (useful for debugging)
    pytest test/sedov -v -s

Each test automatically: (1) builds GIZMO with the test-specific `Config.sh` flags, (2) downloads initial conditions and reference solutions if needed, (3) runs the simulation with appropriate MPI/OpenMP parallelization, and (4) validates results against analytical solutions or reference data. By default, tests use half the available CPU cores as MPI ranks and 2 OpenMP threads per rank.

**Interpreting results.** A passing test prints `PASSED`; a failing test shows the assertion that failed (e.g. "Density L1 error 0.12 exceeds tolerance"). If a test fails, check `test/{test_name}/test_{test_name}.err` for compiler or runtime errors, and `test/{test_name}/test_{test_name}.out` for GIZMO stdout. Simulation output snapshots are in `test/{test_name}/output/`, and diagnostic plots (if generated) are in `test/{test_name}/`.
...
I've also made an effort to "pre-package" these as much as possible, so you can run them with relatively little effort. They don't come with any guarantees though!
...
Plus a flag to specify the hydro solver you want to use, e.g. `HYDRO_MESHLESS_FINITE_MASS` (MFM), `HYDRO_MESHLESS_FINITE_VOLUME` (MFV), or an SPH variant. All test problems listed below similarly require a hydro solver flag; the automated tests in the `test/` directory each include the solver in their `Config.sh` (typically `HYDRO_MESHLESS_FINITE_MASS`).
...
For both versions of the KH test, the number of domain segments per task is chosen automatically, but on a high-resolution IC like this one you can experiment with DOMAIN_SEGMENTS_SCALE to trade load-balancing against memory and communication. You should also experiment with the kernel function and hydro solver, and the `DEVELOPER_MODE` parameters that are set for example in the default parameterfile.
...
The number of domain segments per task is chosen automatically, but on a high-resolution IC like this one you can experiment with DOMAIN_SEGMENTS_SCALE to trade load-balancing against memory and communication.
...
You should experiment with enabling/disabling `ADAPTIVE_GRAVSOFT_FORGAS`, and varying the gravitational softenings (remember, with `ADAPTIVE_GRAVSOFT_FORGAS` set, the softenings in the parameterfile for gas are the minimum values; otherwise they are constant through the run). See how this changes the results. You may also want to experiment with DOMAIN_SEGMENTS_SCALE, which trades load-balancing against memory and communication. You can turn off PMGRID and run in pure tree mode (slower, but should work as well or slightly more accurately), or vary the PMGRID level between e.g. 64-256. 
...
**GMC Cooling with Radiation Transfer** (`test/gmc_cooling_rt`): As above, but with radiation transport enabled (`RT_EVOLVE_INTENSITIES` or LEBRON), testing the combined cooling+RT pipeline.

**Shu 1977 Collapse** (`test/shu1977`): The classic isothermal sphere collapse solution from Shu (1977, ApJ, 214, 488). Tests self-gravitating isothermal gas dynamics and is a standard benchmark for star formation simulations.
...
**Nuclear Detonation** (`test/nuclear_detonation`): A 3D-elongated 1D detonation test (`BOX_LONG_X=4`) for thermonuclear flame propagation in a degenerate-CO mixture, exercising the coupled `EOS_HELMHOLTZ` + `NUCLEAR_NETWORK` + `NUCLEAR_NETWORK_NSE_TABLE` pipeline. Tests that a self-sustained burning front advances at the expected speed and produces the expected NSE-equilibrated post-shock composition.
...
+ [FIREFly](https://firefly.rcs.northwestern.edu/) is a fantastic on-the-fly visualization app originally developed for FIRE (GIZMO) datasets, specifically built to be lightweight and completely runnable in-browser making it ideal for presentations and pedagogical applications
...
+ [AHF](http://popia.ft.uam.es/AHF/): The AMIGA halo finder -- another halo finder, this one designed for cosmological simulations, popular for galaxy/halo identification and building merger trees.
...
+ [MESHOID](https://github.com/mikegrudic/meshoid): Useful Python module for analyzing particle data. Computes neighbour lists, smoothing lengths and meshless derivatives just like GIZMO, and has methods for kernel-to-grid deposition for slice/projection plots. This is extremely useful when you want to do something like re-construct the gradients of some field (say, density) in post-processing, which weren't saved by the outputs directly. Its also a great tool for actual plotting and analysis.
...
+ [ACCESS](https://access-ci.org/): Go here to create an ACCESS account, necessary to apply for time on the NSF-supported network of super-computers, and to access various machines GIZMO is commonly run on. Lots of questions about super-computing in general are answered there. 

+ [TACC](https://tacc.utexas.edu/): One of the NSF hubs, TACC hosts many machines that users often run GIZMO on. This is a link to their website, where they have user guides for getting anything set up or running on their machines. Even if youre not using those specific machines, some of these can be a good introduction, but of course check the manuals for the specific machines you are using. 
...
On Frontera (TACC), a typical SLURM script looks like:
...
Where `-p` specifies the queue (default=normal, short runs=development), `-t` specifies the wall-clock runtime (hours:minutes:seconds), `-N` specifies the node number, `-n` the total number of MPI processes. Frontera has 56 cores per node, so here we request 16 nodes with a total of 896 MPI processes (up to `n`=`N`*56). `-A` is the account to charge it to. When you log in, the first thing you see should be a list of the accounts you are listed on. If there is only one, you dont need to specify this (it will default to that one). Other SLURM-based machines will use a very similar script structure; consult the user guide for your specific machine for queue names, core counts, and launcher commands.

The line `export OMP_NUM_THREADS` should have the leading `#` removed if and only if you compile the code in hybrid OPENMP-MPI mode. This sets the number of OPENMP threads. In the Config.sh file used to compile the code, the value of OPENMP must be set/uncommented (e.g. `OPENMP=2`, here) and MUST EXACTLY MATCH the integer value here. Also, since there are now `OMP_NUM_THREADS` number of threads per MPI process, the `-n` above (number of MPI processes) must be lowered. Each thread uses one CPU. So the maximum value of `-n` is `-N` * (cores per node) / `OMP_NUM_THREADS`. 
...
In the **Config.sh** file: (1) (`ALLOW_IMBALANCED_GASPARTICLELOAD` used to appear here; it is retired and has no effect, because the gas-cell capacity now always follows the particle capacity.) (2) There are some algorithms that can be especially memory-intensive. Setting `PMGRID` or `TURB_DRIVING_SPECTRUMGRID` to very large values means you need to allocate memory for that size **cubed** because you are making a three-dimensional grid, so try lowering these values, or looking for other areas where you are asking for large memory overheads. The code dumps of the memory footprint on startup and at particular use times are helpful for this. (3) `DOMAIN_SEGMENTS_SCALE` multiplies the number of domain segments each MPI task is given. While more segments allow for more sub-chunking, each one carries fixed memory overheads (storing tree and communication data) as well as additional comm-buffers. This is also how you can often overload the comm buffers, since even on a modest number of nodes the number of simultaneous communication requests is your MPI task count times the segments per task (a very common source of problems when set too large). (4) Adjust `OPENMP` as described above.

In the **parameterfile**: (1) The working memory pool and the communication chunk (`WorkingMemoryPoolSize`, `CommChunkSize`) are worked out by the code and should not normally be set. The code prints the pool size it chose at start-up. If a run stops with a message naming the pool, something needed more of it than was predicted, and setting that parameter is the right response; note that the pool is only one of several places memory is used, so it is not a cap on the task's total. (2) `PartAllocFactor` sets the largest particle-load imbalance the decomposition may produce (if the average task holds N elements and this is 5, one task may be given up to 5N). Lowering it reduces the worst-case memory on any one task, at the cost of a worse work-load balance. Raising it does not buy room for imported data -- that is measured separately and added on top -- and it does enlarge the tree's node arrays, so raising it to fix a memory problem generally makes the problem worse. If the value is too low the decomposition may fail to find any valid solution at all. (3) (4) Decreasing `DomainBuild_ActiveFraction`, making domain decompositions more frequent, will allow finer-grained refactoring of the memory balancing, making it easier to avoid memory overflows (at obvious computational cost). Optimal values are wildly problem-dependent, anything from 1e-8 to 0.1 in simulations I have run personally.
...
In older versions of GIZMO, there were also a couple of `Config.sh` parameters which could be required for large runs (`LONGIDS` and `NO_ISEND_IRECV_IN_DOMAIN`). In the current version of GIZMO, these are always on and hard-coded into the Makefile and source, so users do not need to set them.
