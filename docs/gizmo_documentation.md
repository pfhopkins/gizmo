...
GIZMO includes Python routines for reading snapshots, available in two ways. The **recommended** approach is to install the `gizmo` Python package by running `pip install .` from the GIZMO source directory. This provides `load_from_snapshot` and other utilities as importable modules (e.g. `from gizmo.load_from_snapshot import load_from_snapshot`). The package version is the most up-to-date and includes fixes for unit conversions and modern field names. Alternatively, standalone copies of the scripts are available in the `scripts/` folder (`load_from_snapshot.py`, `readsnap.py`, `readsnap.pro`), though these may lag behind the packaged versions.
