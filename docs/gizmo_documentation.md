...
**Parameterfile Settings:** The params file also has a couple settings that can have a big impact. (1) `TreeRebuild_ActiveFraction` or `TreeDomainUpdateFrequency` (just alternate names for the same parameter): this requires more hand-tuning from experience than almost any other parameter. See its description: decreasing it will force the code to rebuild the domain decomposition more frequently, and can reduce load imbalances. But making it too frequent will cause it to bottleneck the code. A good 'rule of thumb' is that if the code is spending comparable time on imbalances (`treeimbal`+`densimbal`+`hydimbal` in `cpu.txt`) and domain decomposition (`domain` in `cpu.txt`), then this is set about right (but if both of these are very large, this is obviously an issue). (2) `PartAllocFactor`: this sets how much particle-load imbalance the decomposition may accept in order to reduce work-load imbalance. Raising it does *not* buy room for data imported from other tasks, which is measured and allocated separately, and it does enlarge the gravity tree's node arrays, so a large value costs memory without helping. (3) Other parameters like force softening, neighbor number, time between snapshots or statistics, etc, are generally small corrections, unless they are set to inappropriate values and cause problems. For example, setting a force softening much too-small means the timesteps are artificially small; setting it too large will introduce large imbalances as the particles need to perform direct-$N^{2}$ gravity operations between every neighbor within their force-softening kernels.
