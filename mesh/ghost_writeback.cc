...
void ghost_writeback_grainbackrx(void)
{
    ...
    /* Multi-rank collective: only NTask<=1 short-circuits the MPI calls below.
     * num_ghosts==0 OR snapshot==NULL still participates with empty buffers,
     * since narrow-supply migrated callers can have ghost imports on one rank
     * but not its peer; gating the collective on local counts would deadlock. */
    if(NTask <= 1) {
