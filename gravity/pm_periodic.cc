  ...
  for(ngrp = 0; ngrp < (1 << PTask); ngrp++)
    {
      ...
      if(task < NTask)
	{
	  /* Self-send: memcpy avoids int-count overflow for PMGRID >= 1024 single-rank. */
	  if(task == ThisTask) {
	    memcpy(field + (size_t)PMGRID * first_slab_of_task[task] * nslab_x,
		   scratch + (size_t)PMGRID * first_slab_of_task[task] * nslab_x,
		   (size_t)PMGRID * nslab_x * slabs_per_task[task] * sizeof(fftw_real));
	  } else {
...
void pm_periodic_transposeB(fftw_real * field, fftw_real * scratch)
{
  ...
  for(ngrp = 0; ngrp < (1 << PTask); ngrp++)
      ...
      if(task < NTask)
	{
	  /* Self-send: memcpy avoids int-count overflow for PMGRID >= 1024 single-rank. */
	  if(task == ThisTask) {
	    memcpy(scratch + (size_t)PMGRID * first_slab_of_task[task] * nslab_x,
		   field   + (size_t)PMGRID * first_slab_of_task[task] * nslab_x,
		   (size_t)PMGRID * nslab_x * slabs_per_task[task] * sizeof(fftw_real));
	  } else {
...
static char power_spec_fname[DEFAULT_PATH_BUFFERSIZE_TOUSE];
...
void calculate_power_spectra(int num, long long *ntot_type_all)
{
  ...
  snprintf(power_spec_fname, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s/powerspec_%03d.txt", All.OutputDir, num);
	...
	if(ntot_type_all[i] > 0)
	  {
	    ...
	    snprintf(power_spec_fname, DEFAULT_PATH_BUFFERSIZE_TOUSE, "%s/powerspec_type%d_%03d.txt", All.OutputDir, i, num);
