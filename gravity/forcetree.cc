...
int force_treebuild(int npart, struct unbind_data *mp)
{
    ...
    if(gpu_topology_finalize_father(Numnodestree)  != 0) {gizmo_hard_abort_reviewed(90000065, "TEMP_HARD_CANDIDATE_INTERNAL: GPU finalize_father failed (rank-local); invalid tree topology fed into pseudo/LET collectives + GPU walk; a return would skip peer collectives (deadlock); original endrun(913320)", __FILE__, __LINE__, __FUNCTION__);}
    if(gpu_topology_finalize_sibling(Numnodestree) != 0) {gizmo_hard_abort_reviewed(90000066, "TEMP_HARD_CANDIDATE_INTERNAL: GPU finalize_sibling failed (rank-local); invalid tree topology fed into pseudo/LET collectives + GPU walk; original endrun(913321)", __FILE__, __LINE__, __FUNCTION__);}
    ...
    if(gpu_node_reset_ephemeral(Numnodestree) != 0) {gizmo_hard_abort_reviewed(90000067, "TEMP_HARD_CANDIDATE_INTERNAL: GPU node_reset_ephemeral failed (rank-local); invalid tree state fed into pseudo/LET collectives + GPU walk; original endrun(913323)", __FILE__, __LINE__, __FUNCTION__);}
    if(gpu_moment_refresh(-1) != 0) {gizmo_hard_abort_reviewed(90000068, "TEMP_HARD_CANDIDATE_INTERNAL: GPU moment_refresh failed (rank-local); invalid node moments/device state fed into pseudo/LET collectives + GPU walk; original endrun(913311)", __FILE__, __LINE__, __FUNCTION__);}
    if(gpu_nextnode_thread() != 0) {gizmo_hard_abort_reviewed(90000069, "TEMP_HARD_CANDIDATE_INTERNAL: GPU nextnode_thread failed (rank-local); invalid tree topology fed into pseudo/LET collectives + GPU walk; original endrun(913312)", __FILE__, __LINE__, __FUNCTION__);}
    if(gpu_topology_writeback_d_to_aos(Numnodestree) != 0) {gizmo_hard_abort_reviewed(90000070, "TEMP_HARD_CANDIDATE_INTERNAL: GPU topology writeback_d_to_aos failed (rank-local); invalid AoS tree fed into pseudo/LET collectives + GPU walk; original endrun(913322)", __FILE__, __LINE__, __FUNCTION__);}
    ...
    if(gpu_force_flag_localnodes() != 0) {gizmo_hard_abort_reviewed(90000071, "TEMP_HARD_CANDIDATE_INTERNAL: GPU force_flag_localnodes failed (rank-local); invalid TOPLEVEL/DEPENDS bitflags fed into pseudo/LET collectives + GPU walk; original endrun(913340)", __FILE__, __LINE__, __FUNCTION__);}
    ...
    if(let_run_exchange() != 0)             {gizmo_hard_abort_reviewed(90000072, "TEMP_HARD_CANDIDATE_INTERNAL: LET MPI exchange failed mid-collective; pseudodata_complete + GPU walk would proceed on a broken exchange (no safe drain); original endrun(913343)", __FILE__, __LINE__, __FUNCTION__);}
    ...
    if(gpu_scatter_pseudo_to_soa() != 0)    {gizmo_hard_abort_reviewed(90000073, "TEMP_HARD_CANDIDATE_INTERNAL: GPU scatter_pseudo_to_soa failed (rank-local); invalid foreign moments fed into topnode resum + GPU walk; original endrun(913341)", __FILE__, __LINE__, __FUNCTION__);}
    ...
    if(gpu_topnode_moment_resum() != 0)     {gizmo_hard_abort_reviewed(90000074, "TEMP_HARD_CANDIDATE_INTERNAL: GPU topnode_moment_resum failed (rank-local); invalid tree moments handed to the GPU gravity walk (no safe drain); original endrun(913342)", __FILE__, __LINE__, __FUNCTION__);}
...
{
    int i, no, m;
    if(DomainMoment_pending != NULL) {gizmo_hard_abort_reviewed(90000075, "TEMP_HARD_CANDIDATE_INTERNAL: pseudodata issue() re-entrant (pending != NULL); continuing leaks the buffer and re-posts the Iallgatherv, corrupting the collective; original endrun(913401)", __FILE__, __LINE__, __FUNCTION__);} /* re-entrant call: bug */
...
{
    if(DomainMoment_pending == NULL) {gizmo_hard_abort_reviewed(90000076, "TEMP_HARD_CANDIDATE_INTERNAL: pseudodata complete() unmatched (pending == NULL); continuing derefs NULL + Waitall on stale requests; original endrun(913402)", __FILE__, __LINE__, __FUNCTION__);} /* unmatched complete: bug */
    struct DomainNODE *DomainMoment = DomainMoment_pending;
...
void force_treeupdate_pseudos(int no)
{
    ...
    for(j = 0; j < 8; j++)    /* since we are dealing with top-level nodes, we now that there are 8 consecutive daughter nodes */
    {
        if(p >= All.MaxPart && p < All.MaxPart + MaxNodes)    /* internal node */
        {
            ...
            gizmo_hard_abort_reviewed(90000077, "TEMP_HARD_CANDIDATE_INTERNAL: invalid node type during moment summation walk; continuing derefs Nodes[p].sibling on a corrupt node and produces invalid tree moments; original endrun(6767)", __FILE__, __LINE__, __FUNCTION__);        /* may not happen */
...
void force_flag_localnodes(void)
{
    ...
    for(m = 0; m < MULTIPLEDOMAINS; m++)
            ...
            i <= DomainEndList[ThisTask * MULTIPLEDOMAINS + m]; i++)
        {
            ...
            if(DomainTask[i] != ThisTask) {gizmo_hard_abort_reviewed(90000078, "TEMP_HARD_CANDIDATE_INTERNAL: domain ownership invariant violated (DomainTask[i] != ThisTask in local-domain loop); continuing mismarks a foreign subtree's DEPENDS bitflags (invalid tree state); original endrun(131231231)", __FILE__, __LINE__, __FUNCTION__);}
...
int force_treeevaluate(int target, int mode, int *exportflag, int *exportnodecount, int *exportindex)
{
    ...
    if(mode != 0 && mode != 1) {printf("%d %d %d %d %d\n", target, mode, *exportflag, *exportnodecount, *exportindex); gizmo_hard_abort_reviewed(90000079, "TEMP_HARD_CANDIDATE_INTERNAL: force_treeevaluate called with mode not in {0,1}; continuing runs the walk with undefined export accounting, mismatching the export exchange; original endrun(444)", __FILE__, __LINE__, __FUNCTION__);}
#endif
                        ...
                        if(Exportnodecount[task] == NODELISTLENGTH)
                        {
                            if(*nexport >= All.BunchSize)
                            {
                                *nexport = nexport_save;
                                if(nexport_save == 0) {gizmo_hard_abort_reviewed(90000080, "TEMP_HARD_CANDIDATE_OOM: export buffer (BunchSize) too small to process even a single particle; soft-continue returns -1 with zero progress -> infinite export-communication retry; original endrun(13002)", __FILE__, __LINE__, __FUNCTION__);} /* in this case, the buffer is too small to process even a single particle */
                ...
                if(no >= All.MaxPart + MaxNodes + MaxForeignNodes)    /* pseudo particle (Phase 9: foreign-node range below pseudos) */
                {
                    if(mode == 0)
                    {
                        ...
                        if(Exportnodecount[task] == NODELISTLENGTH)
                        {
                            ...
                            {
                                *nexport = nexport_save;
                                if(nexport_save == 0) {gizmo_hard_abort_reviewed(90000081, "TEMP_HARD_CANDIDATE_OOM: export buffer (BunchSize) too small to process even a single particle; soft-continue returns -1 with zero progress -> infinite export-communication retry; original endrun(13001)", __FILE__, __LINE__, __FUNCTION__);} /* in this case, the buffer is too small to process even a single particle */
                                for(task = 0; task < NTask; task++) {nsend_local[task] = 0;}
                                for(no = 0; no < nexport_save; no++) {nsend_local[DataIndexTable[no].Task]++;}
                                return -1; /* buffer has filled -- important that only this and other buffer-full conditions return the negative condition for the routine */
                            }
...
void force_treeallocate(int maxnodes, int maxpart)
{
    ...
    if(!Nodes_base)
    {
        ...
        gizmo_hard_abort_reviewed(90000082, "TEMP_HARD_CANDIDATE_OOM: Nodes_base UVM alloc failed; continuing sets Nodes = NULL - MaxPart (wild pointer) deref'd across the tree machinery; original endrun(3)", __FILE__, __LINE__, __FUNCTION__);
    }
    ...
    if(!Extnodes_base)
    {
        ...
        gizmo_hard_abort_reviewed(90000083, "TEMP_HARD_CANDIDATE_OOM: Extnodes_base UVM alloc failed; continuing sets Extnodes = NULL - MaxPart (wild pointer) deref'd across the tree machinery; original endrun(3)", __FILE__, __LINE__, __FUNCTION__);
    }
    ...
    if(!Nextnode)
    {
        ...
        gizmo_hard_abort_reviewed(90000084, "TEMP_HARD_CANDIDATE_OOM: Nextnode UVM alloc failed; continuing aliases a NULL Nextnode and deref's it during the walk; original endrun(8267342)", __FILE__, __LINE__, __FUNCTION__);
    }
    ...
    if(!Father)
    {
        ...
        gizmo_hard_abort_reviewed(90000085, "TEMP_HARD_CANDIDATE_OOM: Father UVM alloc failed; continuing writes Father[i] through a NULL pointer; original endrun(438965237)", __FILE__, __LINE__, __FUNCTION__);
    }
...
void force_refresh_node_moments(void)
{
     ...
     * produce, so the CPU pseudo-particle path can run unchanged. */
    {
        ...
        force_refresh_hmax_per_type_host(Numnodestree);
        if(gpu_force_flag_localnodes() != 0)     {gizmo_hard_abort_reviewed(90000087, "TEMP_HARD_CANDIDATE_INTERNAL: GPU force_flag_localnodes failed (rank-local) in pseudo-refresh; invalid bitflags fed into force_exchange_pseudodata collective + GPU walk; original endrun(913340)", __FILE__, __LINE__, __FUNCTION__);}
        force_exchange_pseudodata();
        if(gpu_scatter_pseudo_to_soa() != 0)     {gizmo_hard_abort_reviewed(90000088, "TEMP_HARD_CANDIDATE_INTERNAL: GPU scatter_pseudo_to_soa failed (rank-local) in pseudo-refresh; invalid foreign moments fed into topnode resum + GPU walk; original endrun(913341)", __FILE__, __LINE__, __FUNCTION__);}
        if(gpu_topnode_moment_resum() != 0)      {gizmo_hard_abort_reviewed(90000089, "TEMP_HARD_CANDIDATE_INTERNAL: GPU topnode_moment_resum failed (rank-local) in pseudo-refresh; invalid tree moments handed to the GPU gravity walk; original endrun(913342)", __FILE__, __LINE__, __FUNCTION__);}
