...
{
  if(r<l){ gizmo_hard_abort_reviewed(7777, "TEMP_HARD_CANDIDATE_INTERNAL: original endrun(7777) -- parallel_sort r<l invariant (mid-collective sort; no symmetric poll)", __FILE__, __LINE__, __FUNCTION__);}
  const int w     = r-l;
  const int mid   = l + w/2;
  if(w <= 1) {
